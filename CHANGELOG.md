...
- `/repo desc [name] [<text>|clear]` — show or set the blurb. Setting one writes `<repo>/.claude/repo.json`, preserving any other keys, because the sentence describes the repo and should travel with a clone. `desc` and `deploy` joined `_RESERVED_REPO_NAMES` so neither can be shadowed by a repo of that name.
...
- Harness: `python scripts/test_repo_desc.py` (61 checks), registered in `.claude/test.json`.
