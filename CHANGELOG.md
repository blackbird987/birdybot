...
### Fixed
- Worktree-recovery scan no longer spams duplicate "lost git metadata" warnings for chained builds. `recover_partial_worktrees` now layers three filters (extracted as `_select_recovery_candidates` for testability): (1) skip FAILED/KILLED status — those worktrees are the cleanup pass's job, surfacing drift on crashed builds is noise the user can't act on; (2) in-pass dedupe by `(repo_path, branch)` so chained siblings sharing a branch produce one event, not N; (3) cross-pass dedupe via `flagged_branches` set so once any sibling is parked `manual_recovery_needed`, ALL siblings on that branch stay quiet on later reboots — without this the next-newest sibling re-fired the same warning every reboot. Verified: `cleanup_stale_worktrees` protects branches with `inst.branch` set and refuses `shutil.rmtree` on `git worktree remove` failure, so FAILED/KILLED worktree files are not destroyed by the filter narrowing.

### Tests
- New `scripts/test_worktree_recovery_dedupe.py` regression covers six scenarios: chained-sibling dedupe (4 → 1), FAILED/KILLED skip with COMPLETED/RUNNING/QUEUED kept, positive lock that COMPLETED is scanned, flagged-sibling silences whole branch, post-filter handles flag-found-late iteration order, and missing-required-fields skip.
