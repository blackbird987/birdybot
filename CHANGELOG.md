...
- **Four copies of the CLI's project-directory encoder had drifted apart.** The CLI names each project folder after the absolute working directory with `:`, `\`, `/` and `.` all flattened to `-`, and four places in the bot reimplemented that independently. Two of them never replaced `.`, so any path containing a dot encoded wrong — a build worktree at `…/claude-telegram-bot/.worktrees/t-6839` came out as `…-bot-.worktrees-t-6839` while the CLI had actually written `…-bot--worktrees-t-6839`, so "find the latest session for this repo" and the sessions-index rebuild could never match a worktree at all. All four now delegate to the single shared encoder that the runner and the JSONL forker already agreed on.
