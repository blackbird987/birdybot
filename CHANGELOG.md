...
### Fixed
- **Steer no longer renders a red "FAILED: Exit code 1" embed for the killed in-flight run (t-4135).** Real-world incident: every Steer tap in a Build thread (e.g. t-4126) produced a `❌ FAILED: Exit code 1 / 0s | Build / Mode | Build | master` card before the replacement run started. Root cause: `kill_and_wait` terminated the active CLI process, the runner saw `proc.returncode != 0` and stamped `RunResult.is_error = True` with `error_message = "Exit code 1"`, and `finalize_run` classified the instance as `FAILED`. There was no signal that the kill was user-initiated, so the standard FAILED delivery path fired. Fix is four-fold:
  - **Intentional-kill signal on `ClaudeRunner`** (`bot/claude/runner.py`). New `self._intentional_kills: set[str]` populated **only after `proc.terminate()` returns without raising** — so the set always means "we successfully sent the kill signal," never the weaker "we intended to." `kill()` gains a keyword-only `intentional: bool = False`; `kill_and_wait()` gains `intentional: bool = True` (every current caller — Steer at `interactions.py:586`, resolve-cancel at `commands.py:2808` + `commands.py:3028`, deploy preflight — is user-initiated, so the default reflects intent). The already-dead path (`kill()` early-returns `False` before terminate fires) naturally leaves the set untouched, eliminating that leak class.
  - **Race guard against real crashes coinciding with Steer.** In `_stream_output` after `proc.wait()`, the runner only flips `result.killed_intentionally = True` when both the set membership AND a kill-shape returncode are satisfied — negative returncode on POSIX (-15 SIGTERM, -9 SIGKILL). Read+discard runs inside `try/finally` so a downstream exception can never leave a stale entry stuck in the set. Windows limitation is called out explicitly in an inline comment: `proc.terminate()` always yields returncode 1 there, indistinguishable from real "exit 1" crashes, so the filter degenerates to a no-op on Windows — the mitigation is the next bullet.
  - **Preserve underlying error_message even when classified KILLED.** `finalize_run` (`bot/engine/lifecycle.py`) now branches the failure path: `if result.killed_intentionally: status = KILLED; inst.error = result.error_message or None` (keeps the real crash detail accessible via `/log` and the history file even though the user-facing embed is suppressed). Without this, a process that genuinely crashed in the same window we asked to terminate would have its error silently swallowed.
  - **Suppress the FAILED embed + edit thinking message to a tombstone.** `run_instance` short-circuits after `finalize_run` when `result.killed_intentionally` is set — placed BEFORE `schedule_cooldown_retry`, `notify_parent_on_finalize`, and `send_result`, so an intentional kill cannot trigger a spurious cooldown retry on the "Exit code 1" parse path nor falsely tell an orchestrator parent the child "FAILED" when we just steered it. The completion thinking-message edit at lifecycle:317 now reads `⚡ {id} steered ({elapsed})` instead of `❌ ... failed (...)` when `killed_intentionally` is true — that one-line edit is the minimal trace the user sees in the thread before the replacement Steer run renders its own progress. Belt-and-braces: `schedule_cooldown_retry` also has a defensive early-return on `result.killed_intentionally` so future code paths that bypass the run_instance early-return cannot accidentally schedule a retry of a Steered run.
