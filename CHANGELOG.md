...
### Added
- **The prompt reviews itself once a week** (`bot/engine/prompt_review.py`, `bot/discord/prompt_review.py`, `bot/discord/wizard.py`, `bot/discord/slash_commands.py`, `bot/claude/types.py`, `bot/app.py`, `bot/config.py`). Every session was already scored and every recurring flag already attributed to the prompt block that should have prevented it, and nobody ever read the result in aggregate: findings piled up on disk and the prompts that caused them never changed. A weekly run now aggregates the eval record deterministically, hands one fresh-context agent that bounded table (25 rows, 8K chars) and posts up to three proposed diffs into a **Prompt Review** thread in The Ark with Approve and Reject buttons. It proposes and never applies: the agent is created in explore mode with `bash_policy="none"` and `bash_policy_baseline="none"` (the read-only floor, since explore alone leaves Bash as a write backdoor through `sed`/`echo >`/`tee`), so it cannot edit the constraints it is reviewing, and Approve opens an ordinary build thread with the diff as its brief rather than writing files. Each finding must be classified `contradicted`, `disobeyed` or `obsolete`, and only the first and third may become an edit: a rule that is merely not being followed is reported and left alone, because deleting a rule for being hard to follow is exactly backwards. The gate reads a persisted timestamp stamped *before* the run, so a reboot neither re-fires it nor skips the week and a crash mid-review costs one week rather than looping; no stamp at all seeds instead of firing. A rejected proposal is remembered by its `(file, flag)` targets rather than its diff text, so the same idea coming back reworded is suppressed by the bot. `/promptreview [days]` runs it on demand (owner only). Knobs: `PROMPT_REVIEW_ENABLED`, `PROMPT_REVIEW_WINDOW_DAYS`, `PROMPT_REVIEW_INTERVAL_DAYS`, `PROMPT_REVIEW_MAX_PROPOSALS`, all inert without `EVAL_ENABLED`. Harness: `python scripts/test_prompt_review.py` (54 checks).
