...
### Fixed
- **A draft the bot created never appeared in Outlook** (`bot/services/outlook.py`). Every draft was reported as created, and every one of them was — the item was genuinely written to the Drafts folder, confirmed by reading the folder back over COM. It just wasn't visible, because two different Outlooks are involved. The mail client actually being read is the *new* Outlook (`olk.exe`), which is the web client in a window and shows only what the Exchange server holds. The COM integration drives *classic* Outlook, which isn't normally running, so each `draft` call silently booted a headless instance to do the work. Classic Outlook runs in cached mode against this mailbox (`IsCachedExchange` is true, `q.klerk@amsterdam.nl.ost`): a saved item lands in the local cache first and goes up to the server on the next send/receive. The headless instance exits within seconds of the script finishing, long before any scheduled sync, so the draft stayed stranded in a file on disk that the client being watched has no knowledge of — a "success" that was indistinguishable from doing nothing. `create_draft` now forces a send/receive after `Save()` and waits for it to complete, which both pushes the item up and keeps the process alive long enough for the push to happen. Completion is detected by binding to the sync group's `OnSyncEnd` event and pumping the COM message loop, capped at 45s; where event binding is refused it falls back to a fixed 20s pump, the shape that was verified by hand to move a stuck draft. The helper never raises — a sync that fails or times out still leaves the draft safely saved locally, so the failure mode is unchanged rather than worsened — and the result is reported instead of assumed: the CLI prints "synced to server" only when the sync actually finished, and otherwise warns that the draft may not show up until Outlook next syncs on its own.
