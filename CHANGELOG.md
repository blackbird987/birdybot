...
- **`scripts/test_account_failover.py`** grew the t-6570 replay (limit + 401-in-result-text → retry scheduled, not a failure), the doomed-spawn skip, the all-accounts-fail safety valve, the actionable dead-end message, and the login-busts-the-cache check. New **`scripts/test_account_alerts.py`** locks the notice: exactly one embed per outage, exactly one all-clear, snoozed alerts stay quiet, and the buttons aren't built for an account that has since left `CLAUDE_ACCOUNTS`.
