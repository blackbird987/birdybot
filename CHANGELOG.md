...
### Fixed
- **A build killed for "autocompact thrashing" now resumes itself instead of blocking a chain on a button whose only job is "run it again"** (`bot/claude/runner.py`, `bot/claude/parser.py`, `bot/config.py`, `bot/claude/types.py`). The message was never ours: the Claude Code CLI kills its own process, exit 1, when the context refills to the limit within 3 turns of a compact three times running — a self-protection guard against burning tokens in a compaction loop. We relayed it verbatim as a build failure, and 14 of them have landed since Aug 11, every one in the same large C# repo. Two came an hour apart in the same thread (`t-7357` at 3:05 PM, `t-7358` at 4:13 PM, 19 and 23 minutes of work each), both blocking a verify chain and paging the user, who clicked Retry once and had it work both times. It works because that counter lives in the CLI *process*, not the conversation: a resumed session starts from the compact summary with the counter back at zero. The click therefore carried no information the bot didn't already have, which is the definition of a click the bot should be making — it now resumes the session itself, bounded at `CONTEXT_THRASH_MAX_RETRIES` (default 2, so 3 attempts), then surfaces the real error with the Retry button exactly as before rather than looping. The cap is the honest part of this: a session whose context is structurally too heavy will keep tripping the guard — one conversation in the logs thrashed on Aug 11 and twice again on Aug 13 — and each attempt is a real 20-minute run, so "retry forever" would just spend the quota slower. Two details make the resume worth anything. The session id has to come from the stream's init event, not the `result` event: a killed process never emits the latter, so a *fresh* spawn that died after 50 turns previously reported no conversation at all, and neither the auto-resume nor the user's Retry had anything to resume — it started the work over, which is its own quiet version of this bug and is fixed here for every failure path, not just this one. And the resumed agent is told why its predecessor was killed, since the CLI's own "read in smaller chunks" advice is addressed to the operator and the agent just sees its process vanish; the note leads with `git status`, because every edit the aborted attempt made is still sitting in the worktree and an agent that assumes otherwise redoes it. Placed after the dead-session cascade and before the model-limit and account-failure branches, neither of which can match this text anyway. Covered by `scripts/test_context_thrash.py` (registered in `.claude/test.json`), which stubs only the subprocess boundary so the real cascade runs: a thrash-then-succeed pair returns the success and resumes the id the *killed* process created, the recovery note reaches the second attempt and not the first, a session that always thrashes stops at the cap with its real error intact, a plain compile failure is not retried at all, and — driving the real stream loop rather than a stub — a process killed before its result event still names its session. Both halves were checked to fail when disabled, so neither assertion is vacuous.
