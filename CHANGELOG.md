...
- Prior-deferred items injected into the plan-review prompt now deduplicate using the same normalized key (`StateStore._dedup_key`) that `append_deferred` uses on write — prevents semantically-equivalent rewordings from piling up across review rounds.
