...
### Fixed
- **A draft the bot created never appeared in Outlook** (`bot/services/outlook.py`). Every draft was reported as created, and every one of them was — the item was genuinely written to the Drafts folder, confirmed by reading the folder back over COM. It just wasn't visible, because two different Outlooks are involved. The mail client actually being read is the *new* Outlook (`olk.exe`), which is the web client in a window and shows only what the Exchange server holds. The COM integration drives *classic* Outlook, which isn't normally running, so each `draft` call silently booted a headless instance to do the work. Classic Outlook runs in cached mode against this mailbox (`IsCachedExchange` is true, backed by a local `.ost`): a saved item lands in the local cache first and goes up to the server on the next send/receive. The headless instance exits within seconds of the script finishing, long before any scheduled sync, so the draft stayed stranded in a file on disk that the client being watched has no knowledge of — a "success" that was indistinguishable from doing nothing. `create_draft` now nudges a send/receive after `Save()` and then holds the process open for 30 seconds while the upload happens — the holding open is the actual fix, and `SyncObject.Start()` is only a nudge. There is deliberately no completion check, because two obvious ones were tried against the real mailbox and both are mirages. Binding the sync group's `OnSyncEnd` event looks authoritative and is the first thing to reach for; instrumenting it shows SyncStart, Progress(0/1000) and SyncEnd all firing within 60ms, *before `Start()` even returns*, so it reports the send/receive group finishing and says nothing whatever about a cached-mode upload. Shipping that would have returned "synced" in a fifth of a second, exited immediately, and reproduced the original bug behind a success message — strictly worse than the bug, which at least did not claim to have worked. The second candidate, watching for the item to be re-keyed to a server `EntryID`, is likewise dead: the EntryID is byte-identical 45 seconds after creation. The upload is done by a background sync engine that reports through neither, so a fixed wait is the honest mechanism rather than a fallback. The 30s figure is empirical and no stronger than that: it is the wait that was used the one time a stranded draft was confirmed to reach the server, established as sufficient on this mailbox and not as a minimum. The helper never raises, which is load-bearing twice over: a failed nudge still leaves the draft safely saved locally, so the failure mode is no worse than before, and `create_draft` runs inside the module's retry wrapper, which would re-run the whole operation and create a *second* draft if a COM error escaped from the sync step. The reporting is scoped to what is actually known: the CLI says a send/receive was requested and waited out, states plainly that delivery to the server is not independently confirmable, and warns separately in the one class of failure that *is* detectable — the send/receive never being started, whether because the mailbox exposes no group or because the call to start it failed.
