...
### Fixed
- **A failed deploy push could post an access token into Discord.** Found while checking whether the new reason-picker was needed anywhere else. The push that runs before a deploy reported failures by slicing git's raw stderr to 200 characters and sending it — and when an HTTPS remote carries an embedded credential, git quotes the whole URL back at you inside `fatal: Authentication failed for 'https://user:ghp_…@github.com/o/r.git/'`. Nothing scrubs secrets on the way out: `redact_secrets` is applied at individual call sites, not at the send boundary, and this was not one of them, so the token went to the channel verbatim and a 200-character truncation does not remove something that appears near the start. Both the push and the tag push now route through the same picker as the merge notes, which redacts. The picker is public (`git_fail_reason`) for that reason — a leading underscore on a name imported from another package misdescribes the interface. Pinned by a test built from the exact stderr shape.
