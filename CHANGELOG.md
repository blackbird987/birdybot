...
- **A cooldown retry can no longer leave a thread it just ran in permanently sessionless** (`bot/app.py`). The retry built its `RequestContext` without ever calling `attach_session_callbacks`, so it had no way to register a session even in principle — the same omission that was already fixed once for post-reboot replays (`_replay_to_thread`). It now wires the callbacks and, after the run, fills the thread's session **only if the thread has none**. Fill-a-gap rather than rebind is deliberate: chain steps (plan, review, verify) hit usage limits too and their session belongs to the step, not the conversation, so rebinding from here would let a step amputate a thread's chat history on the cooldown path while never doing so on the normal one. Worktree builds are excluded on top of that, since a build's isolated session must not become a thread's chat session even when the slot is free.
...
### Added
- `bot/engine/lifecycle.should_bind_session()` / `bind_thread_session()` — the single place that decides whether a turn may claim the thread's session, and the single place that writes it. The chat path and the cooldown retry both go through them instead of open-coding the rule.
- `scripts/test_cooldown_session_bind.py` — 23 checks. Locks the eligibility rule (success and usage-limit bind; a plain crash and a recovery-exhausted crash don't; a recovered session that *then* hit the limit does), drives a real `ForumManager` to prove the bind reaches `ThreadInfo` and that the cross-repo rebind guard is untouched, and pins the two structural properties that were the actual bug: the bind sits above the cooldown early-return, and the retry's bind is fill-only. Both structural checks were mutation-tested — reverting either one fails the suite. Registered in `.claude/test.json`; the contract is written up in `CLAUDE.md`.
