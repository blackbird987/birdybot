...
- **Account health is visible at a glance.** `/status` gains an `**Accounts** — 1/2 usable | signed out: klerk` line, and the dashboard's usage label reads `· 1/2 accts` when one is down instead of the flat `· 2 accts` that made a dead rotation look healthy.
- **Waiting is explained honestly.** When a turn is rescheduled because the backup is logged out, the countdown says so — "Backup account logged out — waiting for your main account to reset, auto-retrying at 4:00 PM" — and carries an **Auth panel** button, rather than blaming a usage limit the backup never hit.
- **`scripts/test_account_failover.py`** grew the t-6570 replay (limit + 401-in-result-text → retry scheduled, not a failure), the doomed-spawn skip, the all-accounts-fail safety valve, the actionable dead-end message, the login-busts-the-cache check, the both-accounts-dead case (no fabricated wait, both accounts named), a one-turn answer *about* 401s that must not be read as a dead account, and the re-login that clears an auth cooldown but leaves a usage cooldown alone. New **`scripts/test_account_alerts.py`** locks the notice: exactly one embed per outage, exactly one all-clear, a snooze that goes quiet and then lapses exactly once, and buttons that carry the account label and aren't built for an account that has since left `CLAUDE_ACCOUNTS`.
