...
### Added
- **A repo can be hidden from Discord without being deleted** (`bot/store/state.py`, `bot/engine/commands.py`, `bot/discord/forums.py`, `bot/discord/channels.py`, `bot/discord/adapter.py`, `bot/discord/dashboard.py`, `bot/discord/slash_commands.py`, `bot/platform/base.py`). Twenty registered repos meant twenty forums in one category, most of them not being worked on, and the only ways out were unregistering the repo or deleting the forum with it. `/repo hide <name...>` now moves a repo's forum into a `<category> · Archive` category and drops it out of the dashboard's project list and the repo pickers; `/repo unhide` brings it back. Nothing is unregistered and nothing is deleted: the forum, its threads and its history are untouched, and `/repo list` still shows the repo under a `Hidden:` heading. Permission overwrites deliberately are not the mechanism: Discord shows the server owner every channel regardless of them, so denying `view_channel` would hide nothing from the person asking. `list_repos()` stays unfiltered because dozens of callers resolve a repo path through it for resume, merge, worktree and deploy work; only display surfaces read the new `list_active_repos()`. Work starting in a hidden repo un-hides it (`wake_repo_if_dormant`), so a schedule or a spawn can never post its result into a forum nobody is looking at. Harness: `python scripts/test_repo_hide.py` (29 checks).
