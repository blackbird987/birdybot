...
- `/repo desc [name] [<text>|clear]` — show or set the blurb. Setting one writes `<repo>/.claude/repo.json`, preserving any other keys, because the sentence describes the repo and should travel with a clone. `desc` and `deploy` joined `_RESERVED_REPO_NAMES` so neither can be shadowed by a repo of that name. It defaults to the *thread's* repo (`ctx.repo_name`) before the globally active one, since the command is typed inside a repo's forum and the last-switched repo is the wrong file to write. A repo whose directory is missing is refused rather than created by `mkdir(parents=True)`, and the file is replaced atomically so a crash mid-write cannot take its other keys with it.
...
- Harness: `python scripts/test_repo_desc.py` (65 checks), registered in `.claude/test.json`.
