...
- **A failed push now tells you what went wrong in Discord, not just in the log.** The merge note carried the exit code alone. That stayed hidden while git was hanging — the note said `Push to origin timed out (30s)` instead — but disabling the prompt turns those hangs into fast non-zero exits, which would then have arrived on the phone as a bare `Could not push to origin (exit 128)`, with the sentence naming the cause still stranded in `bot.log`. Fixing the hang without this would have swapped one uninformative message for another. Both the branch push and the tag push now append git's own explanation (`_git_fail_reason` in `runner.py`). Picking the right line takes some care and "the first one" is wrong: a credential failure does lead with `fatal: could not read Username ...`, but the far more common failure — a branch behind its remote — leads with `To github.com:owner/repo.git`, which explains nothing while looking like it should, and buries `! [rejected] ... (fetch first)` on the next line. So it prefers the first line git marks as a diagnosis (`fatal:`, `error:`, `remote:`, `!`) and only falls back to the first non-blank line if none is marked. `hint:` is deliberately not a marker, being the advice that follows the reason rather than the reason, and progress chatter is excluded for the same reason — a push refused by a server-side hook reports `remote: Resolving deltas: 100% (1/1), done.` *before* `remote: error: hook declined`, so matching on the prefix alone would have offered a percentage as the cause. Run through `redact_secrets` and length-capped, because it is user-facing and a remote URL can carry an embedded token.
