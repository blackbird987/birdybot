...
- **Signing back in also revives an account the *server* rejected.** The recovery above worked off the credentials file: a `/login` rewrites it, the probe re-reads it, the account returns. But the failure this whole feature exists for is the other kind — a token that is present and parses fine, and that the server refuses at runtime. Nothing on disk changes when that happens, so re-reading the file proves nothing, and the account sat out its full 24-hour sideline even after being signed back in. The bot now records *which* credentials file the server rejected (modification time and size) when it opens the sideline, and treats that file being rewritten as the proof a login happened — the one durable signal available. Because it is persisted, this survives a reboot, unlike the in-memory record of which cooldowns were auth-kind; without it, a restart during an outage would strand the account for the rest of the day.
- **`/status` and the dashboard no longer disagree with The Ark about a dead account.** Both counted an account as usable if its credentials file looked healthy, so an account the server had rejected — the exact failure being announced in The Ark — still read as `2/2 usable` and `· 2 accts`. Both now consult the same sideline table the notice does, through one shared partition helper, so the three surfaces cannot drift apart.
...
- **`scripts/test_account_failover.py`** grew the t-6570 replay (limit + 401-in-result-text → retry scheduled, not a failure), the doomed-spawn skip, the all-accounts-fail safety valve, the actionable dead-end message, the login-busts-the-cache check, the both-accounts-dead case (no fabricated wait, both accounts named), a one-turn answer *about* 401s that must not be read as a dead account, the re-login that clears an auth cooldown but leaves a usage cooldown alone, a server-rejected account that stays sidelined when its credentials file is merely re-read and returns the moment it is rewritten, and the three retry countdowns — each asserted to name the right wait, with the all-accounts-signed-out one checked for the absence of the word "reset". New **`scripts/test_account_alerts.py`** locks the notice: exactly one embed per outage, exactly one all-clear, a snooze that goes quiet and then lapses exactly once, and buttons that carry the account label and aren't built for an account that has since left `CLAUDE_ACCOUNTS`.
