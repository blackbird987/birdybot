...
- **The Linux login-terminal launch reported success when nothing opened.** It passed `-e` to every candidate emulator, but GNOME dropped that flag in favour of `--`, so a modern `gnome-terminal` exited non-zero without ever showing a window — and because only a missing *binary* was caught, that counted as a win and the user was told login was open. Each emulator now gets its correct argument form, the candidate list covers the current crop (kgx, ptyxis, alacritty, kitty, xfce4-terminal), a launched terminal has to still be alive a moment later to count, and total failure prints the exact command to run by hand instead of "check bot logs". The probe also moved off the Discord event loop, where its liveness waits would have stalled the heartbeat.
- **Verify could never restart a bot it found down on Linux.** `.claude/test.json` is static JSON with no way to branch per platform, and its `start`/`stop` named `start_bot.bat`/`stop_bot.bat` — files that do not exist on Linux. Both now go through `scripts/botctl.py` (`start|stop|restart|status`), which works on either OS, waits for the bot to actually claim its PID file rather than assuming a spawned process means a running bot, and stops the whole process tree so CLI children don't outlive it holding worktrees open. The `.bat` wrappers stay for double-clicking from Explorer.

### Added
- **A portability guard, so this class of bug fails loudly instead of silently.** Every problem above was invisible from whichever machine you happened to be sitting at. `python scripts/check_portability.py` fails on CRLF in the git index, a `.sh` that lost its executable bit, a filename Windows cannot check out (reserved names, illegal characters, trailing dots), a hardcoded drive letter or `/home/` path outside a platform branch, and a `.claude/test.json` command that only runs on one OS. The path check tokenizes rather than grepping lines, so a docstring documenting a Windows path and a `C:\ProgramData` default inside a `sys.platform == "win32"` branch are correctly left alone. Registered as a harness so the verify step runs it.
