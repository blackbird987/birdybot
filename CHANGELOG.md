...
### Added
- **The bot now reads how much memory the machine has, not just how much each session is using** (`bot/claude/memory.py`, `bot/claude/runner.py`, `bot/config.py`, `scripts/claude-bot.service`, `scripts/test_memory_guard.py`). On 2026-08-21 the user's desktop froze for a minute and the kernel ran a global out-of-memory kill that shot their browser — while every guard in this bot reported everything normal, because every guard in this bot was asking the wrong question. No session was over its ceiling. The bot's own cgroup recorded `oom_kill 0` and never left its policy. What actually happened is visible only in numbers nothing was reading: the cgroup was holding 12.08 GB of anonymous memory against a 12 GB `MemoryHigh` and had been throttled at that watermark 746,489 times, the machine's 8 GB of zram swap was at 100%, and the largest single process the bot was responsible for was not a session at all. Three separate blind spots, each now closed. **A real pressure reading** (`read_pressure`) combines free memory machine-wide, swap fill, `/proc/pressure/memory` stall percentage and the cgroup's own anon-vs-`MemoryHigh` into one `ok`/`tight`/`critical` verdict with the evidence attached in the words a message can use. Stall pressure is the rule that would have caught this earliest — a machine can be thrashing while `available` still reads gigabytes, because the kernel counts reclaimable pages as available right up to the moment it cannot reclaim them fast enough. Full swap alone is deliberately only `tight`: on zram a 100% figure is the normal state of a busy day and treating it as a crisis would hold every session forever. **Admission control** now sits inside the concurrency slot, before the spawn: `MAX_CONCURRENT` bounds how many sessions run at once and says precisely nothing about whether the machine can afford the next one, so until now the bot's entire response to a starving machine was to add another process to it. A critical machine reclaims first, then holds the start and tells the thread why in facts rather than in "waiting for memory" — with a bounded wait (5 min by default) after which it proceeds anyway, because pressure the bot did not create must not block work forever. It only ever holds on `critical`; holding on `tight` would mean holding most of the time on this machine and would train the user to ignore the message. **Cross-session arbitration** answers the question the per-session ceiling structurally cannot — five sessions at 2.5 GB never individually breach an 8 GB limit and together they finish a 32 GB box that is also running a browser. Before this the only thing that could act on that sum was the kernel's OOM killer, which is safe (the unit survives, by design) but silent: the session that died was never told why and the user saw an unexplained failure. Now the largest live session is chosen deliberately and told what happened, gated in cost order — too small to be worth taking, machine not actually in trouble, the memory is not ours (machine-wide pressure caused by a browser is not solved by killing our own work; the real offender would re-take it immediately and the session's run would be gone for nothing), reclaim first and re-check, not the largest of the live samples, and a cooldown so a crunch that one kill already fixed does not unwind the whole fleet. Every session's tree measurement is now shared for exactly this, and dropped the moment the session ends so a dead session's stale number can never win the contest and spare the live one that is genuinely the problem.
- **Memory in the bot's cgroup that no session owns is now found and reclaimed** (`bot/claude/memory.py`, `bot/claude/runner.py`). The single largest process the bot was responsible for during the incident was a detached Roslyn compiler server holding between 3.3 and 4.9 GB — 45% of the bot's entire footprint — parented to PID 1. `dotnet build` starts it deliberately detached and deliberately leaves it running so the next build is faster. Every memory guard here walks *downward* from a session's CLI process, so a reparented process leaves the tree entirely and becomes unattributable and unreapable: no session could be blamed for it, and killing any session would not have freed a byte of it. The cgroup is what makes it findable at all, because a process that leaves every process tree never leaves the cgroup it is charged to — so everything in our cgroup, minus everything reachable from the bot's own tree, is memory nobody owns. That definition is dangerous by itself, and three gates make it safe: only a short allowlist of known cache daemons (Roslyn, MSBuild nodes, `tsserver`) is ever killed, and only when it is old enough not to be mid-startup and idle enough not to be mid-build, because a compiler server *compiling* looks identical to a leaked one by parentage alone. Pids that an armed `/watch` is waiting on are excluded outright — a session that launched a long job with `setsid nohup ... &`, which is exactly what the watch feature tells it to do, is reparented and unowned by precisely the same test, and reaping it would silently destroy the job the thread is visibly sitting and waiting for. Anything else unowned and large is logged every time and touched never; a 4.9 GB process nobody can account for is worth seeing in `bot.log` whether or not anything is going to be done about it.
