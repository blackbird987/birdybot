...
### Changed
- **A finished answer now arrives whole instead of collapsing to a preview card** (`bot/engine/lifecycle.py`, `bot/config.py`). The bot posted a result inline only if it fit under 2000 characters — one Discord message. Everything larger became a card holding the first paragraph, capped at 500 characters by `extract_summary`, with an Expand button under it. Measured against the 643 stored result files, 568 of them (88%, median 4.4 KB) took that path: collapsing was the normal case, not the exception, and nearly every answer arrived as a stub the user had to tap. The inline ceiling is now `RESULT_INLINE_MAX` (6000, env-overridable) and the existing chunker — which already splits on line breaks and rebalances code fences — spreads it across two or three complete messages. Buttons and the mention still land on the correct chunk; that logic already handled multi-chunk output and was simply never reached.
- **Expand expands** (`bot/platform/formatting.py`, `bot/engine/commands.py`, `bot/claude/types.py`). Tapping Expand re-rendered the result sliced at 3900 characters with a `... truncated — use /log` footer, so on a 12 KB answer it showed a third and pointed elsewhere for the rest. `format_expanded_result_chunks` now returns the whole text as chunks: chunk 0 goes back into the result embed (4096 cap), the remainder posts as plain messages (2000 cap), bounded by `RESULT_EXPAND_MAX_CHUNKS` (6) so a 500 KB log can't carpet-bomb a thread. Only a result past that bound is cut, and the marker then names how many characters are missing rather than saying "truncated". Collapse — and a second Expand — delete the follow-up messages again, tracked in a new persisted `Instance.expand_msg_ids` so the cleanup survives a restart; without it a thread accumulates orphaned halves of an answer.
...
### Added
- `scripts/test_result_delivery.py` — harness over the whole delivery path: a normal answer arrives whole and loses nothing to chunking, a collapsed card carries multiple paragraphs and an honest count, Expand returns the full text and marks only a real cut, code fences stay balanced across every split, and the embed-cap cut announces itself while still respecting the cap. Registered in `.claude/test.json`.
