...
- The derive is cached in `data/state.json` under `repo_descriptions`, keyed on a stat-only signature of the source files. `refresh_control_room` runs on every instance start and completion, so a six-file read per refresh to render a sentence that changes once a month is exactly the cost this avoids; a repo that says nothing about itself caches the *miss*. The signature names every candidate that exists with its own mtime rather than collapsing to the newest — behind a source carrying a future mtime (tarball restore, skewed clock on a mount), a newly written `.claude/repo.json` would never move a maximum and the override would silently never apply.
- `/repo desc [name] [<text>|clear]` — show or set the blurb. Setting one writes `<repo>/.claude/repo.json`, preserving any other keys, because the sentence describes the repo and should travel with a clone. `desc` and `deploy` joined `_RESERVED_REPO_NAMES` so neither can be shadowed by a repo of that name. It defaults to the repo of the channel it was typed in, before the globally active one, since the command is typed inside a repo's forum and the last-switched repo is the wrong file to write. That takes both halves: the engine prefers `ctx.repo_name`, and `cmd_repo` fills it in — `_run_slash` builds its ctx with no repo at all, so the engine half alone would have been inert on the path that matters. The resolution is `ForumManager.repo_for_channel`, which falls back to the parent forum so the Control Room post — the surface that displays the blurb — resolves like any session thread. Deliberately scoped to `/repo`: setting it in `_run_slash` would also change which repo `/bg` runs in. A repo whose directory is missing is refused rather than created by `mkdir(parents=True)`, and the file is replaced atomically so a crash mid-write cannot take its other keys with it.
...
- Harness: `python scripts/test_repo_desc.py` (71 checks), registered in `.claude/test.json`.
