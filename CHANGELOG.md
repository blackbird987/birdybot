...
### Removed
- **Code with no reader, deleted rather than documented** (`bot/discord/bot.py`, `bot/discord/adapter.py`, `bot/discord/channels.py`, `bot/platform/base.py`, `bot/store/state.py`, `bot/engine/usage.py`, `bot/engine/eval.py`, `bot/discord/access.py`, `bot/claude/runner.py`, `bot/claude/models.py`, `bot/engine/ai_project.py`, `bot/engine/pending.py`, `bot/discord/spawn_colors.py`). Fifteen functions that nothing called, proved by tokenising every `.py`, `.md`, `.json`, `.toml`, `.sh`, `.service` and `.env` file in the repo into a reference count rather than by reading call sites. The largest was `_route_lobby_message`, the pre-forum path that routed an Ark message to a session, superseded by `get_or_create_session_thread` when the architecture moved to one forum per repo. Deleting `Messenger.create_conversation` cascaded: it was the only caller of `channels.create_thread` and `channels.create_task_channel`, whose removal in turn orphaned `adapter._get_guild` and three constructor fields, so the adapter's `__init__` is now one line. Seven unused imports went with them.
- **The Telegram shell** (`bot/telegram/`, `bot/config.py`). Four files left behind when Telegram support was stripped: an empty `__init__` and three identical two-line comments pointing at the `Messenger` protocol, which `bot/platform/base.py` documents anyway. Nothing imported the package. The three `TELEGRAM_*` settings went with it: `TELEGRAM_ENABLED` was a hardcoded `False` that nothing branched on, and the token and user id were parsed into module globals nobody read.
- **Five arguments and three record fields that were computed, passed and ignored** (`bot/claude/runner.py`, `bot/engine/workflows.py`, `bot/engine/commands.py`, `bot/store/state.py`, `bot/engine/auto_fix.py`, `bot/discord/interactions.py`, `bot/discord/monitoring.py`, `bot/claude/memory.py`, `bot/engine/usage.py`). `runner.check_spawn_allowed` took a `session_id` whose guard was removed when the per-channel lock took over serialising same-channel spawns; the body only checks the reboot drain now, so the argument had no reader, and a comment in the autopilot chain runner claiming that passing a session id would block the guard stopped being true with the guard. `StateStore.append_deferred` took a `thread_id` and a `topic` and wrote neither into `TODO.md`, with the chain runner loading the source instance purely to build the topic string. `auto_fix.spawn_fix_session` took an `error_summary` and an `error_output` and used neither, since all three callers already fold the error text into the `fix_prompt` the session actually receives. `CgroupMemory.oom_kills` was parsed out of `memory.events` on every read and never consulted, and `UsageBlock.start_time` and `end_time` were parsed from the CLI's usage JSON for a record only ever consumed for its token counts.
- **The one-shot thread recovery script** (`scripts/recover_thread.py`). Written in April 2026 for a single incident, hardcoding the thread id, session uuid and repo name it was meant to repair, and instructing in its own docstring that it be run once. The general capability shipped since: `/reset` unbinds a thread's session without stopping the bot, whereas the script SIGTERMs the running process and hand-edits `data/state.json` underneath it.

### Changed
- **Four near-copy pairs unified, none of them across a seam CLAUDE.md says must stay apart** (`bot/engine/commands.py`, `bot/claude/branch_utils.py`, `bot/claude/runner.py`, `bot/engine/workflows.py`). The Retry button and typed `/retry` shared fifty lines of clone-and-post-a-card, now `_start_retry`, which deliberately stops short of running the instance: folding the run in would drop `commands.py` from three `lifecycle.run_instance` call sites to two and break `test_cooldown_session_bind.py`, which asserts the count so that a fifth caller added later fails the suite instead of silently losing a thread's session. `clear_stale_branches` existed byte-identically in the runner and the workflow engine and now lives in `bot/claude/branch_utils.py`, the leaf both layers can import without closing the `bot.claude` to `bot.engine` cycle, re-exported from `workflows` so its six call sites in `commands.py` are untouched. Ten rungs of `_run_impl`'s recovery ladder each re-entered the turn with the same eleven arguments written out longhand, which is how a twelfth gets forwarded by nine of them and dropped by the tenth; one closure now carries them, reading `provider`, `binary` and `recovery_state` at call time exactly as the inlined copies did. The Merge and Discard button branches each ended in the same two blocks, the already-resolved report and the post-action tidy-up, and had already begun to drift, since only Merge carried the comment explaining why the stale branch is cleared from history. That is the `perform_kill` precedent: the drift between a button and its typed twin is what let one be fixed while the other kept producing red cards.

## v0.101.22, A session can ask what the bot is doing, and stops taking the whole machine (2026-09-08)

### Added
- **The bot can start a conversation on a rhythm, with somebody who is not the operator** (`bot/engine/nudges.py`, `config/nudges.json`, `bot/scheduler.py`, `bot/store/state.py`, `bot/claude/types.py`, `bot/config.py`, `bot/app.py`, `scripts/test_nudges.py`). The vin-coach workspace is a coaching tracker whose whole design assumes a daily end-of-day question and two rationed weekly chases; three weeks in, not one had ever fired, and nothing in the bot could have sent them. `/schedule` looked like the answer and is not: a cron spawns a fresh session against the *active repo* and broadcasts its summary to The Ark, so it can never put a message in front of the person being coached. The only thing that writes into a named thread is a self-wake, and it is a one-shot bound to its own channel with no way to target another. A **nudge** closes that gap as a recurring thread-bound wake: `_fire_wake` now re-arms a recurring row instead of consuming it, and `_step_from` advances the next fire from the row's own due time rather than from now, so a 17:30 nudge stays at 17:30 instead of drifting later by however long the 30s tick and the resumed turn took, and three days of downtime collapse to one message rather than three. They are declared in `config/nudges.json` and reconciled on every startup, not created by a command: a hand-made schedule lives only in gitignored `data/state.json`, and a habit engine that vanishes on a state reset fails *silently*, which is the one failure mode a nudge cannot have, because "he stopped replying" and "we stopped asking" look identical from the outside. Reconciliation matches on a stable `label` rather than the generated id, so editing a prompt updates the live row instead of orphaning one that keeps sending the old wording, and `delete_nudges_except` removes rows dropped from the file. `next_run_at` is re-anchored on every boot from the next wall-clock occurrence in the nudge's own IANA zone: preserving the stored value looks safer but stepping a UTC instant by 86400s reads an hour wrong once DST ends, which for this workspace is the exact week its October check-in lands. Re-anchoring cannot delay or double-fire because "next occurrence strictly in the future" is idempotent. Two collateral-damage guards: `add_wake`'s supersede sweep now skips recurring rows, so an ordinary poller armed in the same thread cannot delete the nudge as a side effect, and `recalculate_next_runs` skips labelled rows, whose generic missed-run recovery ("fire in 30s") would otherwise drop an evening nudge at whatever hour the bot restarted. A broken config is logged and leaves the live nudges running rather than clearing them, and a bad time, weekday or zone fails the whole file at load rather than silently dropping one entry. Harness: `python scripts/test_nudges.py` (48 checks).
- **A session can read the bot's own instance registry** (`scripts/instances.py`, `scripts/test_instances.py`, `CLAUDE.md`, `~/.claude-main/CLAUDE.md`). A session that had fanned work out with `/spawn` was asked whether its three children had finished and had to answer "I can't tell, run /list yourself", while `/list` held the full answer (two completed, one failed). `ListAgents` was the only thing that looked close and it is a different dataset: peer Claude CLI sessions on the machine, which knows nothing about bot instances. The new reader exposes the registry to anything that can run bash: `list`, `show`, `children`, `tree`, `log`, `diff` and `find`, each with `--json`. It reads `data/state.json` and `data/history.jsonl` and is read-only by omission, the way the mail and telegram readers in The Citadel are: no kill, no retry, no write path in the file, no `StateStore` (whose `save()` is one typo away) and no `bot.config` (which resolves `DATA_DIR` against the caller's cwd and drops a path marker on init). It resolves the *installed* bot's data directory through `procutil.install_root`, so it answers the same from another repo's working directory or from inside a build worktree. The join that makes `children` work is the load-bearing part: a `/spawn` child carries no `parent_id` (only button and chain steps do), so it is matched back to its parent through the forum thread it ran in, `history.jsonl` first and the live session-to-thread map only as a fallback, because a thread moves on and resolving by session alone reports a finished child under whatever is running there now. An instance that died before the CLI reported a session id *and* never finalized has neither link and reports as unlinkable rather than being guessed at from timing; that is the shape of a bot restart mid-run, and it is exactly what the failed child in the original incident looks like. A `RUNNING` row is annotated `live` / `STALE` / `nopid` from the recorded pid, so a run the bot lost track of is visible before the next startup sweep marks it failed. Branch and worktree path are surfaced first-class, since "where did its output go" was half the original question. Documented in this repo's `CLAUDE.md` and in the global instructions, since the failure happened in a session working on a different repo; the two Claude account config directories and `~/.claude` had drifted into three different sets of global rules, so they are now one file with the other two symlinked to it. Harness: `python scripts/test_instances.py` (48 checks), which also asserts the absence of a write path against the tool's own source.

### Changed
- **The machine's second resource is governed too, and the bot no longer costs you your desktop** (`scripts/claude-bot.service`, `bot/config.py`, `bot/claude/runner.py`, `bot/claude/memory.py`). On 2026-09-08 the desktop became unusable while every memory guard correctly reported a healthy machine: 15 GB free, memory PSI 3%, and the bot's own cgroup holding 0.88 GB of swap. The readings that mattered were never taken. Load average was 110 on 12 cores, one Roslyn `VBCSCompiler` held 605% CPU with six sessions live, CPU PSI ran 9-18% and IO PSI 11-24%, and the unit had no `CPUWeight` or `IOWeight` at all, so this cgroup fought Plasma and the browser at the default weight of 100. Three layers, because there are three distinct contests: `CPUWeight=20` and `IOWeight=20` on the unit settle bot versus desktop, and they are weights rather than a `CPUQuota` on purpose, since weights bind only under contention and an unattended machine still builds across all 12 cores at full speed. `SESSION_CPU_NICE` (10) settles the bot versus its own sessions: `CPUWeight` divides nothing *inside* the cgroup, which is why a 250 MB asyncio loop that must answer Discord within 3 seconds was competing at equal priority with six CLIs and a 605% compiler, and a missed gateway heartbeat disconnects. The renice is applied from the parent immediately after the spawn and deliberately not in a `preexec_fn`: CPython runs that in the child between fork and exec where only async-signal-safe work is legal, and this bot forks from a process with well over a hundred threading callsites, so a child landing on a lock held at fork time would hang the spawn forever while holding the runner's slot. The target is computed from the supervisor's own nice, so a unit that later grows a `Nice=` cannot silently close the gap, and niceness survives exec and is inherited, so one call covers the CLI, its shells, dotnet and Roslyn. Third, `MemoryPressure` now carries `cpu_psi_pct` and `io_psi_pct`, recorded and logged but never escalated on: with the weights in place CPU saturation costs throughput rather than anyone's desktop, and what was missing was never enforcement but that the next incident of this shape be diagnosable from `bot.log` alone. Each PSI signal keeps its own reader rather than sharing one parameterised entry point, so a test stubbing memory PSI cannot silently decide the CPU answer, and both new numbers print only beside a reading the verdict was actually made from, since a pressure read that could measure nothing must not look healthy because `/proc/pressure/cpu` happened to answer. The durable copy of the unit file is the one that matters: `~/.config/systemd/user/` holds an install-time copy that `migrate-off-windows-disk.sh` regenerates, so a setting added only there disappears at the next migration. Harness: `python scripts/test_memory_guard.py`.

## v0.101.21, A timer may be days long (2026-09-08)

### Changed
- **A timer can now be set for longer than a day** (`bot/config.py`, `bot/textutil.py`, `bot/engine/lifecycle.py`, `bot/engine/watches.py`, `bot/platform/formatting.py`). A self-wake was clamped to 24h and a `/watch` timeout to the same, so a session waiting on something that lands next week had no way to say so and had to either shrink the wait into a poll loop of short wakes or hand it back to the user. The ceiling for both is now 30 days (`WAKE_MAX_DELAY_SECS`, `WATCH_MAX_TIMEOUT_SECS`, both env-overridable). Nothing else in the path needed changing and that is the point: a wake is stored as an ordinary one-shot schedule, polled on the scheduler's existing 30s tick, persisted in `state.json` and never expired or pruned, so how far out it sits was only ever the clamp's business — verified end to end by arming a 3-day wake through the real directive parser and store, reloading it from disk as a reboot would, and confirming it lands 3.000 days out and is not yet due. It is deliberately not unbounded: a wake that fires weeks later resumes a session whose CLI transcript may have been cleaned up by then, and while the runner recovers from "No conversation found" by running fresh, the thread loses its history.
- **`/wake delay=` accepts the same unit suffixes `/watch timeout=` always did** (`bot/textutil.py`, `bot/engine/lifecycle.py`, `bot/engine/watches.py`, `bot/platform/formatting.py`). `delay=` was parsed with a bare `int(float(...))`, so `delay=3d` — the obvious spelling the moment the ceiling moves past a day — silently became the 180-second fallback with nothing said. Both directives now share one duration grammar (`45s`, `90m`, `6h`, `3d`, `2w`, or a bare number of seconds), and so does the collapsed directive chip, which previously swallowed any unit-suffixed delay on its `int()` and showed the user a wake with no stated duration. The parser moved from `bot.engine.watches` to `bot.textutil` because the chip renderer lives in `bot.platform`, upstream of `bot.engine`; a leaf module is the only home all three callers can reach without closing an import cycle, and `watches.parse_duration` stays importable as a re-export. It is anchored on purpose, so a typo like `3days` falls back rather than parsing as its `3d` prefix. `format_delay_secs` grew a days rung above 36h — a week-long timer rendered as "168.0 h" in both the "I'll check back in ~X" notice and the watch heartbeat's elapsed line. `WAKE_GUIDANCE` states the new range and units, and says outright that a delay of days is supported so a session does not shrink one into a poll loop. Harnesses: `python scripts/test_wake_directive.py`, `python scripts/test_watch.py`, `python scripts/test_botcmd_collapse.py`.

## v0.101.20, The eval record reviews the prompts that caused it (2026-09-08)

### Added
- **The prompt reviews itself once a week** (`bot/engine/prompt_review.py`, `bot/discord/prompt_review.py`, `bot/discord/wizard.py`, `bot/discord/slash_commands.py`, `bot/claude/types.py`, `bot/app.py`, `bot/config.py`). Every session was already scored and every recurring flag already attributed to the prompt block that should have prevented it, and nobody ever read the result in aggregate: findings piled up on disk and the prompts that caused them never changed. A weekly run now aggregates the eval record deterministically, hands one fresh-context agent that bounded table (25 rows, 8K chars) and posts up to three proposed diffs into a **Prompt Review** thread in The Ark with Approve and Reject buttons. It proposes and never applies: the agent is created in explore mode with `bash_policy="none"` and `bash_policy_baseline="none"` (the read-only floor, since explore alone leaves Bash as a write backdoor through `sed`/`echo >`/`tee`), so it cannot edit the constraints it is reviewing, and Approve opens an ordinary build thread with the diff as its brief rather than writing files. Each finding must be classified `contradicted`, `disobeyed` or `obsolete`, and only the first and third may become an edit: a rule that is merely not being followed is reported and left alone, because deleting a rule for being hard to follow is exactly backwards. The gate reads a persisted timestamp stamped *before* the run, so a reboot neither re-fires it nor skips the week and a crash mid-review costs one week rather than looping; no stamp at all seeds instead of firing. A rejected proposal is remembered by its `(file, flag)` targets rather than its diff text, so the same idea coming back reworded is suppressed by the bot. Its output is treated as a report rather than a turn: `lifecycle._NO_DIRECTIVE_ORIGINS` stops the post-turn scanners from acting on anything it writes, because its job is to quote the very documents that carry the literal `[BOT_CMD: /watch ...]`, `/spawn`, `/image` and `/repo add` examples, and the parsers' quoted-prefix guards do not catch an example indented inside a fence. Measured by driving the real turn-completion path with a proposal that quotes those examples: on an ordinary origin the bot spawned a real session, tried to upload `architecture.png`, tried to register `/path/to/myapp` and posted four messages of noise; on this origin it posted one message, the report itself. Its result card carries only Kill, Log and Expand for the same reason: The Ark is not a repo forum, so a Retry or Branch button on it would resolve against whatever repo happened to be globally active. `/promptreview [days]` runs it on demand (owner only). Knobs: `PROMPT_REVIEW_ENABLED`, `PROMPT_REVIEW_WINDOW_DAYS`, `PROMPT_REVIEW_INTERVAL_DAYS`, `PROMPT_REVIEW_MAX_PROPOSALS`, all inert without `EVAL_ENABLED`. Harness: `python scripts/test_prompt_review.py` (67 checks).

### Removed
- **The `tool_hygiene` eval check is gone, and adding one back would be wrong rather than merely noisy** (`bot/engine/eval.py`, `bot/engine/report.py`). It flagged every Bash `cat`, `head`, `sed -n`, `grep` and `find` as "should use the Read/Grep tool" while `bot/claude/provider.py` passes `--permission-mode bypassPermissions` on **every** run, and that mode's own system text instructs the session to prefer Bash for exactly those reads. The check was therefore scoring sessions against a rule the harness itself was telling them to break: its two messages fired 47,319 and 25,754 times over 30 days, roughly 73,000 of about 76,000 total flags, so the aggregate view was almost entirely one false positive and every real finding sat beneath it. With the category retired, the same 3,845-session window surfaces ten meaningful rows led by "Query took N turns" (847), "Low prompt-cache reuse on resume" (540) and "Long response with dense paragraphs" (306). A new `_RETIRED_CATEGORIES` set is skipped in `build_digest`'s grouping loop and in `/report`, deliberately not in `load_evals`: the flags stay on disk and a per-instance view still shows them exactly as recorded, since a history that rewrites itself makes an old session unexplainable. `attribute_flag` degrades a retired category to `"unattributed"` rather than naming an owner block that no longer has a check.

## v0.101.19 — A repo you are not using is hidden, and a guest reaches only their repo (2026-09-08)

### Added
- **A repo can be hidden from Discord without being deleted** (`bot/store/state.py`, `bot/engine/commands.py`, `bot/discord/forums.py`, `bot/discord/channels.py`, `bot/discord/adapter.py`, `bot/discord/dashboard.py`, `bot/discord/slash_commands.py`, `bot/discord/bot.py`, `bot/platform/base.py`, `bot/config.py`). Twenty registered repos meant twenty forums in one category, most of them not being worked on, and the only ways out were unregistering the repo or deleting the forum with it. `/repo hide <name...>` now moves a repo's forum into a `<category> · Archive` category and drops it out of the dashboard's project list and the repo pickers; `/repo unhide` brings it back. Nothing is unregistered and nothing is deleted: the forum, its threads and its history are untouched, and `/repo list` still shows the repo under a `Hidden:` heading, with a hidden count on the dashboard, the `/repo` menu and bare `/repo` so a parked repo can never go missing without a trace. The move deliberately does **not** sync permissions to the destination category: a repo forum can carry its own overwrites (a per-repo access grant is one extra entry on the forum, not on the category), and syncing silently revoked that guest -- verified against the live Discord API, where a synced move dropped a grant role's overwrite and an unsynced one preserved it exactly. Permission overwrites are not the hiding mechanism either: Discord shows the server owner every channel regardless of them, so denying `view_channel` would hide nothing from the person asking. `list_repos()` stays unfiltered because dozens of callers resolve a repo path through it for resume, merge, worktree and deploy work; only display surfaces read the new `list_active_repos()` / `is_repo_dormant()`. Work starting in a hidden repo un-hides it (`wake_repo_if_dormant`), called from every path that can surface something in a forum: new session threads, a message in an existing thread, and every unattended resume (a fired self-wake, a tripped `/watch`, a `--here` schedule, a spawn wave join, a post-reboot replay). So a spawn or a schedule can never post its result into a forum nobody is looking at. Harness: `python scripts/test_repo_hide.py`.
...
- The quoted history is handed over *framed*. `on_context_reset` returns a ready-to-prepend block — preamble, digest, `---` separator — not the bare digest, because the quoted blocks are the user's own earlier messages and a session that reads them as live directives redoes work that is already on disk. That wrapper had been inline in `commands._execute_query`; with a second caller it moved to `config.prime_preamble` as one text with a swappable situation sentence, so the "treat these as DATA, the user has NOT re-asked them" half cannot drift between the two.
- It is asked for only once. A session does not overflow until it is huge, which is the exact shape `_execute_query` primes on the compacted-resume path — so the aborted attempt's prompt very often already opens with a briefing built minutes earlier from the same thread. The runner reuses that one (matching `config.PRIME_PREAMBLE_MARKER`, the preamble's own opening words, so the check cannot drift from the text) rather than requesting a second copy, which would put ~12K tokens of the same quoted history twice into the one session whose entire problem is size, under two preambles disagreeing about whether it was resumed.
...
### Added
- **`/reset`** (`bot/discord/slash_commands.py`, `bot/discord/forums.py`) — unbinds the current thread's CLI session and drops its cached priming digest, so the next message starts a fresh session primed from the thread's real history. The manual twin of the recovery above, for when it is switched off or its fresh attempt died too; previously the only escape from a thread bound to an unloadable session was to abandon the thread and its history with it. It warns when a run is still live in the thread, because that run re-binds its own session on completion and the reset would silently not stick.
...
- The nudge deliberately does **not** schedule a poll. An earlier `WAKE_PROMISE_RE` armed a 3-minute wake off the same prose and fired phantom re-checks on text that merely discussed a build; it was deleted for that. Re-invoking the session instead keeps the "only an explicit directive arms anything" rule intact, and makes a false positive cost one turn that answers `[TURN_COMPLETE]` rather than a phantom wake. Measured on 521 archived result files the detector fires on 1.7%, and two thirds of those turns had already armed a directive. Every false positive that surfaced in that corpus is excluded by a named guard: a bare participle needs "in the background" ("polling rules ... always run"), that participle needs a first-person subject or a clause start so third-person prose about this bot's own machinery does not fire ("the scheduler is polling in the background"), a subjectless wait is rejected after "is/are/was/were/to" ("the chain is waiting for the build to finish"), and the first-person contractions require their apostrophe — with it optional, "id", "ill" and "im" read as "I'd", "I'll" and "I'm".
...
- The nudge stands down for a wake armed for *later*, not for the one being consumed. A firing wake resumes the thread and is deleted only after that turn returns, so on a wake-sourced turn the row still in the store is the one that just fired — counting it would have silenced the nudge for the likeliest case of all: a watch or a wake resumes the thread, the job is still running, and the turn promises to report back again.
- Both dead-end nudges now share one body (`_nudge_once`). They were near-copies of the same policy — hand off when out of context, stop at the cap, otherwise re-invoke after `WAKE_MIN_DELAY_SECS` — and drift between near-copies is what let the Kill button be fixed while `/kill` kept producing red cards.

### Added
- **The Control Room says what its repo is about** (`bot/engine/repo_desc.py`, `bot/store/state.py`, `bot/discord/channels.py`, `bot/discord/forums.py`, `bot/engine/commands.py`). The repo control room embed used the filesystem path as its entire description, so a forum of ten repos read as ten paths. It now leads with a one-line blurb and demotes the path to subtext. The blurb is derived rather than typed in — `.claude/repo.json`, then `CLAUDE.md`, `README.md`, `pyproject.toml` (`[project]`/`[tool.poetry]`), `package.json`, `Cargo.toml` — taking the first *paragraph* that reads as prose, skipping titles (both `#` and underlined), fenced code, bullets, quotes, HTML comments, rules, table rows and badge rows, then reducing it to one plain line under 120 chars. The paragraph rather than the line because most of these READMEs are hard-wrapped, so the first line ends mid-sentence and reads as truncation with no ellipsis to admit it; the fit then prefers the longest run of whole sentences that will fit ("Agentic media downloader." over the first 118 characters of the paragraph it opens), falling back to a word-boundary `…` cut when the leading sentence is too terse to describe anything. Checked against all 19 registered repos: 15 produce a sentence that reads correctly. The other 4 have nothing to derive from — two have none of the candidate files, one has a `package.json` with no `description` key, and one is a stale registration whose directory no longer exists.
- The derive is cached in `data/state.json` under `repo_descriptions`, keyed on a stat-only signature of the source files. `refresh_control_room` runs on every instance start and completion, so a six-file read per refresh to render a sentence that changes once a month is exactly the cost this avoids; a repo that says nothing about itself caches the *miss*. The signature names every candidate that exists with its own mtime *and size*, rather than collapsing to the newest mtime — behind a source carrying a future mtime (tarball restore, skewed clock on a mount), a newly written `.claude/repo.json` would never move a maximum and the override would silently never apply, and a file restored with its timestamp preserved but its content changed would otherwise serve the old blurb forever. Size rides along free out of the same `stat`.
- `/repo desc [name] [<text>|clear]` — show or set the blurb. Setting one writes `<repo>/.claude/repo.json`, preserving any other keys, because the sentence describes the repo and should travel with a clone. `desc`, `deploy` and `clear` joined `_RESERVED_REPO_NAMES`, so no repo can be named one of them — a repo called `clear` would have made `/repo desc clear` print a blurb instead of clearing one. It defaults to the repo of the channel it was typed in, before the globally active one, since the command is typed inside a repo's forum and the last-switched repo is the wrong file to write. That takes both halves: the engine prefers `ctx.repo_name`, and `cmd_repo` fills it in — `_run_slash` builds its ctx with no repo at all, so the engine half alone would have been inert on the path that matters. The resolution is `ForumManager.repo_for_channel`, which falls back to the parent forum so the Control Room post — the surface that displays the blurb — resolves like any session thread. It is handed the interaction's own channel rather than only its id, because a Control Room auto-archives like any forum post and discord.py evicts an archived thread from its cache, so the id-only lookup would miss exactly the case the fallback exists for. Deliberately scoped to `/repo`: setting it in `_run_slash` would also change which repo `/bg` runs in. A repo whose directory is missing is refused rather than created by `mkdir(parents=True)`, an existing `repo.json` that will not parse is refused rather than replaced (it is committed next to `test.json` and may hold hand-written keys), and the file is replaced atomically — with the temp file removed on failure so none is ever stranded in someone's `git status`.
- `Messenger.on_repo_meta_changed` — redraw a control room after its blurb changed. Separate from `on_deploy_state_changed` so the call site reads as what it is, even though both redraw the same surface today.
- Recording a blurb marks state dirty for the 60s auto-save rather than writing through: control rooms refresh on every instance start and completion, and a cache miss that forced a full rewrite of a multi-megabyte `state.json` would cost more on the miss path than the reads the cache exists to avoid.
- Harness: `python scripts/test_repo_desc.py` (85 checks), registered in `.claude/test.json`. There is deliberately no sync twin of `refresh_repo_description` — one existed with no caller but the harness, which is a standing invitation to fix one half and leave the tests passing against a path production never takes.
...
- `eval._check_unarmed_promise` — counts the recurrence and attributes it to `WAKE_GUIDANCE`, the prompt block that is supposed to make the session arm the directive in the first place. A recovery that fires often is a prompt problem, not a runtime one. It judges with the production predicates (`promises_continuation`, `armed_a_directive`, `promise_evidence`) rather than its own copy, and skips worktree builds, where `WAKE_GUIDANCE` is never injected.
- Harness: `python scripts/test_wake_promise_nudge.py` (74 checks), registered in `.claude/test.json`.
...
- `scripts/test_copy_block_wrapping.py` (registered in `.claude/test.json`) — 21 checks: the rule is in `WORKING_CONTEXT` and the prompt assembler still appends it, the real incident text is flagged and correctly attributed without shadowing the neighbouring over-long/mobile rules, the same email unwrapped is silent, every legitimate fence shape stays quiet, a wrapped fence beside a clean one flags exactly once, the flag's live count collapses so the digest can group it, and a result written with CRLF is still flagged — the last pinning a blind spot that would have made the check silently dead (`split("\n")` leaves a `\r` on every line, so no line ever ends on a word).
...
- **A child's report that arrives after its wave closed is delivered instead of dropped** (`bot/discord/orchestrator.py`, `bot/claude/types.py`). Once a wave was released, a straggler's finalize callback found no open wave and returned on a `debug` line nobody sees — so the child that the partial release had just described as "contributed nothing" finished, wrote a full report, and told no one. That is what forced the conductor to fall back to its own self-wake and go digging. A late child is now posted on its own with its full report path and the correction that whatever the parent was told about it then is out of date, auto-resuming the parent only once the last outstanding child of that wave is in (otherwise a button, so N stragglers cannot burn N parent turns). Recorded per child in `Instance.spawn_late_reported_thread_ids`, so a retry or replay cannot post the same report twice, and a wave old enough to have been retired (12h+) still shows its late child's work but never resumes a parent that moved on days ago. This is the safety net that makes a wrong release recoverable rather than lossy.
- **...but only for a child the wave could not account for** (`bot/discord/orchestrator.py`, `bot/claude/types.py`). A child thread stays usable after its wave closes — the user asks a follow-up, a retry re-finalizes it — and every one of those is a terminal outcome on a child with a closed wave, so an ungated late path would have posted "Late report" and woken the parent for ordinary chat. The release now snapshots which children it could **not** close cleanly (`Instance.spawn_wave_unresolved_thread_ids`, written in the same await-free block as the released flag) and only those may be reported late. Gating on "the release was partial" instead would fail in the other direction: a child that hits a usage limit is recorded as failed and *settled*, so its wave closes as complete — and the report from its retry, which is the thing the parent is actually waiting for, would be dropped. Waves recorded before this change carry an empty list and are inert, which is also what retires the historical backlog.
...
- `scripts/test_orchestrator_join.py` gains 21 checks across two new cases (78 -> 99). A live child holds a past-deadline wave open (by session *and* by channel, for a child too young to have a session yet) while a stale RUNNING record still times out and the ceiling still forces a release. And, driving a real release rather than a hand-built one, a late child's report is delivered exactly once, carries its report path, auto-resumes only when nothing else is outstanding, is posted without a resume for a retired wave, and is not raised at all for a child the wave had already reported as completed.

## v0.101.13 — A paused turn remembers which thread it belongs to (2026-08-27)

### Fixed
- **A turn that pauses for a usage limit no longer loses the thread's session** (`bot/engine/commands.py`, `bot/engine/lifecycle.py`). `_execute_query` wrote the finished turn's `session_id` onto the thread *after* the cooldown early-return, so a turn that ended on a usage limit never registered its session at all — and the retry path (`app._do_cooldown_retry_locked` -> `lifecycle.run_instance`) never bound one either. On 2026-08-27 that stranded three overnight ev-nova children: all three hit the weekly limit at 00:38, were retried on the backup account at 01:55, finished real work by 02:45, armed a `/watch`, and then had their wake-ups dropped as `thread gone/sessionless` — because `ThreadInfo.session_id` was still `None` while the instances held a perfectly resumable id (`q-15431` failed on `76cd59ce…`; its retry `q-15434` resumed that same session and completed). A usage limit is a pause, not an ending, so the bind now happens **before** the cooldown return. Every other error still skips it — a crashed or recovery-exhausted run can emit a *fresh* session id, and adopting that would amputate the thread's history. The bind is also wrapped: having moved above the cooldown scheduling and the result delivery, a failing state write must not be able to cost the turn its retry or its answer.
- **Retrying a turn no longer leaves the thread it ran in permanently sessionless** (`bot/app.py`, `bot/engine/commands.py`, `bot/engine/lifecycle.py`). Four paths run a turn through `lifecycle.run_instance` directly instead of through `_execute_query`: the cooldown auto-retry, `/retry`, the Retry button and **continue-on-pay-per-use** — the last being the manual twin of the cooldown retry, offered on the very usage-limit card this change exists to fix. `run_instance` deliberately never registers a session, so none of them did, and a thread that was already sessionless stayed that way no matter how many times the work was re-run. The cooldown retry additionally built its `RequestContext` without `attach_session_callbacks`, so it had no way to register one even in principle — the same omission already fixed once for post-reboot replays (`_replay_to_thread`). All four now call one shared `lifecycle.backfill_thread_session()`, which fills the thread's session **only if the thread has none**. Fill-a-gap rather than rebind is deliberate: chain steps (plan, review, verify) and instances belonging to other threads reach these callers too, and their session belongs to the step, not the conversation — rebinding from here would let one of them amputate a thread's chat history. Worktree builds are refused outright, since a build's isolated session must not become a thread's chat session even when the slot is free.
- **A self-wake into a live-but-sessionless thread dispatches cold instead of vanishing** (`bot/app.py`). "Thread is gone" and "thread exists but has no session" were one branch, both silently dropped at INFO. They are now separate: a genuinely missing thread is still dropped, while a live one gets the wake body dispatched as a fresh session, logged at WARNING. A wake body is written to stand on its own ("read the log tail, report the numbers"), so a cold turn in the right thread beats eight hours of silence.
- **Corrected a comment that asserted the opposite of the truth** (`bot/engine/workflows.py`). The autopilot chain runner claimed "individual steps register their session via lifecycle.run_instance". `run_instance` does not bind a session and never has — that belief is the shape of this whole bug. It now says so explicitly and points at the CLAUDE.md contract.

### Added
- `bot/engine/lifecycle.should_bind_session()` / `bind_thread_session()` / `backfill_thread_session()` — the single place that decides whether a turn may claim the thread's session, the single place that writes it, and the fill-a-gap-never-rebind variant the four direct `run_instance` callers share. `on_session_resolved` now has exactly one call site in the whole codebase.
- `scripts/test_cooldown_session_bind.py` — 33 checks. Locks the eligibility rule (success and usage-limit bind; a plain crash and a recovery-exhausted crash don't; a recovered session that *then* hit the limit does), drives a real `ForumManager` to prove the bind reaches `ThreadInfo` and that the cross-repo rebind guard is untouched, proves the backfill fills an empty binding but never rebinds and refuses a worktree build, and pins the two structural properties that were the actual bug: the bind sits above the cooldown early-return, and **every** direct `run_instance` caller outside the chain runner backfills — so a new one added later without it fails the suite rather than silently reintroducing the loss. All structural checks were mutation-tested. Registered in `.claude/test.json`; the contract is written up in `CLAUDE.md`.

## v0.101.12 — A long run is killed for going quiet, not for getting old (2026-08-27)

### Fixed
- **A long-running session is no longer killed for being old while it is still working** (`bot/claude/runner.py`, `bot/config.py`). The 4h safety-net exists — by its own docstring — to catch "truly orphaned processes", but it only ever measured how *old* a process was, never whether it was still doing anything. On 2026-08-27 that killed q-15433, a four-hour benchmark run that had produced output **five and a half minutes earlier**, had not gone silent for even sixty seconds across its final two hours, and was sitting at 350 MB with live HTTPS connections open and 1,843 stream events processed. It died mid-batch, waiting on subagents, purely for turning four hours old. Raising the number would only have moved the guillotine: a bench that farms work out in serial subagent batches can legitimately run all day. Age is now the point at which the watchdog *starts asking*, and continued output is the answer that keeps a run alive — a reap requires `MAX_PROCESS_LIFETIME_SECS` of age **and** `MAX_PROCESS_SILENCE_SECS` (new, 30m) of producing nothing. Only two lifetime kills had ever fired in the log; the other (q-15010) had been silent for 43 minutes and is still caught. A legitimate over-age run now logs "still producing output" hourly instead of dying.
- **A reaped run keeps the work it did instead of reporting an empty failure** (`bot/claude/runner.py`). The kill path returned a bare `RunResult`, so four hours of real work came back as a red FAILED card with no text, no tools, no cost and — worse — no `session_id`, which is what makes Retry a *resume* rather than a fresh start. Both watchdog reaps now build their result through one shared salvage step (`_reaped_result_base`) that names what a reap has to preserve, recovering the last assistant message, the tool record the chain reads to decide whether a build changed code, the token counters, and the session id captured from the init event. The session is also told why it is stopping *before* the terminate, since terminating closes stdout and cancels the watchdog mid-sentence. `num_turns` is deliberately left alone — the account-failover heuristic reads `>1 turn` as proof the account took the turn, so inventing turns could hide a dead backup account.
- **The failure message says which condition fired, in hours and minutes** (`bot/claude/runner.py`). "Process exceeded 4h lifetime limit" is what made the unwarranted kill look reasonable in the log; it now reads "no output for 43m after 4h of running", or names the hard ceiling when that is what fired. The phrase "lifetime limit" is retained on purpose and pinned by the harness: `parser.is_account_agnostic_error` matches on it to stop a reaped run being handed to the backup subscription to burn the same hours again.
- **A reap that races the user's Kill honours the Kill** (`bot/claude/runner.py`). The orphan reap returned before the two stand-downs the memory reap already had, so a Kill or Steer landing in the same few seconds as the watchdog's decision would have rendered as a red FAILED card instead of the quiet tombstone — the same class of bug as v0.101.11, with the same consequence, since a failure with no turns is the account-failover heuristic's signature for "this account fell over instantly". Both reaps now share both stand-downs: a `result` event proving the turn finished, and the session being on the intentional-kill list. The checks stay behind a single guard so the event walk they need never runs on an ordinary completion.

### Added
- `MAX_PROCESS_SILENCE_SECS` (30m) and `MAX_PROCESS_HARD_LIFETIME_SECS` (24h, `0` = off) in `bot/config.py` and `.env.example`. The hard ceiling is the backstop for the opposite failure — something that heartbeats forever without ever finishing should not be immortal just because it is noisy.
- `WATCHDOG_TICK_SECS` (10s) — the poll cadence shared by the stall warning, the memory guard and the lifetime cap, extracted from a hardcoded `sleep(10)` so the harness can scale the whole watchdog down instead of sleeping through real hours.
- `OLD_BUT_ALIVE_RELOG_SECS` (1h) — how often an over-age run that is still producing output says so in the log. At the 10s watchdog tick it would otherwise write 360 lines an hour; hourly is enough to leave a paper trail for the next time someone asks why a run lived for nine hours.
- `scripts/test_lifetime_cap.py` — five contracts. Four drive the real `_stream_output` against a fake process, with the watchdog knobs scaled down to seconds: an over-age run that keeps talking survives (the q-15433 regression), an over-age run that goes silent is reaped and its failure carries the recovered text, session id and tool record, the hard ceiling still stops a permanently chatty process, and a Kill arriving mid-reap wins. The fifth is a pure check that both new wordings still trip the failover suppression. Registered in `.claude/test.json`; the contract is written up in `CLAUDE.md`.

## v0.101.11 — An interrupted session stops instead of crashing (2026-08-26)

### Fixed
- **Interrupting a session ends it quietly again, instead of reporting a crash** (`bot/claude/runner.py`, `bot/engine/commands.py`). Tapping Kill produced the expected "Killed [q-15391]" message and then, one second later, a red **"Failed: Exit code 143"** card with Retry/Log under it. Nothing in the kill path was broken — the whole "this was deliberate, render a quiet tombstone" machinery was intact and unreachable, because the single boolean it hangs off had stopped being computable. That boolean corroborates a kill by asking whether the process really died from the signal we sent, and it accepted only a *negative* returncode, which is how Python reports a process the kernel killed for ignoring the signal. The Claude CLI is now a compiled binary that **traps** SIGTERM, shuts down cleanly and exits normally with 143 (128+N), so every Kill and every Steer failed the test and fell through to the failure path. Both shapes are now accepted (`is_kill_shape`), for SIGTERM, SIGKILL and SIGINT; ordinary exits (0, 1, 2, 255) are still failures, so a crash that lands during a Steer is not laundered into a cancellation. Verified against the CLI itself: terminating it returns 143, not -15.
- **The same fix stops a killed session being restarted on the backup account** (no new guard — `scripts/test_kill_shape.py` pins it). The same log second carried `Account error unmatched by auth patterns (no-turns heuristic)`. A killed run has no output and no completed turns, which is exactly the account-failover branch's signature for "this account fell over instantly", and the early-return that keeps a cancelled run away from it keys off the same dead boolean. That early-return was already there and correctly placed; restoring the boolean is what made it effective again. With two accounts configured and a spare one free, killing a build would have silently handed it to the other subscription and started the work over. Nothing was free when this was caught. The ordering it depends on is now asserted rather than trusted to a comment.
- **`/kill` typed as a command now behaves like the Kill button** (`bot/engine/commands.py`). It called the runner's plain `kill()`, whose `intentional` flag defaults to False, so it never announced the kill as deliberate — the red card would have survived the fix above. Both now run the *same* function (`perform_kill`) rather than two near-copies — the drift between those copies is what let the button be fixed and the command left behind. It guarantees the channel lock is released (via finalize, or the 10s force-clear), waits for finalize before reporting, and attaches the same Retry/Log buttons.

- **A stopped session's progress card reaches a terminal state whichever way you stop it** (`bot/engine/lifecycle.py`, `bot/claude/types.py`, `bot/claude/runner.py`). Lifecycle skipped its terminal edit of the live progress message whenever the kill reason was `"kill"`, because the Kill button rewrites that exact message into "Killed [id]". That reasoning holds only for the button, which edits the message it is attached to; typed `/kill` posts a separate message and would have left the card reading "thinking…" for the rest of the thread's life. The suppression now keys off what it actually means — `RunResult.kill_owns_card`, set only when the caller passes the message id it is going to rewrite — so `/kill` gets its card resolved and the button still keeps sole ownership of its own. A stopped run also reads as **⏹ stopped** rather than "steered": "steered" is only true when a replacement run is already starting.

### Added
- `scripts/test_kill_shape.py` — regression harness for interrupting a session. Beyond the shape table it drives **real subprocesses**: a child that traps SIGTERM and exits 143 the way the CLI does, and a control child that ignores SIGTERM and has to be SIGKILLed, asserting against the returncode each one actually produced rather than a constant. That is the arm that would have caught the CLI's behaviour change. Also asserts a killed run never reads as an unproductive account, that the killed early-return still precedes the account-failover branch in `_run_impl`, that finalize maps a kill to KILLED while preserving the error text for `/log`, that `/kill` reaches `kill_and_wait` with `reason="kill"` without claiming the progress card, and that the Kill button does claim it and rewrites the message it sat on. A sixth arm drives the real `run_instance` end to end and asserts the progress card ends on "stopped" for a typed `/kill` and is left untouched for the button. `--live` terminates the installed CLI itself and checks the returncode it actually produces (measured: 143) — everything else tests what the bot does with a returncode, that arm tests what the CLI gives it. Registered in `.claude/test.json`; the contract is written up in `CLAUDE.md`.
...
### Changed
- **A finished answer now arrives whole instead of collapsing to a preview card** (`bot/engine/lifecycle.py`, `bot/config.py`). The bot posted a result inline only if it fit under 2000 characters — one Discord message. Everything larger became a card holding the first paragraph, capped at 500 characters by `extract_summary`, with an Expand button under it. Replaying the 529 stored result files through the real formatters: **407 of them (76%, median 3.3 KB of display text) took that path**, so collapsing was the normal case rather than the exception, and nearly every answer arrived as a stub the user had to tap. The inline ceiling is now `RESULT_INLINE_MAX` (6000, env-overridable) and the existing chunker — which already splits on line breaks and rebalances code fences — spreads it across up to four complete messages. On the same corpus that leaves 55 results (10%) collapsed, and the rest posted whole: 122 as one message, 199 as two, 136 as three, 17 as four. The multi-chunk mention and button placement it now depends on was already written and was reachable only when a short result's footer tipped it past 2000; the change moves it from an edge case to the normal path.
- **Expand expands** (`bot/platform/formatting.py`, `bot/engine/commands.py`, `bot/claude/types.py`). Tapping Expand re-rendered the result sliced at 3900 characters with a `... truncated — use /log` footer, so on a 12 KB answer it showed a third and pointed elsewhere for the rest. `format_expanded_result_chunks` now returns the whole text as chunks: chunk 0 goes back into the result embed (4096 cap), the remainder posts as plain messages (2000 cap), bounded by `RESULT_EXPAND_MAX_CHUNKS` (6) so a 500 KB log can't carpet-bomb a thread. Only a result past that bound is cut, and the marker then names how many characters are missing rather than saying "truncated" — with room for that marker held back from the final chunk's budget, since appending it afterwards is what pushes a message past the cap it was sized for. Collapse — and a second Expand — delete the follow-up messages again, tracked in a new persisted `Instance.expand_msg_ids` so the cleanup survives a restart; without it a thread accumulates orphaned halves of an answer.
- **The card behind a genuine collapse is worth reading** (`bot/platform/formatting.py`). For results still too large to post inline, `format_result_md` takes the result text and shows its leading paragraphs up to `RESULT_PREVIEW_MAX` (1200) — cut on a paragraph, line or word boundary — plus how many characters sit behind the button, instead of one 500-character paragraph out of several KB. Directives and legacy verify fences are folded out of the preview the same way the inline path already folds them, so a `~~~plan` body can't eat the budget. With no preview passed it falls back to the stored summary exactly as before.
- **The prompt asks for a shorter answer, and stops demanding a long one** (`bot/config.py`). Brevity was one sentence in `MOBILE_HINT`; against it stood a much longer, more imperative block ordering per-tool narration ("You must narrate your work: if you read a file → summarize…"), four bad/good example pairs, and a storytelling section forbidding terse output. The verbose instructions were longer, more specific and more numerous, so they won. `CHAT_APP_CONSTRAINT` is rewritten around outcomes rather than steps — what is true now, what changed, what it means, not a log of what was done — with an explicit ~1200-character target and a note that length costs the reader, because a long answer gets collapsed or cut. The rules that exist for a reason are kept: plain language over identifiers (a report built out of `SomeMethodName` is unreadable to someone without the code open), subagent findings must be written out, never refer back to text the user may never have seen. The examples are cut — they teach verbosity as much as they teach the rule. Net effect on the block: 3457 characters down to 1966.

### Fixed
- **Resolving a branch no longer overwrites what the session said** (`bot/engine/commands.py`). `_strip_post_merge_buttons` does one job — take the Merge/Discard buttons off a finished result — but it did it by re-rendering the whole message from `format_result_md()`. On a result posted inline that meant editing the last message of the answer and replacing its text with a two-line summary card; on a collapsed one it threw away the preview. `edit_text` already accepts `None` for the body and updates only the buttons, which is what it now passes. The bug predates this change and was survivable when almost everything was a card; raising the inline ceiling would have made it routine.
- **Three places stopped guessing whether a result was collapsed, and the delivery path records it instead** (`bot/claude/types.py`, `bot/engine/lifecycle.py`, `bot/engine/workflows.py`, `bot/engine/commands.py`). Whether to keep an Expand button on a message was inferred in two separate spots from the result *file* being at least 2000 bytes — a proxy for, but not the same quantity as, the display text the delivery path actually measures, since directives are folded away before the comparison. A file can be well over the line while the text shown is under it, in which case both spots hang an Expand button on a message that already holds the whole answer, and tapping it overwrites that text with the expanded head. `send_result` now sets `Instance.result_collapsed` in every branch and the consumers read it, which deletes both size heuristics rather than re-tuning them.
- **The last-resort truncation no longer happens in silence** (`bot/discord/formatter.py`). `apply_discord_safety` sliced anything over the embed cap and returned it with no marker, so content vanished mid-sentence with no sign that anything was missing — the failure mode that makes truncation worse than useless, since the reader cannot tell a cut answer from a complete one. It now reserves room and appends the number of characters dropped plus a pointer to `/log`, while still guaranteeing the output is within the cap and its code fences balanced.
- **Run facts survive the inline path** (`bot/platform/formatting.py`, `bot/engine/lifecycle.py`). Duration, tokens, cost and mode are rendered as embed *fields*, and a plain message has no fields — so raising the inline ceiling would have quietly dropped all of them for every result between the old threshold and the new one. `format_inline_meta_line` puts the same facts on one grey `-#` line at the end of the last chunk, alongside the model and worktree that were already there.
- **A preview or chunk that lands mid-code-block closes it** (`bot/platform/formatting.py`). An odd number of ``` doesn't just look wrong: everything after it renders as monospace, which on a preview card swallowed the "+N more characters" line and the duration/cost line into the code block. The expand chunker already reopened fences across a split; the preview cut now closes them too, through a shared helper. The boundary search itself is also shared now (`_cut_index`) instead of being written twice with different fallbacks — and it looks only in the back half of the budget, so a newline near the start can't produce a near-empty message.
- **A ping no longer arrives as a message containing nothing but the mention** (`bot/engine/lifecycle.py`). The mention is prepended to the first chunk when it fits, and a chunk that filled to the 2000-character cap has no room — which got likelier the moment the inline ceiling went up. It now falls back to the end of the last chunk before giving up and sending itself.
- **Chunks are measured after table conversion, not before** (`bot/discord/adapter.py`). Pipe tables are rewritten into padded code blocks at send time, which grows the text; measuring before that meant a chunk could pass the 2000-character check and be cut on the way out. The Discord chunker now converts first and splits the converted text.
- **The verbosity eval fires on the shape the bot actually produces** (`bot/engine/eval.py`). The check only triggered on a direct question over 4000 characters *and* only when paragraphs also averaged 300+ characters, so a 6000-character answer written in tidy short paragraphs never registered. It now flags any response past 2500 characters, escalating from info to warning once the result is large enough to collapse behind Expand, and is attributed to `CHAT_APP_CONSTRAINT` — where the length target now lives — rather than to `MOBILE_HINT`.

### Added
- `scripts/test_result_delivery.py` — 44 checks over the whole delivery path: a normal answer arrives whole and loses nothing to chunking, a collapsed card carries multiple paragraphs and an honest count, Expand returns the full text and marks only a real cut, the cut marker is paid for out of the budget rather than bolted on past it, code fences stay balanced across every split, the inline meta line carries the run facts, and the embed-cap cut announces itself while still respecting the cap. Registered in `.claude/test.json`. Backed by a replay of all 529 stored results through the real formatters: no message over its Discord cap, no unbalanced fence.

## v0.101.8 — A session gets the work, not the bot's keys (2026-08-26)

### Security
- **A session no longer inherits the bot's own Discord identity** (`bot/claude/runner.py`). The spawned CLI got a copy of the bot's entire environment, and because `bot/config.py` loads `.env` with `override=True`, that meant every value in `.env` — including the Discord bot token and both test webhook URLs — was one `env` away from any session. Nothing a session legitimately does needs them, so `DISCORD_BOT_TOKEN`, `TEST_WEBHOOK_URL`, `TEST_LOBBY_WEBHOOK_URL` and `TEST_WEBHOOK_IDS` are now stripped before the spawn. The same list is applied to the log-triage subprocess in `bot/discord/log_triage.py`, which is a Claude CLI process by the same definition and had the same inheritance — it reads a log off stdin in a temp directory and needs none of them. Deliberately a denylist rather than an allowlist: an allowlist drops whatever per-project variable nobody predicted — a toolchain home, a proxy setting, `SSH_AUTH_SOCK`, `GH_TOKEN` — and fails confusingly mid-chain. `OPENAI_API_KEY` and `TWITTER_BEARER_TOKEN` are left in place for the same reason, since other repos may read them from the inherited environment. This is not a containment boundary — `.env` is still a readable file — it closes the accidental path: an env dump while debugging, a crash trace, a logged subprocess environment.
- **The shared redactor now strips secrets by value, not only by shape** (`bot/platform/formatting.py`, `bot/discord/log_triage.py`). The existing patterns recognise credential *formats*, and an audit confirmed the two that matter most to this bot have no recognisable format: a bare Discord bot token and a Discord webhook URL both survived every pattern, bare and in `NAME=value` form alike — so a session that printed one into a thread published it verbatim. The real values are now collected from config once and replaced literally, which has no false positives by construction and needs no new pattern when a credential format changes. This is the approach `log_triage` had used all along; it just lived in one module instead of on the path every message takes — so that module's private copy is deleted rather than left running alongside the shared one, and the secret-name list now has exactly one home (`_ENV_SECRET_NAMES`). The literal pass runs *before* the shape patterns, not after, which is not cosmetic: a pattern can chew a fragment out of a secret without removing the whole thing, and the surviving remainder no longer matches the literal. Demonstrated with a webhook-shaped value — literal-first scrubs it entirely, literal-last leaves the `wh_` prefix behind.
- **Text attachments are fenced as data instead of pasted into the prompt** (`bot/discord/bot.py`). An uploaded `.txt`/`.md`/`.json` had its contents glued onto the user's message with a blank line between them, which made a `[BOT_CMD: …]` line inside the file indistinguishable from one the user typed. Being precise about the mechanism, because it bounds how strong this can be: directives are parsed out of the model's *own output*, never out of the prompt, so file content cannot execute anything by itself. The route is model reads file → model treats it as an instruction → model reproduces the directive in its reply → the post-turn parser acts on it and really does spawn sessions and register repos. The fence breaks the second link only, which makes it a mitigation the model has to cooperate with rather than a hard boundary — worth saying plainly rather than filing under "injection fixed". Attachment content now gets the same treatment as quoted prior messages: a per-attachment nonce in the fence, with the nonce and the fence marker scrubbed from the body so nothing inside can close its own fence, plus a preamble naming it untrusted file data.
- **A build worktree now finds its own `.env` instead of relying on inherited secrets** (`bot/config.py`). `.env` is gitignored, so a worktree never has one, and `import bot.config` there only ever worked because the spawned session inherited the bot's entire environment — the exact coupling the change above removes. Stripping the Discord token without this would have broken every verification harness a build session runs on this repo, `scripts/smoke_test.py` included. When the project root has no `.env`, config asks `bot/procutil.py:install_root()` where the main checkout is and loads the `.env` there. That helper already existed for precisely this question — its docstring names the gitignored `.env` — and it asks git (`--git-common-dir`) rather than parsing the `.git` pointer file by hand, so it survives relative gitdir paths, submodules and nested worktrees. A normal checkout has its own `.env` and takes a fast path that never shells out; anything the helper can't resolve falls back to the unchanged project root, so a plain clone behaves exactly as before. `DATA_DIR` is deliberately left worktree-local — only `.env` and its per-machine overlay follow the main repo.
- **The owner check fails closed** (`bot/discord/bot.py`). `_is_owner` returned `True` for everyone when `DISCORD_USER_ID` was unset, on the theory that an unconfigured bot has nobody to lock out. The realistic failure is a `.env` that loses the line, at which point every user in the guild is silently the owner. It now returns `False` and logs an error once. Nobody being owner is a visible, recoverable outage; everybody being owner is not.

## v0.101.7 — The bot measures the machine, not just itself (2026-08-24)

### Added
- **The bot now reads how much memory the machine has, not just how much each session is using** (`bot/claude/memory.py`, `bot/claude/runner.py`, `bot/config.py`, `scripts/claude-bot.service`, `scripts/test_memory_guard.py`). On 2026-08-21 the user's desktop froze for a minute and the kernel ran a global out-of-memory kill that shot their browser — while every guard in this bot reported everything normal, because every guard in this bot was asking the wrong question. No session was over its ceiling and the bot's own cgroup recorded `oom_kill 0`. The kernel's dump at 16:55:50 shows what nothing was reading: 57249 free pages — under 230 MB free on a 31 GB machine — `Free swap = 68kB` with the whole 8 GB of zram gone, page cache squeezed to about 180 MB, and roughly 25 GB of anonymous memory nothing could reclaim. **A real pressure reading** (`read_pressure`) combines free memory machine-wide, swap fill, `/proc/pressure/memory` stall percentage and the cgroup's own anon-vs-`MemoryHigh` into one `ok`/`tight`/`critical` verdict with the evidence attached in the words a message can use. The free-memory rule is the one that is provably early here: 230 MB is under any sane floor, and it was true before the kill. Full swap alone is deliberately only `tight` — on zram a 100% figure is the normal state of a busy day and treating it as a crisis would hold every session forever. **Admission control** now sits inside the concurrency slot, before the spawn: `MAX_CONCURRENT` bounds how many sessions run at once and says precisely nothing about whether the machine can afford the next one, so until now the bot's entire response to a starving machine was to add another process to it. A critical machine reclaims first, then holds the start and tells the thread why in facts rather than in "waiting for memory" — with a bounded wait (5 min by default) after which it proceeds anyway, because pressure the bot did not create must not block work forever. It only ever holds on `critical`; holding on `tight` would mean holding most of the time on this machine. **Cross-session arbitration** answers the question the per-session ceiling structurally cannot — five sessions at 2.5 GB never individually breach an 8 GB limit and together they finish a 32 GB box that is also running a browser. That is *not* what happened on 2026-08-21 (each session held about 0.3 GB), but the only thing that could ever act on such a sum was the kernel's OOM killer, which is safe — the unit survives, by design — and silent: the session that died was never told why. Now the largest live session is chosen deliberately and told what happened, gated in cost order — too small to be worth taking, machine not actually in trouble, the memory is not ours (machine-wide pressure caused by a browser is not solved by killing our own work; the real offender would re-take it immediately), reclaim first and re-check, not the largest of the live samples, and a cooldown so a crunch that one kill already fixed does not unwind the whole fleet. Every session's tree measurement is now shared for exactly this, and dropped the moment the session ends so a dead session's stale number can never win the contest and spare the live one that is genuinely the problem.
- **Memory in the bot's cgroup that no session owns is now found and reclaimed** (`bot/claude/memory.py`, `bot/claude/runner.py`). In the kernel's task table from that OOM, the second-largest process on the entire machine was a detached `dotnet` at 4.10 GB, while the three `claude` sessions running at that moment held 0.30, 0.31 and 0.34 GB — reaping every session would have freed under a gigabyte. `dotnet build` starts a Roslyn compiler server deliberately detached and deliberately leaves it running so the next build is faster. Every memory guard here walks *downward* from a session's CLI process, so a reparented process leaves the tree entirely and becomes unattributable and unreapable. The cgroup is what makes it findable at all, because a process that leaves every process tree never leaves the cgroup it is charged to — so everything in our cgroup, minus everything reachable from the bot's own tree, is memory nobody owns. That definition is dangerous by itself, and several gates make it safe: only a short allowlist of known cache daemons (Roslyn, MSBuild nodes, `tsserver`) is ever killed, and only when it is old enough not to be mid-startup and idle enough not to be mid-build, because a compiler server *compiling* looks identical to a leaked one by parentage alone. Pids that an armed `/watch` is waiting on are excluded outright — a session that launched a long job with `setsid nohup ... &`, which is exactly what the watch feature tells it to do, is reparented and unowned by precisely the same test, and reaping it would silently destroy the job the thread is visibly sitting and waiting for. The reap also re-reads each candidate's start time before it signals, so a pid recycled between the scan and the kill is skipped rather than shot. Anything else unowned and large is logged every time and touched never; a multi-gigabyte process nobody can account for is worth seeing in `bot.log` whether or not anything is going to be done about it.
- **Every session is told its memory budget before it is enforced, instead of only after a kill** (`bot/config.py`, `bot/claude/runner.py`). Until now the first thing an agent ever heard about memory was that its run had just been destroyed for exceeding a limit nobody had mentioned — advice arriving strictly after the only moment it could have been acted on. The system prompt now states the ceiling up front along with what actually causes it to be hit here: prefer streaming over loading a dataset whole, bound the parallelism of anything launched (`-j$(nproc)` multiplies peak memory by the worker count), a job started with `&` or `nohup` outlives the command that started it and still counts, build servers outlive the build that started them, and `free -m` costs nothing before committing to a big job. The numbers are formatted from live config rather than written into the prose, so a retuned ceiling cannot leave the prompt lying about it. A session reaped for the *machine* rather than for its own size gets a separate note that leads with the machine running out and says explicitly that it was not over its own ceiling — the two situations call for different next steps, and a template hedging between them would give useful guidance for neither.

### Changed
- **The per-session ceiling and the unit's own limits were retuned downward, because staying under them was never evidence of behaving well** (`bot/config.py`, `scripts/claude-bot.service`). The per-session kill threshold was 12 GB on a 32 GB machine — more than the entire spare capacity once the desktop is running, so it could only ever fire after the machine was already lost. It is now 8 GB, warning at 4 GB. The unit's `MemoryHigh` drops from 12G to 10G and `MemoryMax` from 16G to 13G: a 16 GB allowance for the bot alone is half of a machine that is also running a browser, a chat client and a charting app, and the old numbers were not breached during the incident that froze it. `MemoryHigh` now has a second job as well — the bot reads it as the "our own sessions are the ones filling this" signal that gates cross-session arbitration, so it is a threshold and not only a throttle.

### Fixed
- **A leaked build daemon is re-checked for idleness at the moment it is killed, not just when it was scanned** (`bot/claude/memory.py`, `bot/claude/runner.py`). The scan samples CPU for a third of a second per candidate, serially, and the caller then logs and hops threads before reaping — so the first candidate's "idle" verdict is already seconds old when it is acted on. That is the same order as a compile burst: while verifying this feature, a real Roslyn compiler server on this machine was observed going from 0% to 840% CPU and back inside two minutes, which is exactly long enough to be judged idle and then killed mid-build. The reap now re-samples immediately before signalling and skips anything that has gone busy. The "reclaimed N GB" log line is also summed over what actually went rather than what was offered, since candidates can now be skipped for either reuse or busyness.
- **A stop request during the new memory hold is now honoured instead of swallowed** (`bot/claude/runner.py`). The hold is one more window where a run is registered as active but holds no process, which is exactly the failure this file already carries scar tissue for: a Kill landing there found nothing to terminate and evaporated, and the session would have spawned minutes later into a thread that had already reported it stopped. The hold now checks the intent before it waits and again after every poll, and returns — the existing pre-spawn check does the rest.
- **Cross-session arbitration no longer kills anything when killing is switched off** (`bot/claude/runner.py`). `SESSION_MEM_KILL_MB=0` is documented as "warnings only", but the fleet path did not consult it, so the off switch only turned off half the guard — and every message on that path quotes the ceiling the session was under, which read as "not over its own 0.0 GB ceiling" when there was no ceiling at all.
- **The budget block in the system prompt no longer promises a warning at 0 GB** (`bot/config.py`, `bot/claude/runner.py`). `SESSION_MEM_WARN_MB=0` is a legal setting; the warning sentence is now passed to the template already rendered, so switching warnings off simply omits it rather than announcing one at zero. A sub-minute `MEM_ADMISSION_MAX_WAIT_SECS` also no longer renders as "up to 0 min" — the hold reuses the same duration formatter as the `/wake` confirmation.
- **The fleet-kill log line counts live sessions, not remembered ones** (`bot/claude/runner.py`). It was reporting the size of the sample cache, which still holds entries for sessions that have already finished.
- **The .NET sensor step now hands back the build server it starts** (`bot/engine/sensors.py`). A chain's sensor step is a guaranteed spawner of the exact detached compiler server described above, and it also knows precisely when it is done with it, which makes it the cheapest possible place to clean up — `dotnet build-server shutdown` runs after the sensor, pass or fail, with its output and exit code discarded because a cleanup that fails leaves things exactly as they were rather than producing a regression worth reporting into a build's feedback. A skipped sensor never invoked the tool and so has nothing to release. The general reaper still catches these when a *session* rather than a sensor starts them; this just means the common case never gets that far.

## v0.101.6 — A thread picks its own model, and no list of model names decides which (2026-08-22)

### Added
- **A thread can now pick the model it runs on, and switching mid-conversation costs nothing** (`bot/engine/commands.py`, `bot/platform/formatting.py`, `bot/platform/base.py`, `bot/discord/forums.py`, `bot/discord/bot.py`, `bot/discord/slash_commands.py`, `bot/engine/workflows.py`, `bot/config.py`). Until now the model was decided entirely above the user's head: build-family steps got `BUILD_MODEL`, plan steps stayed unrouted, and an ordinary chat turn set no model at all, so it always landed on `DEFAULT_SESSION_MODEL`. There was no way to say "this conversation runs on that model" short of editing `.env` and rebooting. `/model <name>` pins one to the thread the way `/mode`, `/verbose` and `/effort` already pin theirs — stored on the thread record, reloaded into every request through the single context helper every entry path funnels through, so it survives a restart and applies to typed messages, self-wakes, replays and button-driven workflow steps alike. Nothing about the conversation is disturbed: the session id is untouched and `--resume` still carries the whole transcript, only the `--model` flag differs, so the switch takes effect on the very next message with no re-priming. `/model default` hands the thread back to normal routing. Precedence is deliberate and documented at the choke point: an automatic model-limit downgrade still outranks the pin (a thread pinned to a model whose own quota is spent keeps working on the fallback rather than failing), the pin outranks configured routing (if you pinned a model and then tapped Build, silently running something else would be the surprising outcome), and everything below that is unchanged. The part written most carefully is what is *not* here: there is no table of model names anywhere in it. Model names version-bump and get renamed, and a hardcoded whitelist would quietly start rejecting the model you actually want the week after it ships — so validation is a *shape* check rather than a membership check (letters, digits, dot, dash, underscore, colon, bounded length), which refuses a fat-fingered `fable x` before it can reach a command line while accepting names that do not exist yet, including a future model called simply "X". Display goes through the existing label parser, which already renders `claude-opus-9` as "Opus 9", `claude-fable-5-5` as "Fable 5.5" and a Bedrock-style id as its bare name. The autocomplete list is derived at runtime from the models this deployment has actually run plus the ones its own settings reference, so it maintains itself; `MODEL_CHOICES` in `.env` overrides it for deployments that want a fixed list, and an empty list is fine because free text always works. A name the bot has never seen is accepted with a soft warning naming the one-word way back, never blocked, because a model released yesterday is indistinguishable from a typo by any test this code can perform — the CLI stays the authority, and the model label the bot already prints in the live header and on the finished result is the honest confirmation of what actually ran. Autocomplete falls back to offering whatever is typed, so a model this deployment has never run is one tap rather than a fight with an empty dropdown on a phone -- only when nothing else matched, because a half-finished name sitting next to the real name it prefixes is a mis-tap waiting to happen. Harness: `python scripts/test_model_command.py` (14 checks, including one that fails if anyone reintroduces a hardcoded model-name table and one that drives the real autocomplete callback off a real command tree).

### Fixed
- **The aiagent monitor thread had been dead for seventy-one days, and every stale dashboard it left behind was still pinned as if it were current** (`bot/monitor/service.py`, `bot/monitor/formatter.py`, `.env`). The monitor was polling degenai.dev every four hours and being rejected every time -- 395 consecutive `401`s, last successful fetch 2026-06-10T12:58 -- because the stored admin credential had a four-character password where the real one is twenty-three. Nobody noticed for two and a half months, and that is the more interesting half of the bug: the auth-failure path alerted only when `consecutive_failures == 1`, so a single notice went out on June 10 and the monitor then failed silently 394 more times. Failure alerts are now time-based rather than one-shot -- first failure, then at most once a day for as long as it stays broken -- and the same rule replaces the `failures == 10` one-shot on the all-endpoints-failed path, which had exactly the same hole. The banner itself was equally uninformative: "Authentication failed. Check `MONITOR_AIAGENT_AUTH` in .env" reads identically on minute one and on day seventy-one, so it now carries the consecutive-failure count and when data was last good, rendered as an absolute stamp plus an age ("Jun 10, 12:58 UTC (71d ago)"); the stale-data banner gets the same rendering instead of a raw ISO string. Separately, and visible in the thread as a pile of contradictory dashboards, the refresh recreated its message roughly every thirty days: `_edit_or_recreate` catches any non-`NotFound` exception, falls through to posting and pinning a fresh message, and never touches the old one -- which stays pinned, frozen on whatever data it last held, so the thread accumulated five "current" dashboards spanning March to August plus a Discord pin-notice for each. The message it could no longer edit is now unpinned when it is replaced, and -- the reason the thirty-day failure was never diagnosed in the first place -- that exception was being logged at `debug` while the bot runs at `INFO`, so it vanished without a trace. It logs at `warning` with the exception type and message, so the next occurrence names itself instead of being inferred from message timestamps.

## v0.101.5 — A session waits for the job, not a guessed timer (2026-08-18)

### Added
- **A session that starts a long job can now wait for the job itself, instead of guessing a delay — and the thread shows it is still busy the whole time** (`bot/engine/watches.py`, `bot/claude/types.py`, `bot/store/state.py`, `bot/scheduler.py`, `bot/engine/lifecycle.py`, `bot/app.py`, `bot/config.py`, `bot/platform/formatting.py`, `bot/discord/tags.py`, `bot/discord/idle.py`, `bot/discord/interactions.py`). Until now a session that kicked off a two-hour render or fit had exactly two options, and both were bad: promise to "pick this up when it finishes" — which nothing in the system could ever honour, because a finished turn is a dead process that nobody re-invokes — or arm a timer and pick a number, waking early to find the job still running or late after it had been done for an hour. Meanwhile the thread went quiet and lost its `active` tag, so the one question the user actually had ("is this still going?") had no answer anywhere on screen. A **watch** closes both gaps by treating a wake-up as something an *event* triggers rather than a clock: `[BOT_CMD: /watch pid=... log=... done=...]` with a `~~~watch` body names the job, and the bot polls it every 30 seconds until the process is gone or a completion marker appears in the log, then resumes the thread with that body as the prompt. The mechanism is deliberately small — when a watch trips it does not open a second way back into a session, it manufactures an ordinary due self-wake, so everything already built around wakes (the runaway-loop cap, the re-arm when the thread is mid-run, the unattended-turn nudge protocol) applies to watches for free and cannot drift apart from them. Only an explicitly parsed directive arms anything; the heuristic that used to infer a wake from prose was removed twice for firing on turns that merely *discussed* a job, and nothing here brings it back. While the watch is armed the thread stays honest about being busy: it keeps its `active` tag, it never picks up the 💤 idle marker, and a single heartbeat message **edits itself in place** — a progress bar, elapsed time, the log path and the last line of that log, plus a "Stop watching" button — rather than re-posting, because thread renames are rate-limited but message edits are not. PID reuse is the failure mode that would hurt most (a recycled number reads as "still running" and the thread hangs until the safety timeout), so the process start-time token from `/proc/<pid>/stat` is captured at arm time and a mismatch counts as finished, as does a zombie that has exited but not been reaped. A thread waits on exactly one thing: arming a watch retires any leftover timer, arming a timer retires the watch, and a new watch supersedes the old one. `timeout=` (default 6h, hard cap 24h) exists as a backstop, not a plan. A `/watch` the bot cannot arm — no `pid=`, or a `done=` marker with no `log=` to find it in, or a missing `~~~watch` body — is refused out loud rather than quietly, because a turn that believes it is being watched while nothing is watching is the same silent dead-end in a new costume. The other way a pid lies is at the very start: `setsid` and `nohup` fork when the caller is already a process-group leader, so the `$!` a session captures can be a launcher that exits the instant the real job starts. Arming on that pid used to mean waking a minute later and reporting "finished after 0s" about a job still running — a confident description of output that never existed. A pid that is already gone at arm time is now recorded as such, the arming notice says so, and the resume prompt tells the session outright not to assume success: check the log and whether the real process is still running before reporting anything. The guidance the model reads was updated to match — verify the pid is alive before arming, and prefer `done=` + `log=` when the job runs behind a wrapper script. Knobs are the `WATCH_*` entries in `bot/config.py`; harness is `python scripts/test_watch.py` (104 assertions, driving real subprocesses and the real `check_wake_request`, no liveness mocking).

## v0.101.4 — A parent waits for its whole spawn wave (2026-08-18)

### Added
- **A parent session now waits for its whole spawned wave, reads every child's full report, and picks the work back up on its own** (`bot/discord/orchestrator.py`, `bot/engine/commands.py`, `bot/engine/lifecycle.py`, `bot/discord/bot.py`, `bot/platform/base.py`, `bot/claude/types.py`, `bot/config.py`, `bot/claude/runner.py`, `bot/app.py`, `bot/platform/formatting.py`). Before this, a parent that fanned three research children out got three separate progress lines as each one landed, each carrying a 400-character excerpt of that child's answer, and then nothing — the user was the join: they read the three posts, decided the wave was done, and typed something into the parent thread to restart it. Every one of those steps is now the bot's. The excerpt is the part that mattered least and cost most: full reports have always been written to disk (`data/results/<id>.md`), so the parent is handed **absolute file paths** and reads whatever depth it needs, instead of being handed a truncation and having to reason about a summary of a summary. The join itself keeps no roster of its own — a child's state is recomputed from records that already exist (its thread's session id, then the newest instance for that session), which is the same derivation tags, forums and eval already use, so a wave cannot go stale against a state file. What a child is doing is read from *both* its status and its `needs_input` flag, because finalize flips a session to COMPLETED even when the model parked on a question: without that second signal a child that stopped to ask something would be counted as a finished contributor and its question would be dropped on the floor. It is instead pulled out as a blocked child and the parent is told it can answer directly, with `[BOT_CMD: /reply thread=<id>]` — a new directive with the same tilde-fenced body shape as `/spawn`, capped at five per response, whose target must be a thread this session actually spawned (checked against its own dispatched ids, not just "is a number"), so a session cannot reach into a stranger's thread. When the wave closes, the parent resumes itself (`ORCH_AUTO_RESUME`, on by default); this is bounded by the same 12-wave cap that already governs spawning, because a callback resume deliberately does not reset that counter — only a real user message does. A wave released *partially*, because a child was killed or the wave timed out, keeps a manual Resume button instead: proceeding on incomplete evidence is a human call, and the parent is told by name which children contributed nothing. Two failure modes are handled rather than hoped about. A wave whose children never reach a terminal state would otherwise hold the parent forever, so a sweep in the autonomy loop releases anything past `ORCH_WAVE_TIMEOUT_MIN` (default 45) every five ticks — placed outside the ship-sweep overlap guard so a long merge cannot starve it. And the real race, found by writing the test rather than by reading the code: children are created in a loop, so a fast-failing first child can finalize *while later siblings are still being created*, at which point the join looks at a one-child roster, agrees the wave is complete, and silently discards the rest of the fan-out. The roster is now explicitly sealed when the dispatch loop ends (`spawn_wave_sealed`), no wave is joinable before that, and sealing re-checks — which also covers the opposite ordering, where every child finished before the loop did and nothing else would ever have triggered the join. Release is idempotent by writing the released flag before posting, so a double-fire cannot double-resume. Four more edges, each found by re-reading the finished code rather than by running it. A parent woken by one wave can dispatch a second while the first is still filling up, so a child's callback resolves the wave that actually **lists that child** rather than "the newest wave" — otherwise wave B hijacks wave A's callbacks and neither ever closes. A child the user kills takes an early return in finalize and never calls its parent back at all, so a wave whose last child was killed had every child settled and nothing left to notice; the sweep now closes any fully-settled wave on its ordinary tick instead of making the parent sit out the full timeout for a wave that is demonstrably complete. A wave older than twelve hours is *retired* rather than released — no post, no resume — which matters most on the very first tick after deploy: waves recorded before this feature existed have the released flag defaulting to false, so without it the sweep would have "released" every historical wave at once, waking days-old sessions and spraying The Ark with undeliverable notices. And the blocked-child wake-up is a loop, so it is bounded like every other self-driving loop here: the parent answers its child, the child can ask again, and each round is two real CLI runs — the wave carries a budget of four wake-ups, after which the notice still arrives but needs a tap. A blocked child whose wave is already gone has nothing to charge, so it gets the button too. A timeout of `0` now means what `.env.example` says it means — wait for the children however long they take — where before it would have swept every wave on the next tick, since a zero-minute deadline is always already past. If the parent thread is archived or gone, the roundup surfaces in The Ark rather than only in the log, and the wave is still marked released so it cannot retry forever. One last fix came from rendering the actual wake-up prompt during verification rather than reading the code that builds it: a report excerpt's second and later lines were emitted unindented, so a two-child wave read as a six-item list with the report opening running flush against the roster it was nested under. Covered by `scripts/test_orchestrator_join.py` (registered in `.claude/test.json`), which drives the real join against a fake Discord surface: state derivation for each of the seven child states, a full wave closing exactly once with file paths in the parent's prompt and excerpts in the human's post, a blocked child producing a `/reply` prompt instead of a completion, the timeout sweep releasing stale waves and leaving fresh ones alone, an archived parent falling back to The Ark, `/reply` pairing and its ownership and cap guards, the seal race in both directions — an unsealed wave refusing to close on its first child, and a sealed one reporting both children rather than the one that raced — two waves on one parent each closing on their own children, an ancient wave retired without a post or a resume, a fully-settled wave closed by the sweep inside its timeout window, a zero timeout leaving a three-hour-old wave alone, and the blocked-child budget granting the first wake-up, charging for it, and refusing the fifth without charging for it.

## v0.101.3 — A runaway session is reaped before it takes the machine (2026-08-17)

### Fixed
- **One session's runaway memory took the bot and every other session down with it, twice in one night** (`scripts/claude-bot.service`, `bot/claude/memory.py` (new), `bot/claude/runner.py`, `bot/config.py`, `bot/claude/types.py`). Traced from the journal rather than guessed at, and the kernel was not the one that got it wrong. A build session in the `Ev-nova-remake` repo (thread `1538278656936247328`, instance `t-7376`, "Plan resolution upscale strategy across UI and assets") was running diffusion upscale experiments; one of its Python subprocesses reached **13.7 GB** resident on a 31 GB machine whose 8 GB of zram swap was already full, so the global OOM killer fired at 01:09:56 and picked exactly the right victim — the experiment, not the ~220 MB bot. What turned a correctly-handled runaway into an outage is that every process a session spawns lives inside the bot's own systemd cgroup, and the unit never set `OOMPolicy`, so systemd's default of `stop` tore the whole unit down over a kill it was told about. The bot and ten unrelated sessions died for one job's allocation. It then happened *again* at 02:34, because the restart resumed the same session and it re-ran the same command: `Consumed … 13.8G memory peak` in the second record, ninety minutes after the first. The desktop notification the user woke up to ("Memory Shortage Avoided — the background service claude-bot.service has been terminated by the Linux kernel") comes from Plasma's `oom_notifier` KDED plugin, which keys on the *unit's* own `Result=oom-kill`; a descendant being killed under `continue` is not that, so the popup stops too — not by suppressing it, but because the thing it was reporting no longer happens. Both halves of the fix are needed and neither is sufficient. Structurally, the unit gets `OOMPolicy=continue` so a killed descendant is a killed descendant, plus `MemoryHigh=12G`/`MemoryMax=16G` so one job cannot push the machine to where the *global* killer has to choose a victim at all; the honest cost of that cap is that the bot's total footprint is now bounded, so ten heavy sessions at once will throttle where they previously competed with the desktop, which is the tradeoff worth having. But a cgroup cap alone would only move the failure — it would kill *something* in the bot's cgroup, and the something is chosen by the kernel, not by us. So the other half is that the bot reaps its own runaways first: the runner samples each session's whole process tree every `SESSION_MEM_CHECK_SECS` (default 30s), warns the session at `SESSION_MEM_WARN_MB` (6 GB) while it can still choose a smaller batch, and reaps it at `SESSION_MEM_KILL_MB` (12 GB) — deliberately below the unit's `MemoryMax`, so the blast radius is the one job at fault rather than whatever systemd or the kernel would have picked. Setting `SESSION_MEM_KILL_MB=0` keeps the warning and drops the kill, and the warning still reaches the session in that mode rather than only the log. A reap that lands in the same instant as a finishing turn reports the finished turn: the guard samples on a cadence, so it can decide to kill while the CLI is already flushing the result that says the work is done, and failing that run would auto-resume a build whose edits are already on disk. It stands down for a requested stop for the same reason — a Steer or a `/kill` inside the same few seconds otherwise loses to the reap, and the user who stopped a session would watch it restart itself. When the reap does stand, the failure it reports is built from the events the killed attempt actually produced rather than from scratch, because the record of work is folded into the attempt that replaces it and the chain reads that record to decide whether a build changed code at all: a bare failure there meant a build that made every one of its edits before being reaped came back as "no changes made", with its cost and token counts lost too. And the housekeeping that runs once the process is gone — mirroring the session file out of the worktree, releasing the per-repo test-suite lock — now happens before the watchdog exits rather than only on the ordinary one; freeing that lock is specifically the backstop for a session killed mid-test-run, which is the exact shape of a reap, so the case it existed for was the case it never covered, and on the memory path the attempt that auto-resumes would have queued behind a lock its own predecessor still held. The notice explaining a kill is posted *before* the reap rather than after it, because reaping closes the CLI's stdout, which ends the reader loop, whose cleanup cancels the watchdog — usually while it is still parked in the grace wait for a multi-gigabyte process to die, so anything sequenced after that call was on a coin flip. The load-bearing detail is *whole tree*: the CLI is a thin supervisor and the memory always lives in what it spawned, which is why the stall diagnostics that ran during the incident reported `336MB` for this session and were not wrong about the process they were looking at — they had never looked at its children. The stall snapshot now carries the tree figure alongside, measured before the existing 2-second CPU-and-connections probe rather than after it: that probe shares one `try` and one early return, so a single unexpected `psutil` error in it would otherwise have taken the one number that mattered down with it. The reap signals descendants *before* the root for a reason that is easy to get backwards: killing the CLI first orphans the runaway, which keeps every byte, gets reparented to init, and drops out of every process tree we know how to walk — so the guard would report a kill while the actual problem carried on. SIGTERM then SIGKILL after a grace period, because a process that allocates 13 GB in two minutes is exactly the kind that is inside a native allocation and ignoring signals. It also never *waits* on the root, only on the descendants, and that restriction is load-bearing rather than fastidious: the root is the CLI process, which asyncio spawned and therefore owns, and the obvious way to wait for a group of processes to die calls `waitpid` on any of them that is a child of ours — collecting the exit status the runner is still expecting, so asyncio falls back to reporting 255 instead of the signal actually sent. That is not cosmetic, because the test for "did the user ask for this stop?" is a negative-returncode test: a Kill or Steer landing in the same second as a reap would have rendered as a red failure instead of the quiet stop it asked for, undoing the stand-down above. The root gets its grace by polling `/proc` for a zombie instead, which answers the same question without collecting anything. Headroom against the cgroup limit is measured on *anon* pages only, never file cache, since cache crossing a limit triggers reclaim rather than a kill and counting it would cry wolf continuously. And because a silent re-run is precisely what turned one OOM into two, a reaped run resumes itself once (`MEMORY_KILL_MAX_RETRIES`, default 1 — lower than the thrash path's, because a memory ceiling is physical rather than bookkeeping and a second identical attempt has no reason to fit) with the numbers written into its prompt: what it peaked at, what the ceiling is, which process was the offender, that re-running unchanged **will** be killed again, concrete ways to make the job fit, permission to say plainly that it cannot and stop, that its edits are still on disk so it should check `git status` before redoing anything, and — the failure mode that started this — that a job launched with `&` or `nohup` counts against the same ceiling and was killed too. That last point is also the module's one deliberate known gap: a background job whose launching shell has already exited is no longer a descendant of anything we hold, so the tree walk structurally cannot find it, which is why the cgroup figure is reported alongside the tree rather than instead of it. Covered by `scripts/test_memory_guard.py` (registered in `.claude/test.json`), which plants a real three-level process tree holding a real 220 MB and asserts the grandchild is both found and named; shows the control case where signalling only the root leaves that grandchild alive; pins the deepest-first signal order, and — driving real subprocesses, since no stub of the wait can reproduce it — that a reaped run comes back as the signal it was sent rather than as 255, including that a root ignoring SIGTERM is still escalated within its grace; checks the thresholds are ordered warn < kill < the unit's own `MemoryMax`, so the file and the code cannot drift apart into a cap that fires before the bot does; compares the *installed* unit against the one in git whenever it is loaded on the machine running the check, since a template edit that was never copied to `~/.config/systemd/user` reads exactly like a fix and protects nothing; pins both messages the user reads, including that warnings-only mode still reaches the session and does not threaten to stop it at 0.0 GB; pins that a reap stands down for a finished turn and does *not* stand down for a failed one, for the no-result-event stream a killed run actually produces, or for a self-contradicting stream whose tie has to break the way `extract_result` breaks it; pins that the failure a reap reports still carries the killed attempt's tool calls and command log, and that they survive into the run that replaces it; and drives the real recovery cascade to confirm a reaped run resumes once carrying `--resume` and the note, gives up at the cap with its real error intact, and that an ordinary build failure never enters the path.
- **"Interrupted by bot restart" now says when the restart was the machine running out of memory** (`bot/app.py`, `bot/claude/memory.py`). The sessions killed at 01:09 and 02:34 were marked orphaned on the next boot and told exactly that, which is true and useless: it reads as a bot bug, and identifying what actually happened cost a full forensic session the following day. On an unclean start only, the bot now asks the journal whether its *previous* run ended in `Result=oom-kill` and, if so, appends the reason to the notice in each interrupted thread and posts one summary to The Ark — how many sessions went down, that branches and worktrees are untouched, and where to look if it repeats. The cgroup's own `oom_kill` counter cannot answer this, because the cgroup is recreated on restart and always reads zero in the run that wants to know. Reading the journal correctly is the whole trick and the first attempt got it wrong: systemd logs its `Consumed … memory peak` summary *after* the `Failed with result 'oom-kill'` verdict, so a reverse scan that stops at the summary steps over the very line it is looking for. What it does instead is walk back from the unit's own start line to the first line where systemd said how a *run* ended, and report only what that line says — which is also what stops a clean restart from being blamed on an OOM kill hours earlier in the same window, and what correctly stays quiet when `OOMPolicy=continue` let the unit survive a killed descendant. It assumes nothing about the shape of the bot's own startup (an earlier attempt required the unit's `Starting` and `Started` lines to be adjacent, which the two-minute volume-mount wait in `ExecStartPre` is enough to break) and it only trusts lines systemd prefixed with the unit name, since the bot's own stdout shares this journal and a log line reading "Succeeded" would otherwise mask a real kill. All of those shapes are pinned in `scripts/test_memory_guard.py` against the verbatim recorded journal of the 01:09 kill, and the broken reverse scan is kept in the harness alongside them and asserted to still get that fixture wrong — so if the fixture is ever edited into a journal shape systemd doesn't actually produce, the test says so instead of passing on a case that no longer covers anything.

## v0.101.2 — A killed build stays killed, a thrashed one resumes itself (2026-08-16)

### Fixed
- **The meeting-watch harness was registered as a check but couldn't run as one** (`scripts/test_meeting_watch.py`). Found by the project's own portability gate, which has been failing on it since v0.100.0 added it. It's the exact gap that gate was written for in v0.99.17: the bot launches its checks with a bare `python`, which resolves to the *system* interpreter with none of the bot's dependencies on it, so this one would have died at import during a verify run — a failure shaped like a real regression, arriving before any test logic ran. Every other harness imports the shim that relaunches under the project's virtualenv; this one was added without it. One line, and confirmed the way the gate intends: it now passes under `/usr/bin/python` as well as under the virtualenv, and the portability check goes green across all 162 tracked files.
- **A build killed for "autocompact thrashing" now resumes itself instead of blocking a chain on a button whose only job is "run it again"** (`bot/claude/runner.py`, `bot/claude/parser.py`, `bot/config.py`, `bot/claude/types.py`). The message was never ours: the Claude Code CLI kills its own process, exit 1, when the context refills to the limit within 3 turns of a compact three times running — a self-protection guard against burning tokens in a compaction loop. We relayed it verbatim as a build failure, and 14 of them have landed since Aug 11, every one in the same large C# repo. Two came an hour apart in the same thread (`t-7357` at 3:05 PM, `t-7358` at 4:13 PM, 19 and 23 minutes of work each), both blocking a verify chain and paging the user, who clicked Retry once and had it work both times. It works because that counter lives in the CLI *process*, not the conversation: a resumed session starts from the compact summary with the counter back at zero. The click therefore carried no information the bot didn't already have, which is the definition of a click the bot should be making — it now resumes the session itself, bounded at `CONTEXT_THRASH_MAX_RETRIES` (default 2, so 3 attempts), then surfaces the real error with the Retry button exactly as before rather than looping. The cap is the honest part of this: a session whose context is structurally too heavy will keep tripping the guard — one conversation in the logs tripped it six times across four days (twice on Aug 11, three times on Aug 13, once on Aug 14) — and each attempt is a real 20-minute run, so "retry forever" would just spend the quota slower. Three details make the resume worth anything. The session id has to come from the stream's init event, not the `result` event: a killed process never emits the latter, so a *fresh* spawn that died after 50 turns previously reported no conversation at all, and neither the auto-resume nor the user's Retry had anything to resume — it started the work over, which is its own quiet version of this bug and is fixed here for every failure path, not just this one. And the resumed agent is told why its predecessor was killed, since the CLI's own "read in smaller chunks" advice is addressed to the operator and the agent just sees its process vanish; the note leads with "take stock of what is already done before you do anything else" (`git status` and `git diff` when there's a repo), because every edit the aborted attempt made is still sitting in the worktree and an agent that assumes otherwise redoes it. And the aborted attempt's record of work is folded into the result that replaces it, because the lifecycle layer *assigns* that record onto the instance: an agent that made every edit before being killed and then resumed only to confirm the work was already on disk would report an empty tool list, and the chain reads exactly that list to decide whether a build changed code — a real build would have rendered as "no changes made" and skipped its own review. Tools, bash log and main-repo path-poisoning hits are merged, cached-token counts summed; turn count deliberately is not, because the account-failover heuristic treats more than one turn as proof an account did real work and inflating it could hide a dead backup. The recovery note is one-shot and normally spent by the attempt that reads it, but a resume can end before it ever gets that far — if every account is on cooldown the runner refuses to spawn and re-queues this same instance for later, so the note is now cleared when the resume returns rather than only when it's read; otherwise a retry hours later would open with "your previous attempt was aborted". Placed after the dead-session cascade and before the model-limit and account-failure branches, neither of which can match this text anyway. Covered by `scripts/test_context_thrash.py` (registered in `.claude/test.json`), which stubs only the subprocess boundary so the real cascade runs: a thrash-then-succeed pair returns the success and resumes the id the *killed* process created, the recovery note reaches the second attempt and not the first, a session that always thrashes stops at the cap with its real error intact, a plain compile failure is not retried at all, the killed attempt's edits survive into the final tool record, a resume refused before it can spawn leaves no stale recovery note behind, and — driving the real stream loop rather than a stub — a process killed before its result event still names its session. Each half was checked to fail when disabled, so no assertion is vacuous.
- **Pressing Kill no longer loses to whatever the runner was about to retry** (`bot/claude/runner.py`, `bot/claude/parser.py`). Found reviewing the auto-resume above, and it turned out to be older and wider than that one path. Every recovery layer the runner has — the dead-session cascade, the model downgrade, the account failover, the 30-second transient retry and now the thrash resume — works by spawning another attempt, and in the moment between one attempt ending and the next starting there is no process in the table. A Kill or a Steer landing in that window found nothing to signal, reported "already stopped", and evaporated: the thread said Killed while the run quietly started over for another twenty minutes. The window is milliseconds for most layers and a full 30 seconds for the transient retry, but the thrash resume is the one that made it matter, because that is now a routine event rather than a rarity. A kill arriving while the run is still live but between subprocesses is now recorded rather than dropped, and three checkpoints share one helper that honours it, each covering a window the others can't. The first sits at the top of every attempt, which is the single point all five recovery layers re-enter through — so a sixth added later inherits the check instead of having to remember it — and it covers the wait on the concurrency semaphore too, where a queued build can sit for a long time with nothing to kill. The second is in the refuse-to-spawn branch, which never reaches the others: building a worktree takes a per-repo git lock and can queue behind a sibling build's merge, the longest of these windows, and a fleet on cooldown returns from there a usage-limit result the caller *re-queues*, so a run the user stopped would have started itself again hours later. The third sits just after the process is registered, because spawning is itself an await and a kill can land inside it and still find nothing to signal — checked there rather than before, since that's the first moment the answer can't change, and the cleanup that already SIGKILLs a leftover process handles the one we just started. The mark is deliberately left set once it fires, because several layers inspect the result they get back and recurse again, so it has to keep refusing for the rest of the run; the top-level entry point clears it instead, which is what keeps a mark stranded by an earlier run from stopping the next one before it ever spawns. The stopped run keeps its conversation id and its kill reason — so it lands as a quiet stopped tombstone rather than a red build-failed embed, and there is still a conversation to pick back up — along with whatever record of work the layer it interrupted was already carrying, which for the thrash resume means the killed attempt's edits are still reported. Two more edges of the same shape, both live before this and neither related to the thrash work: a terminated process leaves no output and no completed turns, which is *exactly* the signature the failover heuristic reads as "this account fell over instantly" — so on a two-account setup, killing a build handed it straight to the backup account to run again, and killing one that had already failed over stamped the primary's usage-limit reset onto it, which schedules an automatic retry for a run the user had just stopped. And our own four-hour process ceiling has the same empty signature, so a run that hit it was passed to the backup account to burn another four hours; it is classified as account-agnostic now, alongside the CLI-flag and model-availability errors it belongs with. Covered by four new cases in `scripts/test_context_thrash.py` — a Kill fired in the real gap between an abort and its resume, one fired while the process is being created, one fired during worktree setup with the fleet on cooldown, and a mark left behind by an earlier run of the same id — and three in `scripts/test_account_failover.py` (a killed run, a killed run on the backup after a usage limit, and the four-hour ceiling). Every guard was disabled in turn to confirm the tests fail without it, and each failed in its own words: the killed run was resumed anyway, a turn was streamed after the kill, the stopped run came back as a usage-limit wait, a fresh run refused to start on a dead predecessor's mark, the failover fired, the reset was stamped.

## v0.101.1 — Uploads survive the run that needs them (2026-08-14)

### Fixed
- **An uploaded screenshot could be deleted a second before the run that was told to look at it started** (`bot/discord/bot.py`, `bot/discord/interactions.py`, `bot/config.py`, `bot/platform/base.py`). Traced from the logs of one thread rather than guessed at: a picture arrived at 17:57:05 and a second at 17:57:27 while the first was still being worked on; the Steer that came with the second killed the running job at :30 *while it was inside the Read of the first picture*; at :31 both files were unlinked; at :32 the replacement run started holding two paths to nothing and reported, accurately, that the folder was empty. Two separate cleanups fired at that instant, and both were written on the same wrong assumption — that an upload's life ends when the Python call that dispatched the prompt returns. It does not. The Steer path dispatches its run from a different task, so the receiving message frame returns immediately and deleted the file out from under a run that had not started yet: a guaranteed loss, not a race. The Run Now and auto-replay paths deleted on `_replay_to_thread` returning, which is also what happens when the run is *killed*, so the picture went away exactly when the replacement needed it most. And the deeper problem outlasts both: the path handed to a session is written into its transcript permanently, so a retry, a resumed turn, or a plain "look at that screenshot again" can need the file hours later — turn-scoped deletion can never be correct for a resumable session, which is why this was intermittent in a way that looked like flakiness. Uploads are now reaped on a retention timer (`PENDING_IMAGES_TTL_HOURS`, default 48h) instead of by whichever frame happens to unwind first, and the once-at-boot sweep became an hourly loop, since it is the only thing deleting these files now. Retention alone would be an unbounded promise, so the sweep also evicts oldest-first back under a size cap (`PENDING_IMAGES_MAX_BYTES`, default 500 MB) — but never a file younger than 15 minutes, whatever the cap says, because that floor is what keeps a burst of uploads in one thread from yanking a picture out from under a live run in another. A file a queued prompt still references is exempt from both rules; the one remaining immediate delete is an explicitly cancelled queue entry, which never ran and whose path nothing can be holding. That exemption ending is its own hazard, and the second half of the fix: a prompt held back by a *weekly* limit can wait days, so the moment its entry leaves the queue its images are both unreferenced and already past a 48-hour window — reapable, by the next sweep, before the run they were queued for has read a byte. The clock is therefore restarted whenever a file is about to be needed rather than only when it arrives, which changes what an upload's timestamp means from "uploaded at" to "last needed by a run". It happens at the pop as well as at the handoff, because due entries replay one at a time and the last one's files would otherwise go unprotected for the length of every run ahead of it — and on the Run Now click before the Discord round-trips that follow it, for the same reason in miniature. The now-unused `images_claimed` ownership flag went with it, ownership being uniform — that flag was the previous, narrower attempt at this same bug (v0.80.0 below), which handed ownership from the receiving frame to the usage queue instead of questioning whether a frame should own it at all. Covered by `scripts/test_pending_image_lifecycle.py` (29 checks, including a replay of the 17:57 sequence, a prompt that waits out a weekly limit and still finds its picture, and the three guards worth stating outright: a file inside the young-file floor survives even `TTL=0`, a burst of uploads over the cap cannot evict one, and a queued prompt's picture is never the eviction victim even when it is both the oldest file present and over the cap by itself; the schedule is pinned too — a burst of uploads nudges the reaper once rather than once each, and the periodic tick sleeps before its first sweep so it cannot double up with the one the startup drain already ran), registered in `.claude/test.json`; `scripts/test_attachments.py` gained the survives-the-turn assertion in place of the one that asserted the opposite behavior. Both harnesses now point the upload directory at a temp dir for the duration — they write real files through the real save path, and the bot they share a machine with is usually running, so without that a check could delete somebody's actual screenshot and reproduce the bug it is testing for.
- **The attachment loop enforced two of its own size limits with numbers written out by hand** (`bot/discord/bot.py`). Found while reading the surrounding code for the fix above. v0.101.0 lifted the accepted extensions and their ceilings to module level precisely so the "I can't take that" reply quotes the same limits the loop enforces, but the image and voice-note branches were left comparing against literal byte counts, with two local aliases in between standing in for the shared sets. They agree today, so nothing is visibly broken; the point is that raising the image ceiling in the one obvious place would have left the loop rejecting at the old size while the reply promised the new one. Both branches use the named constants now and the aliases are gone. No behaviour change, but the image branch had no test at all, so `scripts/test_attachments.py` grew one that drives a real `.png` through it (57 checks): the picture reaches the session as a path inside the pending directory, one byte over the ceiling is refused with a notice quoting the same ceiling the loop enforced, and the missing-file scrub is pinned against the *actual* sentence the image branch produces — which is not the wording the document branch uses, so a drift in either would otherwise ship a dead path into a replay unnoticed.
- **A voice note silently ate every other file attached to the same message** (`bot/discord/bot.py`). Also found while reading around the fix above, and it contradicts the promise v0.101.0 made one release earlier — that an upload never disappears without a word. Transcription replaces the message text, so a second voice note would erase the first and the attachment loop stops at the first one it takes. Everything behind it was skipped without a trace: send a voice note and a screenshot together and the bot answered the audio while the picture went unopened and unmentioned, which reads as the bot ignoring it. The two failure exits were worse than skipping — a voice note that couldn't be transcribed, or that had no speech in it, returned straight past the notice-sending step at the end of the loop, discarding a rejection already written for an *earlier* attachment. So an unsupported file followed by a bad voice note produced one message about the audio and nothing at all about the file that was actually refused. All three exits now name what they left unread, and the notices go out through one send that every path shares instead of two ad-hoc sends and a step that could be jumped over. What runs is unchanged in every case: a failed voice note still dispatches nothing. The same branch was also overwriting the message text with the transcription rather than appending to it, so a question typed alongside a voice note ("what did I just say?") was thrown away before the session ever saw it — it now composes the two the way the image, document and text branches already do. Covered by `scripts/test_attachments.py` (57 checks — a voice note with a screenshot behind it, a caption typed alongside one, and an unsupported file ahead of a failing one; the harness stubs speech-to-text rather than letting it fail, since the real one posts to a paid API and would otherwise make a live call on any machine with a key in its `.env`).
## v0.101.2 — Outlook drafts reach the server (2026-08-14)

### Fixed
- **A draft the bot created never appeared in Outlook** (`bot/services/outlook.py`). Every draft was reported as created, and every one of them was — the item was genuinely written to the Drafts folder, confirmed by reading the folder back over COM. It just wasn't visible, because two different Outlooks are involved. The mail client actually being read is the *new* Outlook (`olk.exe`), which is the web client in a window and shows only what the Exchange server holds. The COM integration drives *classic* Outlook, which isn't normally running, so each `draft` call silently booted a headless instance to do the work. Classic Outlook runs in cached mode against this mailbox (`IsCachedExchange` is true, backed by a local `.ost`): a saved item lands in the local cache first and goes up to the server on the next send/receive. The headless instance is gone the moment the script releases its COM references — measured, not assumed: no `OUTLOOK.EXE` survives to the first sample after exit — long before any scheduled sync, so the draft stayed stranded in a file on disk that the client being watched has no knowledge of — a "success" that was indistinguishable from doing nothing. `create_draft` now nudges a send/receive after `Save()` and then holds the process open for 30 seconds while the upload happens — the holding open is the actual fix, and `SyncObject.Start()` is only a nudge. There is deliberately no completion check, because three candidates were tried against the real mailbox and all three are dead ends. Binding the sync group's `OnSyncEnd` event looks authoritative and is the first thing to reach for; instrumenting it shows SyncStart, Progress(0/1000) and SyncEnd all firing within 60ms, *before `Start()` even returns*, so it reports the send/receive group finishing and says nothing whatever about a cached-mode upload. Shipping that would have returned "synced" in a fifth of a second, exited immediately, and reproduced the original bug behind a success message — strictly worse than the bug, which at least did not claim to have worked. The second candidate, watching for the item to be re-keyed to a server `EntryID`, is likewise dead: the EntryID is byte-identical 45 seconds after creation. The third, reading the server-assigned `PR_CHANGE_KEY` through `PropertyAccessor`, is worse than dead and is called out in the code so nobody reaches for it again — it does not fail, it *wedges* the headless Outlook. Polling it hung past 120 seconds and left an unresponsive `OUTLOOK.EXE` that had to be killed by hand, in a session where an ordinary read of the same folder takes 1.3 seconds. The upload is done by a background sync engine that reports through none of them, so a fixed wait is the honest mechanism rather than a fallback. The 30s figure is empirical and no stronger than that: it is the wait that was used the one time a stranded draft was confirmed to reach the server, established as sufficient on this mailbox and not as a minimum. The helper never raises, which is load-bearing twice over: a failed nudge still leaves the draft safely saved locally, so the failure mode is no worse than before, and `create_draft` runs inside the module's retry wrapper, which would re-run the whole operation and create a *second* draft if a COM error escaped from the sync step. The reporting is scoped to what is actually known: the CLI says a send/receive was requested and waited out, states plainly that delivery to the server is not independently confirmable, and warns separately whenever the wait did not actually happen — no send/receive group to start, the call to start it failing, or the wait being cut short partway. That last case is surface the fix created for itself: a command that used to return instantly now blocks for half a minute, which is easily long enough that Ctrl-C becomes a realistic thing for someone to do, and an escaping `KeyboardInterrupt` would print a stack trace over an operation that had in fact succeeded — the draft is saved in the first moment, only the upload window is lost. It is caught, the run ends on the honest "may not have reached the server" wording instead, and a wait that dies for any other reason is reported identically, since there is no distinguishing a blow-up at 1s from one at 29s.

## v0.101.0 — Document uploads and service hardening (2026-08-11)

### Changed
- **The systemd unit survives a boot where the bot's disk isn't mounted yet, and `start.sh` no longer races it** (`scripts/claude-bot.service`, `start.sh`). The bot lives on the NTFS Windows volume rather than under `$HOME`, so `%h`-relative paths were wrong and the unit is now written with absolute ones. The ordering problem underneath is that a *user* unit cannot order itself against system units — `network-online.target` and the volume's `.mount` unit are invisible to the user manager and were being silently ignored, so at boot the service could fire before the disk existed and fail. It now waits for the interpreter to actually appear (up to two minutes) in `ExecStartPre`, and `StartLimitIntervalSec=0` removes the burst limit so a slow mount can't exhaust the retries and leave the bot permanently down. Desktop session variables are hardcoded because linger starts the service before anyone logs in, so there is no desktop to inherit them from; they point at nothing until login and then GUI-driving work starts functioning without a service restart. Separately, `start.sh` used to kill any running bot as its first act — which, with the service active, meant systemd restarted the one it just killed and the script then started a second, unmanaged copy on top. It now detects an active service and hands over to `systemctl restart` instead, skipping the check when systemd is the caller (`INVOCATION_ID`).

### Added
- **Uploading a PDF or a Word document now works, and an upload can no longer vanish in silence** (`bot/discord/bot.py`). The attachment loop handled exactly three things — voice notes, `.txt`, and images — and every other file type fell straight through it to `if not text: return`. A PDF sent on its own, with no message alongside it, therefore produced *nothing*: no answer, no error, no log line saying why, so the only available reading was that the bot was broken. That blocked the case it was most obviously wanted for, since a work feedback form is essentially always a PDF or a Word file. PDFs now follow the pattern images already use — saved into the pending directory and handed to the session as a path — because the session's own Read tool parses PDFs natively, returning both the extracted text *and* a rendered image of each page, which is what makes a form with boxes and columns readable rather than a scrambled column of words. That was verified against a real PDF before the approach was committed to, rather than assumed, and it adds no dependency. The same check showed Read refuses `.docx` outright as a binary file, so Word is extracted here instead: a `.docx` is a zip whose `word/document.xml` holds the body, and every run of visible text is a `<w:t>` node, so the standard library gets it out with no dependency either. python-docx was the obvious alternative and was rejected for a specific reason — a lazy-imported package that is not installed degrades right back to "I can't read that", which is the exact failure being fixed, and requiring an install step defeats the point when the person uploading the file is non-technical. Table cells are built from the same nodes, so a form laid out as a table comes through too. Two hostile-input guards, since these files arrive from outside: the unpacked XML is size-capped, and a document declaring XML entities is refused rather than handed to a parser that would expand it. `.md`, `.csv`, `.json`, `.log`, `.yaml` and `.yml` join `.txt` as inlined text under the same 500 KB cap, documents cap at 20 MB. Most of the work is the last part: nothing is dropped quietly any more. An unsupported type, a file over the cap, a download that fails, a Word file that is really a scan with no text in it, or a voice note arriving while transcription is switched off each produce a short reply naming the file and saying what would work instead — written for someone who does not read tracebacks, and sent *before* the empty-text bail-out, which is the line that used to swallow them. Archive channels stay read-only. PDFs are appended to the same pending-paths list images use, so they inherit the identical cleanup — claimed by the usage-limit queue, unlinked when the turn ends, swept after 48h if orphaned — rather than accumulating on disk. Covered by `scripts/test_attachments.py` (40 checks), which drives the real `on_message` with stand-in Discord objects rather than reimplementing the loop, and asserts a user-visible reply in every failure case plus a path reference reaching the prompt for a PDF sent with no text.

## v0.100.0 — Local meeting capture and automatic recording (2026-08-07)

### Added
- **Local meeting capture + transcription** (`bot/services/meeting.py`) — records both sides of a call locally (system audio via WASAPI loopback + your microphone), mixes them to 16 kHz mono, and transcribes with a local faster-whisper model (default `large-v3`, `MEETING_WHISPER_MODEL` to override, Dutch by default). Teams/tenant-independent: it captures whatever plays through the speakers, so it needs no Azure bot, no admin consent, and no cloud round-trip — nothing leaves the machine. CLI mirrors the `outlook.py` pattern (invoked via Bash): `devices`, `record <name> [--minutes N]`, `transcribe <wav> [--lang]`, `live <name> [--chunk N]` (rolling transcript appended to `data/meeting/<name>/transcript.txt` for near-live note-taking), `stop <name>`, `status <name>`. Capture runs one COM-initialised recorder thread per device; GPU inference auto-loads NVIDIA cuBLAS/cuDNN from the `nvidia-*-cu12` wheels by prepending their bin dirs to PATH, and validates the GPU with a dummy inference so a machine without working CUDA falls back to CPU cleanly instead of failing every transcription. Live mode starts capturing *before* the model finishes loading (large-v3 takes ~20 s to warm into VRAM) so the opening of the meeting isn't lost — the audio buffers and is transcribed in the first chunk. Whisper's silence-hallucinations (invented filler like "bedankt"/"goedendag" during pauses) are dropped by a confidence filter (`no_speech_prob > 0.6`, `avg_logprob < -1.0`, or runaway `compression_ratio`), tuned against measured speech-vs-silence scores on this machine. Dependencies (`soundcard`, `faster-whisper`, `numpy`) are declared as the optional `meeting` extra and imported gracefully, like `pywin32` for Outlook, so the bot still runs where they are absent.
- **Automatic recording from the Outlook calendar** (`bot/services/meeting_watch.py`) — the half that makes the recorder useful, since a transcript you have to remember to start is one you start after the meeting began. Polls the calendar (`watch [--poll 60]`, plus `status` and a single-tick `once` for debugging); when an event that is running *now* looks like an online meeting — Outlook's own `IsOnlineMeeting` flag, or a Teams/Zoom/Meet/Webex fingerprint in the location or body — it spawns `meeting.py live <slug>` detached and stops it with the `.stop` sentinel at the scheduled end. Overlapping calls resolve to the most recently started one, on the assumption that is the one you just joined; all-day events are skipped so a week-long "Conference" placeholder does not record for a week. Three things the draft got wrong were fixed before it ran unattended. The calendar `Restrict` filter is locale-dependent and can raise *or* silently match nothing, which would have left the watcher permanently convinced there was never a meeting; it now probes `.Count` and falls back to a bounded scan of the sorted folder, the same restrict-then-fallback shape `outlook.read_calendar` already proved on this machine. The detached child's output went to `DEVNULL`, so a live process dying at startup (no loopback device, missing faster-whisper) left the watcher reporting "recording" with nothing to show and no way to find out why — it now lands in `data/meeting/<slug>/live.log`. And the `.recording` marker held only an end time, so if the child died mid-meeting — a crash, or a reboot that killed it while the marker survived on disk — the watcher saw the marker, assumed all was well, and silently recorded nothing for the rest of the call; the marker now carries the pid (JSON, with the old bare-ISO form still readable) and a tick that finds it dead clears it so the next tick restarts the recording. Single-instance locked by pid file. Covered by `scripts/test_meeting_watch.py` (18 checks against fake calendar items and markers in a temp directory — no Outlook, no microphone, no spawned recorder), registered in `.claude/test.json`.
...
### Fixed
- **The project's own checks reported 12 failures that said nothing about the code.** A verify run on Linux failed 12 of the 15 harnesses in `.claude/test.json` at import with `No module named 'dotenv'` — failures shaped exactly like real ones, arriving before any test logic ran, so a healthy tree looked catastrophically broken. Only the three that never reach the bot's dependencies survived, which made it worse rather than better: a partial pass reads as a real regression in the other twelve. The cause is that the bot is started as `.venv/bin/python3 -m bot`, by direct path rather than by activating the virtualenv: `VIRTUAL_ENV` is unset and `.venv/bin` is absent from `PATH` (confirmed from the live process), so every process it spawns resolves a bare `python` to the *system* interpreter, which has none of the bot's dependencies — and all 15 commands are written as `python scripts/x.py`. Those command strings are deliberately left alone, because `.claude/test.json` is read by both installs of this dual-boot machine and hardcoding `.venv/bin/python` would fix Linux by breaking Windows. Instead a stdlib-only `scripts/_bootstrap.py`, imported first by each script, relaunches it under the project's virtualenv when the dependencies are not visible, picking the right location per OS. It also follows a worktree's `.git` file back to the parent checkout, which is what makes this work at all: chain verify steps run inside a build worktree, and a worktree never carries its own `.venv`, so without that step the fix would have missed the one place it matters. Two traps worth naming. The same-interpreter check must compare paths *unresolved* — a virtualenv's `python` is normally a symlink to the system binary, so resolving symlinks first makes `.venv/bin/python` and `/usr/bin/python` look identical and skips the only relaunch that was needed; what makes an interpreter "the venv one" is the path it is invoked by, since that is what `sys.prefix` derives from. And an environment-variable guard bounds it to a single relaunch, so a broken virtualenv fails fast with the real import error instead of spinning. Rejected alternative: having the bot put its virtualenv on `PATH` for children — this bot manages several repos, so that would quietly hand this project's dependencies to sessions working on other projects, trading a loud wrong answer for a silent one. Windows relaunches differently, and it has to: its C runtime has no process replacement, so `os.execv` there *spawns* a new process and kills the caller — the waiting parent unblocks immediately with a success code while the real script is only just starting. A check harness would report a pass without having run, which is worse than the failure being fixed, and it is not hypothetical, since the same shared `test.json` drives the Windows install of this machine. Windows therefore waits on a child and forwards its exit code; POSIX keeps `execv`, where same-pid, same-descriptors is exactly what the caller wants. The single-relaunch guard is also cleared again the moment the dependencies are confirmed visible: it means "this process already relaunched and it did not help", and left set it would export that claim to every child, barring a script spawned by a bare `python` from correcting itself on the strength of an attempt it never made. Four smaller edges are handled because each one fails confusingly rather than cleanly: a worktree whose `.git` file holds a *relative* path (git writes one under `worktree.useRelativePaths`, and it is relative to the worktree rather than to the caller); an interpreter that passes the executable-bit check but cannot actually be exec'd, which now falls through to the next candidate instead of dying on a traceback about `exec` that says nothing about the missing dependency; invocations where the command line cannot be replayed, such as `python -c`, where replaying it would produce a baffling `Argument expected for the -c option`; and a repo reached through a symlinked parent, where the recorded prefix and the derived one are spelt differently for the same virtualenv — that comparison resolves symlinks, unlike the interpreter one, because for a directory it is safe and necessary. When it does give up it now says which of the three dead ends it hit, rather than telling someone standing inside a virtualenv that none was found. The "is this the right interpreter" test asks after two packages rather than one, because one is a single point of false reassurance: `python-dotenv` is distro-packaged nearly everywhere, so an unrelated `dnf install` could put it on the *system* interpreter and switch this whole mechanism off without anyone touching it — after which a harness dies on the next missing package instead, with nothing left to explain why. `discord.py` is not distro-packaged, so the pair is only satisfied together by a real install. Confirmed by putting a stand-in `dotenv` on the system interpreter's path: the single-package test called it healthy, the pair correctly named `discord` as missing and relaunched anyway. Finally, the convention is enforced rather than remembered: `check_portability.py` gained a check that every script `.claude/test.json` runs either imports the shim or imports nothing outside the standard library, so a harness added later without the line fails the guard instead of failing mysteriously on Linux — the same "registered but not wired up" gap that has already cost this project once. That also covers the six `interact` commands, which are the end-to-end surface a verify run drives and were the one thing the first pass missed. Verified by running all 15 harnesses under `/usr/bin/python`, the interpreter that failed 12 of them beforehand: 15/15 pass, unchanged under the virtualenv interpreter, and both the health check and the Discord interact command return byte-identical output either way.
- **A failed sync or deploy could post an access token into Discord.** Found while checking whether the new reason-picker was needed anywhere else. Five places reported a failed git exchange by slicing its raw stderr and sending it — the fetch, pull, push and tag push behind the repo sync/deploy button, and `gh repo create --push`, that last one with no length bound at all. When an HTTPS remote carries an embedded credential git quotes the whole URL back inside `fatal: Authentication failed for 'https://user:ghp_…@github.com/o/r.git/'`, and nothing scrubs secrets on the way out: `redact_secrets` is applied at individual call sites rather than at the send boundary, and none of those five were among them. Truncating to 200 characters does not remove something that appears near the start. All five now route through the same picker as the merge notes, which redacts and bounds. The picker is public (`git_fail_reason`) for that reason — a leading underscore on a name imported from another package misdescribes the interface. Purely local git calls are deliberately left alone: `git rev-parse` and a local merge cannot carry a credential.
- **The secret scrubber missed two credential forms, and had no tests.** It is the control the fix above depends on, so it got audited rather than assumed. Two holes, both in URL credentials. It required 8 or more characters between the `:` and the `@` of a URL, so a short password printed verbatim — but that field is the userinfo half of a URL, which is a credential by definition, and there is no length below which it is safe to show; the floor is gone. And it only understood a credential in the *password* half, while `https://<token>@github.com/o/r.git` is a form GitHub accepts, so a token in an unrecognised vendor format (a GitLab `glpat-…`, say) matched nothing and went out whole. Userinfo of 20 characters or more is now redacted on its own, a floor set well clear of the real usernames that appear in remote URLs — `git`, `oauth2`, `x-access-token`, `gitlab-ci-token` — so the bot's own SSH remotes stay readable. Checked in both directions, because a scrubber that eats real output is its own bug: the key=value rule fires on words like "password" and "token", which git and GitHub use in ordinary sentences (`remote: Invalid username or token. Password authentication is not supported`), and those come through untouched.
- **Git can no longer stop and wait for a human, so a credential failure is an error instead of a hang.** Every push since the move to Linux died as `Push to origin timed out (30s)` — three times overnight in one repo — with nothing in the log naming a cause, because the cause never got as far as the log. Git run as a subprocess with no terminal does not give up when it needs a credential: it looks for a *graphical* askpass helper, and on KDE it finds `SSH_ASKPASS=/usr/bin/ksshaskpass` and blocks on a dialog nobody is watching until the timeout fires. The timeout then reports a network problem that was not happening and hides the credential problem that was. New `procutil.harden_git_env()`, called at the top of `app.run()` before anything can shell out, sets `GIT_TERMINAL_PROMPT=0`, `GIT_ASKPASS=""` (empty-*but-set* on purpose — git only falls through to `SSH_ASKPASS` when `GIT_ASKPASS` is unset, so an empty value both disables the helper and shadows the desktop's; deleting it would hand control straight back to ksshaskpass) and `SSH_ASKPASS_REQUIRE=never` for ssh's own passphrase path. One call at startup covers every git call site in `bot/` — eleven modules — plus git run by the Claude CLI children, which inherit `{**os.environ}`, and cannot be forgotten at a call site added later. Credential *helpers* are untouched, since git consults those before it ever prompts, so Git Credential Manager on Windows keeps working. The trade-off is deliberate: a passphrase-protected SSH key must now be pre-loaded with `ssh-add` rather than unlocked on demand, because for a headless bot a hang is worse than an error. Covered by `scripts/test_git_prompt_hardening.py`, which runs a real `git` against a URL that demands authentication with `SSH_ASKPASS` deliberately pointed at a helper that answers, and asserts it fails in seconds naming its own cause rather than blocking to the timeout. Pointing it at a helper that *answers* is what gives the test teeth: remove the shadow and git takes the prompt text back as a username and fails with GitHub's `Password authentication is not supported` instead, so the assertion distinguishes a working block from an absent one. It is registered in `.claude/test.json`, so a verify run picks it up rather than only the harnesses that predate it.
- **A failed git exchange now tells you what went wrong in Discord, not just in the log.** The merge note carried the exit code alone. That stayed hidden while git was hanging — the note said `Push to origin timed out (30s)` instead — but disabling the prompt turns those hangs into fast non-zero exits, which would then have arrived on the phone as a bare `Could not push to origin (exit 128)`, with the sentence naming the cause still stranded in `bot.log`. Fixing the hang without this would have swapped one uninformative message for another. All four user-facing git failures in a merge now append git's own explanation (`git_fail_reason` in `runner.py`): the branch push, the tag push, the pre-merge fetch and the fast-forward to origin. The last two matter most — a failed pre-merge fetch does not stop the merge, it silently degrades to merging against stale local state under the note "Could not sync with origin", and it is the branch the credential hang actually landed in (`Fetch origin failed ... timed out (30s)`). Picking the right line takes some care and "the first one" is wrong: a credential failure does lead with `fatal: could not read Username ...`, but the far more common failure — a branch behind its remote — leads with `To github.com:owner/repo.git`, which explains nothing while looking like it should, and buries `! [rejected] ... (fetch first)` on the next line. So it prefers the first line git marks as a diagnosis (`fatal:`, `error:`, `remote:`, `!`) and only falls back to the first non-blank line if none is marked. `hint:` is deliberately not a marker, being the advice that follows the reason rather than the reason. Transfer statistics are dropped too, since the server sends those prefixed `remote:` as well and they arrive *before* its refusal. Two things had to be right there. They are matched as a category rather than by symptom, because filtering on "quotes a percentage or ends in `done.`" looks equivalent but lets through the one line that has neither, `remote: Total 3 (delta 0), reused 0 (delta 0)`. And they are dropped up front rather than merely skipped while hunting for a marker — otherwise the fallback hands the same byte count straight back, which is precisely what happens when git's real message carries no marker at all, as in `The remote end hung up unexpectedly`. Run through `redact_secrets` and length-capped, because it is user-facing and a remote URL can carry an embedded token.

## v0.99.16 — Dual-boot path portability (2026-08-06)

### Added
- **Absolute paths now mean the same folder from either operating system.** `.env` and `data/state.json` live on the shared Windows drive and are read by *both* installs, but every path in them is absolute, so `C:/Users/Quincy/…` and `/run/media/quincy/SYSTEM/Users/Quincy/…` are the same directory under two names that neither side recognises as the other. The previous answer, `scripts/migrate_to_linux.py`, is a one-shot one-directional rewrite — on a dual boot it would have to be run in both directions on every reboot, and it still strands conversation history, because the CLI files history under a directory named after the path it was launched from. `bot/paths.py` instead keeps storing whatever spelling the local machine uses and translates on the way *in*: two doors take every path (the `.env` load and the state-file load), so all ~300 downstream readers are untouched, and nothing is migrated, rewritten or made non-backwards-compatible. The map is discovered rather than configured — a `.bot-root-id` marker holding a UUID is written inside the user root, travels with the volume, and is read identically by every machine that mounts it, so booting Windows once and Linux once assembles the map with nothing typed and no heuristic guessing about what a path "looks like". A machine also identifies itself by walking up from its own code to that marker, which is what makes an unpredictable mount point — a live-USB session, a relabelled drive — resolve without being told anything; the configured account paths and `$HOME` are only fallbacks for a first-ever boot. Translation is prefix-based and read-only; an empty or partial map passes paths through untouched, which is exactly today's behaviour rather than a corrupted path. On the state side that covers registered repos, each instance's working tree, build worktree and pinned account, the two artefact files behind `/log` and `/diff` (whose failure is silent — the button just returns nothing), the account-keyed cooldown and outage tables, and scheduled tasks, which fire unattended and so fail with nobody watching. What is deliberately *not* rewritten: prompts, recorded bash commands and path-poisoning records, which are evidence of what happened rather than handles to open. Conversation history is looked up under every known spelling of the directory rather than just the local one, which matters because the CLI files history under a directory named after the path it was launched from: this checkout alone has history under three such directories right now (the Windows spelling, the Linux mount, and a past live-USB session), and encoding only the local name made the other two invisible. That applies at all three places that go looking — resuming a conversation, the `/session` picker and its newest-session-for-this-repo lookup, and the pointer the bot hands the model to its own past sessions — with one shared helper so a fourth caller can't quietly be blinder than the rest. Resume additionally falls back to a glob on the session's UUID when the conversation was written from a spelling the map has never seen — with the UUID shape checked rather than assumed, since a wildcard reaching that pattern would hydrate somebody else's conversation. Boot logs the map plus a warning for any registered repo living outside every known root — the condition that stranded The Citadel. Settings that genuinely differ per machine rather than being two names for one thing — `CLAUDE_BINARY` above all, an OS-native install location with no counterpart to translate to — go in an optional `.env.linux` / `.env.windows` / `.env.darwin` overlay loaded on top of `.env` (and git-ignored, since it holds the same secrets). Harness: `python scripts/verify_paths.py` (74 checks), `--show` prints the live map. Both new harnesses are registered in `.claude/test.json`, so a verify run picks them up rather than only the ones that predate this change.

### Fixed
- **A conversation the CLI could not find was reported as "every Claude account is signed out", on a loop, for most of a night.** The repo had two separate checkouts — one on the Linux home partition where it was registered, one on the shared drive where all its history actually lived — so `--resume` returned "No conversation found" every time. That is supposed to be survivable: the recovery cascade rebuilds the session index, then tries the other account, then abandons the conversation and runs blank. It never reached the blank run. The middle layer marks the account it just used as "already tried", meaning *this conversation isn't filed there* — but that claim stops being true of anything the moment the conversation is abandoned, and the blank run inherited the mark anyway. With the backup account genuinely signed out, the picker was left with zero candidates and refused to spawn at all, on a machine whose primary account was answering normally at that exact moment. The refusal then restored the dead conversation id so the retry queue could resume it, and the whole cycle re-ran every 16 minutes from 22:15 to 01:03. Three corrections. The abandon step now clears the tried-set — real reasons to skip an account, usage limits and failed auth, are recorded separately as cooldowns and still apply. The dead id is handed back only when the attempt was genuinely blocked on account availability; a blank run that simply produced nothing has already proven the conversation exists nowhere, and re-queueing it rebuilds the loop. And the refusal now has three states rather than two, because the middle one is the common one and collapsing it either way loses something: "every account is signed out" requires that every account actually be signed out, "no account was free" is for when nothing is signed out at all, and the case in between says so plainly and keeps offering the auth-panel button that would in fact unblock it. Harness: `python scripts/test_dead_session_recovery.py`, which drives the real cascade through a stubbed subprocess boundary; reintroducing the tried-set bug makes it fail on the behaviour that caused the incident — the blank run never spawns, and the dead conversation id goes back to the retry queue.
...
### Fixed
- **A failed stash-pop no longer leaves the repo unable to start, and no longer does it quietly.** Merging a build branch stashes the main repo's uncommitted work first; if the pop back conflicts, a rollback is supposed to undo it. That rollback ran `git checkout -- .`, which by definition refuses to touch unmerged paths (`error: path ... is unmerged`) — so it failed on **every** occasion it was actually needed, and this repo spent an afternoon with conflict markers in 19 tracked files including `app.py`, `config.py` and `runner.py`. The bot survived only because it was already running; any restart would not have come back. It is the fourth occurrence, three earlier ones in another repo. Rollback is now `git reset --hard HEAD` — the operation that *does* clear an unmerged index, and safe at that point precisely because the tracked-dirty check just above has already established the tree carried no other tracked work — and its success is confirmed against `git diff --diff-filter=U` rather than believed from an exit code, since the whole failure mode is a rollback that reports success. The trigger was line-ending drift: a Windows-era checkout left 121 files differing from an LF-normalised history by nothing but `\r`, so the merge stashed all of them and the pop collided with the normalisation commit. Separately, the warning it emitted was posted into a thread that autopilot archived four seconds later, so nobody saw it; a merge that lands but leaves the tree broken is now classified apart from a merge that fails (`merge_msg_repo_unusable`), skips the post-merge deploy that would restart into the broken tree, keeps the thread open, and pings.
- **`.env.*.bak` and generated `*.desktop` files are ignored.** The Linux migration script writes `.env.windows.bak` / `.env.liveuser.bak` next to `.env` — same tokens, same blast radius — and neither was ignored, leaving them one `git add -A` from being committed.
- **A button tap can no longer dead-end without saying anything.** Chasing a report of buttons spinning forever turned up a Discord-side incident — interactions were not being delivered to the bot at all, which no code change can fix — but the hunt exposed four paths where the *bot* produces the identical symptom, and those are ours. An out-of-scope tap and an unroutable `custom_id` both returned silently without acknowledging, so Discord spun for its 3-second window and then showed "This interaction failed" with no reason; an exception inside the dispatcher was logged but never answered, doing the same; and `handle_callback`'s 29-branch `if/elif` chain had no terminal `else`, so a button from an older build was acknowledged and then visibly did nothing. All four now settle: new `interactions.settle()` acknowledges from any interaction state — before the initial response, after a defer, or after the window has already closed — and the dispatch chain names the action it could not route. The point is diagnostic as much as cosmetic: an unacknowledged interaction looks exactly like a bot that is down, which is what made this outage take an hour to attribute.
- **The Control Room rename loop stopped renaming threads to their own name.** The legacy-name migration tested `thread.name in ("Control Center", "Control Room")` while the destination *is* `"Control Room"`, so every control-room refresh issued a no-op `thread.edit(name=…)` — roughly every 1–2 minutes across three threads. Discord caps thread renames at 2 per 10 minutes each, so the bucket was permanently spent on a rename that changed nothing, leaving no headroom for a real one. Now guarded on the current name.

## v0.99.14 — Linux/Windows parity (2026-08-06)
### Added
- **Linux migration kit.** The daily driver is moving from Windows 10 to Fedora KDE on a new 4 TB NVMe, and the bot has to be back up the same day — it *is* the way the machine gets talked to. An audit of every platform-specific branch found the code already portable: each Windows path has a working POSIX counterpart (process-tree kill via `os.killpg` vs `taskkill`, the login terminal falling back through `x-terminal-emulator`/`gnome-terminal`/`konsole`/`xterm`, the `$DISPLAY`/`$WAYLAND_DISPLAY` console probe, the `/etc/claude-code` managed-settings path), every dependency is pure-Python, and the one Windows-only package is already gated behind a `sys_platform` marker. What does not survive the move is *configuration*: three absolute paths in `.env`, all 13 registered repo paths in `data/state.json` plus the `repo_path`/`worktree_path`/`session_account` fields across 254 instance records, and two build worktrees registered against Windows gitdirs. `scripts/migrate_to_linux.py` rewrites all of it — dry-run by default so it can be previewed from Windows before the switch, `*.windows.bak` copies before any write, and absolute paths outside the old home are *reported rather than guessed at*. Also `start.sh` (the `start.bat` counterpart, preferring `data/bot.pid` over a blind `pkill`) and `scripts/claude-bot.service`, a systemd **user** unit — a system unit would read the wrong `$HOME` and every Claude session would come back signed out. Full runbook in `docs/linux-migration.md`, including the one step that is easy to miss: without `loginctl enable-linger` a user service stops at logout and does not return until the next login, i.e. the bot is down precisely when nobody is at the machine. Only casualty of the move is the optional Outlook integration, which needs `pywin32`.

### Fixed
- **The repo can now be worked from Linux and Windows without either side corrupting the other.** With no `.gitattributes` each platform guessed line endings via `core.autocrlf` and the guesses disagreed: the Windows checkout held CRLF against an LF history, so the *same clone* read clean on Windows and **121 of 143 files modified** on Linux, with not one character of content differing. One commit from the Linux side would have flipped every file, turning every subsequent diff, code review and build-worktree merge into whole-file noise, and conflicts into whole-file conflicts. Endings are now pinned to LF in history and on disk on both platforms, with `.bat`/`.cmd`/`.ps1`/`.vbs` kept CRLF because `cmd.exe` mis-parses multi-line LF batch. `TODO.md` was the one file committed with CRLF and is renormalized, byte-identical otherwise. Separately, `start.sh` and `scripts/bootstrap_linux.sh` were recorded in git as non-executable — invisible on this clone because it sits on an NTFS mount where every file reports as executable, but a fresh clone onto ext4 gets `Permission denied` on the first command of the setup guide. **The Windows laptop needs a one-time `git rm --cached -r . && git reset --hard` after pulling**; runbook in `docs/linux-migration.md`.
- **Every CLI session was invisible on Linux.** The bot located session history by appending to `Path.home()`, but it launches the CLI with `CLAUDE_CONFIG_DIR` pointed at whichever account is active, so sessions land under *that* directory. On Windows the two coincide (`C:/Users/x` and `C:/Users/x/.claude`) which is why this survived for so long; on Linux the account dirs sit on another filesystem entirely, so the bot scanned a directory holding **4** project folders while the real **903** lived elsewhere. `/session list`, `/session resume`, smart thread titles, "branch from here" and the sessions-index rebuild were all reading a near-empty tree and silently finding nothing. Resolution now follows `CLAUDE_ACCOUNTS`, and readers scan *every* configured account rather than just the primary — which also fixes a bug present on both platforms: a session started on the backup account was previously unreachable. Reassigning the primary dir (as the sandboxed test harnesses do) still confines the scan to that one path, because some of these callers delete files and widening a deliberately narrowed scope would be destructive. One wrinkle that had to be handled: boot drops account entries whose directory has gone missing, and everything derived from the list — the primary root, the `CLAUDE_CONFIG_DIR` pin — has to follow. Deriving it lazily instead meant that after a prune the root no longer matched the derivation, which is exactly how a sandboxed harness announces itself, so the scan treated the live config as a deliberate override and returned *nothing*. The list is now only ever changed through one setter that refreshes the rest. Harness: `python scripts/test_projects_dirs.py`.
- **`/session list` was quietly returning fewer sessions than asked for.** It collected every session file, sorted newest-first, took a window of thirty and only *then* discarded repeats. But one session is not one file: the runner mirrors session history between accounts and copies it out of a build worktree's project folder into the main repo's, so the same conversation exists three or four times over — on this machine 1290 files hold 368 distinct sessions, and those thirty rows carried about thirteen real ones. Anything that then failed to parse ate the same budget. Small asks were unaffected, but the count plateaued as the ask grew — measured on this machine, a request for 20 returned 18, for 25 still 18, for 30 returned 22, for 40 returned 28. `/sync` asks for three times the thread count, so it was landing squarely in that range. Duplicates are now collapsed before the window is taken, and every one of those limits returns the number asked for. Which copy wins is unchanged: still the most recently written.
- **Thread-title generation ran against an account that may not be signed in.** The title subprocess inherited the bot's environment without pinning `CLAUDE_CONFIG_DIR`, so it fell back to `$HOME/.claude` — the same directory that on Windows *is* the configured account and on Linux can be an unrelated, possibly unauthenticated config. It is now pinned to the primary configured account, and the stale-jsonl sweep covers every account root so leftovers written by the old behaviour are still cleaned up.
- **Branching a session could strand the fork on the wrong account.** When the source jsonl wasn't under the primary account, the global fallback search found it but the fork was still written to the primary — and since sessions are pinned to the account that started them, `--resume` would look beside the source and miss. The fork is now written next to its source.
- **The Linux login-terminal launch reported success when nothing opened.** It passed `-e` to every candidate emulator, but GNOME dropped that flag in favour of `--`, so a modern `gnome-terminal` exited non-zero without ever showing a window — and because only a missing *binary* was caught, that counted as a win and the user was told login was open. Each emulator now gets its correct argument form, the candidate list covers the current crop (kgx, ptyxis, alacritty, kitty, xfce4-terminal), a launched terminal is judged by its **exit code** a moment later rather than by whether it is still running — gnome-terminal, kgx and ptyxis are D-Bus clients that hand the window to a separate server and return 0 immediately, so "still alive" alone would have rejected the terminals most likely to be installed — and total failure prints the exact command to run by hand instead of "check bot logs". The probe also moved off the Discord event loop, where its waits would have stalled the heartbeat. Verified on this host: `konsole -e sleep 3` is still running after the check (accepted), `konsole --definitely-not-a-flag` exits 1 (rejected).
- **Verify could never restart a bot it found down on Linux.** `.claude/test.json` is static JSON with no way to branch per platform, and its `start`/`stop` named `start_bot.bat`/`stop_bot.bat` — files that do not exist on Linux. Both now go through `scripts/botctl.py` (`start|stop|restart|status|logs`), which works on either OS, waits for the bot to actually claim its PID file rather than assuming a spawned process means a running bot, and stops the whole process tree so CLI children don't outlive it holding worktrees open. The log-reading command had the mirror-image problem — it was `tail -n 50`, which does not exist on Windows — so `botctl.py logs [n]` covers that too. The `.bat` wrappers stay for double-clicking from Explorer.
- **The verify step could have started a second bot.** This project is a singleton — one Discord token, one `data/state.json`, one PID file — and `.claude/test.json` says so at the top: never boot a copy, drive the bot that is already running, and only use `start` if the log says it is down. But builds run in a git worktree, and a worktree gets its *own* empty `data/` tree: no PID file, an empty `logs/`, no `.env` beside it. Both the health check and the new control script derived those paths from their own location, so from inside a build they read a blank installation, correctly-but-uselessly reported the bot as down — and the next step in that rule is to start it, which would have connected a second gateway on the same token and begun writing a second state file. Both now resolve to the real installation via git's own view of where the main checkout is, so from a worktree they report on, and act on, the running bot. Verified from this build: the health check now reads the live log instead of an empty one, and `status` finds the live process instead of claiming nothing is running.
- **A recycled process ID could have got an innocent process killed.** The PID file survives an unclean exit, and the kernel eventually hands that number to something else. Everything that read the file trusted it, so `stop` would have sent SIGTERM and then SIGKILL to that stranger's whole process *group* — on a desktop, plausibly a terminal session — while `start` would have seen "already running" and quietly done nothing, which is the one job it has when verify finds the bot down. The PID must now also still *look* like the bot before anything is signalled, checked one-sidedly: it answers "not the bot" only on positive evidence and assumes the worst everywhere it cannot see, because guessing wrong the other way would start a second instance. Three copies of the liveness check had also all been catching a bare `OSError`, which silently includes "this process exists but belongs to another user" — so a live process could read as dead. Demonstrated on this host: the old form reported PID 1 as not running. All three now share one implementation (`bot/procutil.py`), which also carries the working-directory resolution above.
- **`/reboot` did nothing on Linux, and a killed bot never came back.** Both restart paths spawn a helper that waits for the old process to exit and starts a fresh one, and both passed `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP` — Windows-only names that **do not exist** in POSIX Python. Reaching for them raises `AttributeError` at the moment of the spawn, and both sites sat inside an `except` broad enough to hide it: the emergency handler's was a bare `pass`, so a bot stopped with `kill` saved its state, declined to relaunch, and exited for good; the `/reboot` path's logged a generic failure and dropped the request, so the bot stayed up and the reboot silently never happened — including reboots the model requests for itself via `data/reboot_request.json` after changing bot code. The logs bear it out: every coalesced reboot on record is dated 21–23 March, the Windows era, and there is not one since the move. Both now use a single shared helper that spells the Windows flags as literals and uses `start_new_session` on POSIX, which additionally keeps the relauncher out of the dying bot's process group so a group-kill can't take it down with it. The bare `pass` is now a logged exception, because a restart path that fails invisibly is how this survived at all.
- **Fixing that made the bot unstoppable, so stopping it is now a two-part handshake.** The bot answers a signal by relaunching itself, so an agent killing it doesn't leave it offline. On Windows that only ever fired for an unusual signal — `taskkill /F` runs no handler at all. On Linux `SIGTERM` is how *everything* stops a process, so with the relaunch working, `botctl stop`, `start.sh`, `systemctl stop` and a plain `kill` would each have been answered by a fresh bot seconds later; `stop` "worked" beforehand only because the relaunch it triggered was crashing. A caller that means it now writes `data/stop_requested` first and the handler consumes it and stays down. All three supported stop paths do this for you (`botctl.py stop`, `start.sh`, and `ExecStop=` in the systemd unit); a bare `kill` is deliberately not one of them and still brings the bot back. The marker is consumed on read so one request answers one signal, expires after two minutes, and is cleared at startup, so a stop that dies half-way through cannot leave the relaunch disabled. Separately, under systemd the unit's own `Restart=always` does the restarting and the bot now skips its self-relaunch — otherwise both race for the PID file and, if systemd's copy loses, it restarts into an immediate exit until the unit is marked failed, leaving a bot running with nothing watching it. Detected from the cgroup rather than from `INVOCATION_ID`, which a desktop terminal also sets. Harness: `python scripts/test_stop_sentinel.py`, which drives real signals at a stand-in process and checks all three outcomes.
- **The rule for naming a session's storage folder was written out in six places, and four of them were wrong.** The CLI names each project folder after the absolute working directory with `:`, `\`, `/` and `.` all flattened to `-`. Five places in the bot reimplemented that instead of calling the one shared version, and four of the five forgot the `.` — so any path containing a dot was looked up under a name the CLI had never written. A build worktree at `…/claude-telegram-bot/.worktrees/t-6839` was searched for as `…-bot-.worktrees-t-6839` while the real folder is `…-bot--worktrees-t-6839`. Consequences, worst first: "find the latest session for this repo" could never match a build worktree; the pointer to its own session history that the bot hands the model in the system prompt aimed at a folder that does not exist, and separately looked only under the primary account, so a session running on the backup account got no pointer at all; and the title-generator's cleanup sweep is only correct today because no common temp directory happens to contain a dot — set `TMPDIR` to one that does and every title-generation call would leave a stray session in the picker forever. All five now delegate, and the shared version names them so the next copy has somewhere obvious not to be written.
- **A blocking test could fail at random, roughly once every few hundred runs.** The account-failover harness checks that the CLI's raw "401" authentication error never reaches you in place of a readable one — but it looked for the bare digits `401` anywhere in the message, and that message quotes the account's config directory, which during the test is a throwaway folder with a random name. Caught it in the act: the folder was `/tmp/acct_failover_26401es7`, so the test failed on its own scaffolding while the code was behaving perfectly. Since this harness gates the chain, that is a build stopped for nothing. It now matches the CLI's actual wording instead of a number that can turn up in a filename.
- **The X-voice corpus fetcher could not find its credentials on Linux.** `scripts/fetch_x_corpus.py` reads the sibling DegenAI repo's gitignored `settings.json` for the paid Twitter API token, and the location of that file was a hardcoded `C:\…` path — so on the desktop it exited on the very first read and the writing-style corpus could not be refreshed at all. It now prefers `$REPOS_BASE_DIR` when that is exported, then walks up from its own location, which also covers being run out of a build worktree where the repo root sits two levels deeper than usual; `--settings` still overrides both. Found by the portability guard, but only once that check actually worked.
- **Session-history paths were reconstructed with backslashes on every platform.** `_project_dir_to_original_path` reverses Claude's encoded project-directory names (`C--Users-Quincy-…`) back into a real path, and its non-drive-letter fallback joined with `\` unconditionally. On Windows that is right; on Linux an encoded `-home-quincy-…` came back as `\home\quincy\…`, a path that can never match a real working directory — so `/session list` and `/session resume` would show and target garbage from the first boot on the new install. The drive-letter form is still decoded on any host, so project directories carried over from the Windows install keep reading sensibly; everything else now follows the running platform's separator.
- **The health check reported a live bot as down, which on a singleton points straight at booting a second one.** `.claude/test.json` names `scripts/smoke_test.py` as the health command *and* tells the verify step to run `start` when it says the bot is down, so a false negative here ends with two bots on one Discord token and one `state.json`. It produced one every time: it read a fixed 200 lines from the end of the log, so the better the uptime the further "Bot ready" scrolled out of reach — after eleven healthy hours it sat 2,575 lines past the window, and a bot that was talking to Discord that same second was reported as never having started. It now reads the whole log (which rotates at 10 MB, with a 12 MB seek as a backstop) and, more importantly, checks `data/bot.pid` against the running process first: the log tells you what happened, the PID tells you what is true now, and nothing was checking the latter. Its idea of "startup" was wrong too. It bounded the boot by hunting for `"Acquired PID lock"` or `"Starting bot"`, and **neither string had ever been written by any version of the bot** — `grep -c` returns 0 for both across 73,666 lines — so it ran back to the *previous* boot's "Bot ready" and judged this startup by ten hours of unrelated runtime, failing on a git push that timed out in another repo nine hours in. The marker is now genuinely emitted on taking the PID lock; the window is bounded at both ends and, for logs written before that marker existed, by the clock; and post-startup errors are reported separately and never fail the run. The marker stays authoritative wherever it appears, because an early cut let the clock truncate a boot that legitimately took longer than the slack — caught by the new harness, not by reading. Finally, a signed-out backup Claude account was logged at ERROR, so the second account being logged out made *every* startup on this machine read as a failed startup; it is a handled condition that rotation carries on without and that heals itself on login, and it is now a WARNING, exactly as the "failover degraded" line beside it always was. Pinned by `python scripts/test_smoke_window.py`.
- **The diagnostic scripts could not be run from a build worktree — which is the only place verification ever runs.** `.env` is gitignored, so a worktree never has one, and both `scripts/discord_test.py` and `scripts/discord_read.py` looked for it beside themselves. `discord_test.py` is the interact surface `.claude/test.json` points the verify step at, and it died on `Error: .env not found` before reaching Discord at all. Both now resolve against the *installed* bot, the same way the health check already did: the credentials belong to the one live bot regardless of which checkout is driving it.
- **`wait-response` timed out against a bot that had already answered, and could have passed on its own message.** It takes its "newest message" baseline when it starts, but `.claude/test.json` documents `send` and `wait-response` as separate commands — and the bot replies in about two seconds, so by the time the second command runs the reply is already the newest message and can never look new. It waits out the full timeout standing on the answer. It now accepts the message id `send` prints. The opposite failure was latent in the same loop: Discord marks webhook messages `bot: true`, so the probe this harness had just sent counted as the bot replying to it — a false pass, which is worse. Webhook authors are now skipped.

### Added
- **A portability guard, so this class of bug fails loudly instead of silently.** Every problem above was invisible from whichever machine you happened to be sitting at. `python scripts/check_portability.py` fails on CRLF in the git index, a `.sh` that lost its executable bit, a filename Windows cannot check out (reserved names, illegal characters, trailing dots), a hardcoded drive letter or `/home/` path outside a platform branch, a Windows-only API reached without a platform branch, and a `.claude/test.json` command that only runs on one OS. That Windows-only-API check was added *because* the guard had missed something real — the two dead restart paths above — and it earns its place the same way: run against the code as it stood before the fix it flags both, and against the fixed tree the whole check passes. It matches the constant names bare rather than as `subprocess.X`, because the real defects were written through an aliased import (`_sp.DETACHED_PROCESS`) and the first, prefix-anchored version of the pattern walked straight past them. It also understands that a platform branch is not the only valid guard: an optional import wrapped in `try: … except ImportError:` is how `bot/services/outlook.py` correctly reaches pywin32, so that is recognised from the syntax tree rather than silenced with an escape hatch. That last check walks *every* command in the file and looks at the program being invoked, not just the three top-level keys and not just the file extension — the first cut did neither, which is exactly why `tail -n 50` sat unnoticed in the log-reading command. The path check parses each file rather than grepping it, so prose that merely *documents* a Windows path is left alone — but precisely: an earlier cut skipped every line holding any string literal at all, and since a hardcoded path is always a string literal, that silently disabled the entire check, which could not fire on anything. Comments now come from the tokenizer and docstrings from the syntax tree, a `C:\ProgramData` default inside a `sys.platform == "win32"` branch is still exempt, and `# portability: ok` is the escape hatch for the few files whose job genuinely *is* old Windows paths (the migration tool, two synthetic test fixtures). It also no longer skips all of `scripts/` — that blanket had been added to silence this file's own search needles and was quietly excusing every harness the verify step runs. Line endings are read with one `git ls-files --eol` call rather than a `git show` per file, which is the difference between 0.3s and ~150 subprocesses — and no extension is exempt from that check any more. The earlier exemption for `.bat`/`.cmd`/`.ps1`/`.vbs` was a misreading of git: `eol=crlf` is a *working tree* instruction, and those files are stored LF in the index like everything else, so the list never once fired and was pure hole. Confirmed by planting a CRLF batch file, which the check now catches and previously waved through. Registered as a harness so the verify step runs it.
- **Linux migration kit.** The daily driver is moving from Windows 10 to Fedora KDE on a new 4 TB NVMe, and the bot has to be back up the same day — it *is* the way the machine gets talked to. An audit of every platform-specific branch found the code already portable: each Windows path has a working POSIX counterpart (process-tree kill via `os.killpg` vs `taskkill`, a login terminal that falls back through the common X/Wayland emulators, the `$DISPLAY`/`$WAYLAND_DISPLAY` console probe, the `/etc/claude-code` managed-settings path), every dependency is pure-Python, and the one Windows-only package is already gated behind a `sys_platform` marker. What does not survive the move is *configuration*: three absolute paths in `.env`, all 13 registered repo paths in `data/state.json` plus the `repo_path`/`worktree_path`/`session_account` fields across 254 instance records, and two build worktrees registered against Windows gitdirs. `scripts/migrate_to_linux.py` rewrites all of it — dry-run by default so it can be previewed from Windows before the switch, `*.windows.bak` copies before any write, and absolute paths outside the old home are *reported rather than guessed at*. Also `start.sh` (the `start.bat` counterpart, preferring `data/bot.pid` over a blind `pkill`) and `scripts/claude-bot.service`, a systemd **user** unit — a system unit would read the wrong `$HOME` and every Claude session would come back signed out. Full runbook in `docs/linux-migration.md`, including the one step that is easy to miss: without `loginctl enable-linger` a user service stops at logout and does not return until the next login, i.e. the bot is down precisely when nobody is at the machine. Only casualty of the move is the optional Outlook integration, which needs `pywin32`.
...
### Fixed
- **Bot-command directives now collapse to one line in ordinary chat replies too, not just background runs.** The one-line collapse shipped in v0.99.11 was wired into the background/lifecycle path only, so a directive emitted in a normal thread reply was displayed raw — the bracketed command line, and for `/spawn` or `/chain` its entire multi-KB body, printed above the answer you were actually reading. The chat path now applies the same display transform; the dispatchers still read the raw text, so nothing that was being acted on stops being acted on.

## v0.99.12 — Multi-account auth resilience (2026-07-28)

### Fixed
- **A signed-out Claude account can no longer hard-fail a task.** When the main account hit its usage limit, the bot failed over to the backup, the backup returned `Failed to authenticate. API Error: 401 OAuth access token has expired.`, and that raw string became the headline of a failed build with no retry scheduled (t-6570, 19.5 minutes of work lost). Root cause, in two parts. First, the backup had been dead for five weeks: its `.credentials.json` existed but carried **no refresh token** and an access token that expired 2026-06-19, and startup validation only checked that the file *existed*, so it stayed in rotation. Second — and this is the part worth remembering — the guard that was supposed to notice "the failover target did no real work, so carry the original reset time forward" tested whether the result text was empty, but **the CLI writes its fatal 401 into the result text**, so the bot concluded the backup had done real work and surfaced its error verbatim. That test is now `_no_productive_work()`, which treats a turn as unproductive when it produced no output *or* its output is itself an auth error, and it is used at both failover sites. Three further layers so this can't dead-end again: an account with no usable refresh token is skipped before it costs a spawn (self-healing — a `/login` rewrites the file, which busts the cached probe on the next pick, no restart and no `.env` edit); a confident auth failure with nowhere left to go now schedules the turn to come back when another account's cooldown expires instead of failing; and if there is genuinely nothing left, the message names the account and gives the exact re-auth command instead of echoing the CLI. The credential probe is a heuristic, so it is ignored entirely when it would exclude *every* configured account — a clean sweep is what a wrong heuristic looks like (credentials stored outside the config dir), and it must never take the fleet dark. New shared module `bot/claude/auth_health.py`.
- **A "waiting for your account to reset" countdown is now only ever scheduled against an account that can actually take the work.** Cooldowns are set for two opposite reasons — a usage limit, which ends on the clock, and a failed login, which ends when someone signs in — and the runner could not tell them apart. So with *both* accounts signed out it would announce a 24-hour wait for a reset nobody had hit, burn all three retry attempts, and fail with the same 401. Auth sidelines are now tracked apart from usage ones and excluded when picking a retry moment, in both places that choose one. Where there is nothing real to wait for, the two paths differ on purpose: the nowhere-left-to-fail-over path ends the turn immediately with a message naming every signed-out account, while the refuse-to-spawn path comes back after a short grace window (15 minutes, three attempts, so ~45 minutes total) in case someone signs an account back in — and the countdown says exactly that rather than inventing a reset nobody hit. The same distinction makes the notice's promise true in the other direction: signing an account back in clears its auth cooldown on the spot, so it rejoins rotation immediately instead of sitting out the rest of the day, while a genuine usage cooldown survives untouched.
- **Signing back in also revives an account the *server* rejected.** The recovery above worked off the credentials file: a `/login` rewrites it, the probe re-reads it, the account returns. But the failure this whole feature exists for is the other kind — a token that is present and parses fine, and that the server refuses at runtime. Nothing on disk changes when that happens, so re-reading the file proves nothing, and the account sat out its full 24-hour sideline even after being signed back in. The bot now records *which* credentials file a sideline was judged against (modification time and size) whenever it opens one, whatever the reason, and treats that file being rewritten as the proof a login happened — the one durable signal available. Recording it for every reason rather than just the server-rejected one keeps the recovery rule single: an account whose sideline reason changed while it was out (wiping the config directory before signing back in does exactly that) recovers by the same path instead of being stranded by a reason mismatch. Because it is persisted, this survives a reboot, unlike the in-memory record of which cooldowns were auth-kind — and it has to, because after a restart *nothing else* can tell an auth sideline from a usage limit, so without it a reboot mid-outage would bench a signed-in account for the rest of the day.
- **`/status`, the dashboard and the `/auth` panel no longer disagree with The Ark about a dead account.** All three answered "is this account healthy?" from the credentials file alone, so an account the server had rejected — the exact failure being announced in The Ark — still read as `2/2 usable`, `· 2 accts`, and a green tick. The `/auth` panel mattered most: it is where the outage notice's own button lands, so the user was sent from "klerk is signed out" straight to a screen showing klerk signed in, offering **Re-login** where they needed **Log in**. All three now consult the same sideline table the notice writes — `/status` and the dashboard through one shared partition helper, the panel by being handed the set — so the four surfaces cannot drift apart. They also *name* accounts the same way now: the panel labelled each row with the raw directory name (`.claude-klerk`) while the notice, `/status`, the dashboard and the login-terminal confirmation all use the short form (`klerk`), so the user arriving from "klerk is signed out" landed on a list where no row was called klerk — on the one screen where they are least sure what they are looking at. One shared label function for all of them.
- **Startup now re-checks the accounts instead of trusting the table it saved.** Boot seeded alert records for accounts that were signed out *at boot* and nothing else, so both stale directions survived a restart. An account signed back in while the bot was down kept its sideline and its 24-hour cooldown — The Ark went on showing an outage that was over, and the account stayed benched until some task happened to try it. An account that went out while we were down was recorded but never reconciled against a cooldown that no longer applied. The seeding also carried its own copy of the fingerprint-recording rule, one edit away from drifting from the runner's. Both are now one call to `runner.reconcile_account_health()` right after the runner is built — a thin wrapper over the same probe the per-spawn path uses, which already knows how to open, retire and resolve these records. Failures there are logged and don't stop startup: an account-health check must never be the reason the bot won't boot.
- **The outage notice no longer reads like a log line, or names a survivor that is itself down.** The reason strings are written for the log, where a filename or an HTTP status is the useful detail, and were being dropped straight into a sentence — a server-rejected account announced itself as "**`klerk`** is signed out — logged out — the CLI rejected its OAuth token (401)": a stutter, and an HTTP status aimed at someone reading on their phone. Each reason now has a plain-English cause ("the login expired and Claude turned it away"), with the raw string as fallback. Separately, the "work keeps running on `<other>`" line listed every *other* configured account without checking whether those were signed out too, so in a both-accounts outage each of the two notices calmly pointed at the other as the survivor — and both still closed with "nothing needs your attention right now" while nothing could run at all. The notice is now handed the full sidelined set, says plainly that everything is down when it is, and swaps the reassurance for what actually happens (tasks auto-retry a few times, then give up). An alert for an account since removed from `CLAUDE_ACCOUNTS` is dropped rather than announced — the runner never looks at unconfigured accounts, so its record could otherwise never be resolved.
- **A task finishing successfully no longer deletes a usage limit a parallel task just hit.** A completed run is the only proof a server-rejected account can authenticate again, so the success path clears its sideline — but it was clearing it by force, which drops the cooldown whatever kind it is. This bot runs many tasks at once on the same account: task B hits the 5h limit and cools the account down while task A is still streaming, A then finishes fine and deletes B's genuine limit, and the next spawn walks straight back into it. The reason given for forcing — that a reboot loses the record of which cooldowns were auth-kind — doesn't apply here, because the picker never hands out a cooled account in the first place, so a run reaching this point can only be carrying a cooldown that arrived mid-flight, i.e. exactly the one to keep. A success now says only what it proves: this account can log in. Recovery from a 401 sideline is unchanged.
- **The "credential check would exclude everything" warning no longer repeats forever.** The safety valve that ignores a clean-sweep probe result logged a warning on every single spawn while it was engaged — and the case it exists for (a host that keeps credentials outside the config directory) never resolves, so that is one identical WARNING per task for the life of the bot, in the log you go to when something is actually wrong. It now logs on the transition only, matching the discipline of the probe next to it, and logs once more when the check starts discriminating again.
- **A session that merely writes about 401s is no longer mistaken for an account that died.** Every place that asked "was this an auth failure?" matched any output containing an auth phrase — and this repo's own sessions discuss OAuth and expired tokens constantly. The consequential one was the decision that sidelines an account for 24 hours: a single session writing *about* expired tokens could take a healthy account out of rotation for a day. The same loose test also let a real answer be swallowed by the "did the backup do any work?" guard and replaced with a retry countdown, and rewritten into a re-auth instruction by the failure-message formatter. All four sites now use `parser.looks_like_fatal_auth_error`, which additionally requires the output to be short: a CLI fatal error is one line (the t-6570 one was 99 bytes), anything longer is a work product.

### Added
- **Discord tells you when an account is sidelined, once.** A dead backup was invisible until the day failover needed it. The Ark now gets a single orange notice per outage — which account, what it costs you (work continues on the accounts still up and you've lost the safety net — or, if nothing is left, that nothing can run), and the re-auth command in a copy-pasteable block — with **Auth panel**, **Log in `<account>` on this PC** (hidden on hosts that can't open a terminal) and **Ignore for 7d**, which means later rather than never: the notice goes quiet, comes back once when the week is up, and can be muted again. One line back when it recovers: `✅ Account 'klerk' is back in rotation`. The notice never expires, so its account buttons carry the account's name alongside its position and refuse to act if an `.env` edit has since moved it — no silently logging into the wrong account. State lives in `data/state.json` under `account_alerts`; the runner only opens and resolves records, the notifier owns the posting, so a burst of failed tasks can't turn into a burst of messages. A runtime 401 is never cleared by re-reading the *same* credentials file — the file the server rejected still parses fine, so re-reading it would announce recovery and fail on the very next task. Only two things count as the all-clear: a successful run, or that file being rewritten, which is what a `/login` does.
- **`/auth`** — opens the account panel (who's signed in, which email, cooldowns, per-account re-login) from anywhere, instead of hunting for the Ark button.
- **Account health is visible at a glance.** `/status` gains an `**Accounts** — 1/2 usable | signed out: klerk` line, and the dashboard's usage label reads `· 1/2 accts` when one is down instead of the flat `· 2 accts` that made a dead rotation look healthy. Both count an account the server rejected as down, not just one whose credentials file is visibly empty.
- **Waiting is explained honestly.** A rescheduled turn now says which of three things it is actually waiting for, instead of blaming a usage limit nobody hit: the backup being logged out while the main account cools down ("Backup account logged out — waiting for your main account to reset, auto-retrying at 4:00 PM"), every account being signed out ("retrying at 4:00 PM in case one gets signed back in" — no mention of a reset, because there isn't one), or simply nothing being free this turn. The first two carry an **Auth panel** button; the third doesn't, because there is nothing there to fix. The wording lives in one table keyed by reason, so a fourth case can't quietly reintroduce the old lie.
- **`scripts/test_account_failover.py`** grew the t-6570 replay (limit + 401-in-result-text → retry scheduled, not a failure), the doomed-spawn skip, the all-accounts-fail safety valve, the actionable dead-end message, the login-busts-the-cache check, the both-accounts-dead case (no fabricated wait, both accounts named), a one-turn answer *about* 401s that must not be read as a dead account, the re-login that clears an auth cooldown but leaves a usage cooldown alone, a successful run that must retire a 401 sideline without deleting the usage limit a parallel task set on the same account mid-flight, a server-rejected account that stays sidelined when its credentials file is merely re-read and returns the moment it is rewritten, the same recovery across a bot restart (the case the in-memory bookkeeping cannot cover, and the one the single-process test silently passed without), a startup that reconciles both stale directions at once (an account signed back in while the bot was down is un-benched and gets its all-clear; one that went out is recorded, unannounced, with a fingerprint so a later `/login` can retire it), and the three retry countdowns — each asserted to name the right wait, with the all-accounts-signed-out one checked for the absence of the word "reset". New **`scripts/test_account_alerts.py`** locks the notice: exactly one embed per outage, exactly one all-clear, a snooze that goes quiet and then lapses exactly once, buttons that carry the account label and aren't built for an account that has since left `CLAUDE_ACCOUNTS`, the `/auth` panel reporting a server-rejected account as signed out rather than contradicting the notice that links to it — and naming it the same way, checked with realistically-named `.claude-*` directories, because with plain test names the panel's labelling and everything else's happen to coincide and the disagreement is invisible — the credential fingerprint being refreshed on every re-marked sideline — a stale one would flap the account in and out of rotation on a loop, one wasted spawn and one Ark notice per cycle — the notice copy staying honest (no raw reason string or HTTP status in the prose, no promising work continues on an account that is itself signed out, no "nothing needs your attention" when nothing can run), and an alert for an account dropped from `CLAUDE_ACCOUNTS` being pruned rather than announced, while an unset `CLAUDE_ACCOUNTS` leaves live alerts alone.
...
## v0.99.10 — Context engineering: cache metrics, ranked history, /evals (2026-07-27)

### Added
- **Prompt-cache hit rate is now measured.** Every session eval records how much of its prompt was served from cache (`cache_hit_rate`) alongside the raw cache-read/cache-write counts, and a resumed session that reuses less than half its prefix gets flagged. Fresh spawns are never flagged — they have no prefix to hit. Nor are runs whose cache counters never arrived: both counters at zero means the numbers are missing, not that nothing was cached (3 of 253 persisted instances sit in that state, each having recorded 69k-167k of context), so the rate reports "unknown" rather than a fabricated 0% that would trip the flag with a finding the raw records don't support. A genuine total miss — a resume past the cache TTL, which rewrites the whole prefix — still has a positive cache-write count, so it reports 0% and does flag. `Instance.resumed_session` is stamped at command-build time because the CLI always returns a session id, making a resume indistinguishable from a fresh spawn after the fact. The `/usage` token line gains a `N% cached` suffix (`/cost` reports dollars only and is unchanged).
- **AI-project review lens.** Repos whose root manifests declare a model-provider SDK (anthropic and the Claude agent/code SDKs, openai, langchain, llamaindex, gemini, mistral, cohere, ollama, litellm, ai-sdk) get an extra block appended to the code-review prompt, covering eval coverage, output validation vs prose-scraping, derived-summary honesty, failure taxonomy, cache-shaped prompt assembly, tool-surface scoping and model-id pinning. Detection reads root manifests only and memoises per repo; non-LLM repos are completely unaffected. Matching is case-insensitive, and a marker may not be followed by a lowercase letter — so `coherence` is not read as the Cohere SDK, while camel-cased package names like `OllamaSharp` and `Azure.AI.OpenAI` still match. `-`, `_` and `.` are treated as interchangeable, per PEP 503, so a marker written one way also matches the other spellings (`llama-index` / `llama_index`) instead of needing a hand-maintained variant list. Known blind spot: a repo that drives a model by shelling out to a CLI (this bot itself) declares no SDK and is not detected. New module `bot/engine/ai_project.py`.
- **`/evals` command.** A diagnostic view over the session evals in `data/evals/`: recurring flags across a window (default 7 days, minimum 3 sessions), each attributed to the system-prompt block or harness area responsible for it, plus median prompt-cache reuse across resumed sessions. Complements the existing `/report`, which is the volume/cost/merge-rate scorecard over a short window — `/evals` answers "which part of the prompt keeps failing", not "how much did this week cost". Flags are counted once per session, not once per occurrence — per-command checks fire on every tool call, so raw occurrence counts let one long session outrank a habit spread across fifty; total hits are still shown alongside when they differ. Reports how many one-off flags the threshold held back.

### Changed
- **`/report`'s flag list now uses the same counting as `/evals`.** It had its own counter that grouped on the raw message, so `took 22 turns` and `took 31 turns` occupied two of its five slots as separate findings, and it counted raw occurrences — a single session that used Bash for file reading 200 times outranked a habit seen across fifty sessions. Both commands now share one grouping helper and count sessions, so they can no longer quote different numbers for the same flag. Flags are shown with their owning prompt block. Chain-level flags keep their own tally (they aren't session evals) but are normalised and attributed the same way — three of the attribution rules describe messages only chains produce (plan revision rounds, review loops, chain cost), so leaving chain flags unattributed left those rules permanently dead. `/report` also picked up the window clamp and off-thread hop `/evals` already had — it reads the same eval directory, and an unbounded `days` argument scanned all of it synchronously on the event loop, stalling every other session's progress updates.
- **Session history injected into the system prompt is now relevance-ranked, not just recent.** Candidates are still loaded newest-first, then narrowed to `SESSION_HISTORY_MAX` (default 6) by keyword overlap with the current prompt plus a decaying recency bonus. Entries on the session's own branch and its parent/stacked-build lineage are pinned regardless of score, as are up to two failures from the last 24h — capped, because one account cooldown fails every running session at once and would otherwise fill the whole block with the same infrastructure error. `SESSION_HISTORY_RANKING=recency` turns ranking off and selects newest-first as before — a no-deploy revert switch for the ranking, not for the size cap, which still applies in both modes (the old block injected up to 20 entries; raise `SESSION_HISTORY_MAX` to get that back). An unrecognised value warns at startup and falls back to relevance rather than silently disabling it.
- Session-history block no longer truncates mid-word at a fixed byte count. It now drops whole entries from the end under `SESSION_HISTORY_MAX_CHARS` (default 6000). The old 4000-char cap cited a Windows command-line limit that no longer applies — the system prompt is passed via `--append-system-prompt-file`, not argv. Per-entry topic and summary caps also cut on a word boundary now and flatten embedded newlines, instead of leaving the model a severed fragment like `**What I`. The two helpers that do this (`clip`, `flatten`) live in a new leaf module `bot/textutil.py` that imports nothing from `bot`: they are needed from `bot.store` and `bot.engine`, and the packages that would otherwise host them sit downstream of both (`bot.platform` imports `bot.claude`, which imports `bot.store`), so housing them in either would have closed a cycle.

### Fixed
- **A malformed line in `data/history.jsonl` could stop a session from starting.** All three history readers parsed each line and then called `.get` on the result, so a line that was valid JSON but not an object (`[1,2,3]`, a bare string) raised `AttributeError` — out of `clear_branch`, whose docstring promises it never raises, and out of the system-prompt builder, where it killed the spawn outright. Parsing is now one shared helper that treats "parsed but not an object" the same as "didn't parse", and `clear_branch` still rewrites such lines back verbatim rather than dropping them. Rendering the history block moved into `bot/store/history.py` (every input and rule it applies is about history; the runner only needed the finished string) and is best-effort like the rest of that module — history is context, not correctness, and no ranking edge case should be able to stop a session. Verified byte-identical to the previous inline rendering across four real repo/prompt combinations.
...
### Added
- **Post-Fable-PPU model policy config** (`DEFAULT_SESSION_MODEL`, `MODEL_ROUTING` in `bot/config.py`; resolution in `provider.build_command`, `workflows.spawn_from`, `runner._is_primary_model`). Fable 5 leaves subscription pricing July 8 and goes metered; approved policy: default new sessions to Opus, spend Fable pay-per-use only on plan/review steps. `MODEL_ROUTING=plan:fable,review_code:fable` routes per workflow-step origin at spawn time (malformed/unknown entries warn and skip); `DEFAULT_SESSION_MODEL` is the last-resort `--model` at command-build time so DIRECT sessions get it too. Resolution order: explicit instance model → `MODEL_ROUTING[origin]` → legacy `EXPLORE_MODEL` (review_plan/apply_revisions, kept for compat) → `DEFAULT_SESSION_MODEL` → none (account CLI default). The model-limit failover downgrade (`model_override`) beats everything, so the default can never undo a downgrade, and `_is_primary_model` now resolves `None` through `DEFAULT_SESSION_MODEL` so an Opus-defaulted run is never misread as Fable by the limit machinery. Code-only — both vars ship unset (current behavior unchanged); recommended July-8 values documented in `.env.example`.
- **`.claude/skills/` is now tracked in git** (`.gitignore`: `.claude/` → `.claude/*` + `!.claude/skills/`). The old directory-form ignore blocked ALL of `.claude/`, and git cannot re-include children of an excluded directory — so a wave-1 build's finished skill was silently dropped from its worktree merge (merges only carry tracked files). Local state (`settings.local.json`, `deploy.json`) stays ignored via the glob form; the four existing skills (review-plan, review-code, prep-plan-for-execution, final-step-before-plan-execution) are now committed.
- **`.claude/skills/debugging-playbook/SKILL.md`** — distilled operating judgment for this repo, written while a top-tier model can still write it: reboot_request.json flow (never taskkill — you ARE the process), mandatory pre-reboot preflight and post-reboot verification commands, the worktree merge/gitignore trap (citing this very fix), git-admin lock + test-suite mutex behavior, state.json shape gotchas (`instances` is a list), Discord hard limits, and the log-first diagnosis order.

### Fixed
- **`scripts/test_chain_botcmd.py` no longer consumes live reboot requests** — it drives the real `run_instance` turn-end path, whose reboot check reads `data/reboot_request.json` at call time; a test run while a real reboot was pending read it, DELETED it, then crashed on the fake runner (missing `request_reboot`), silently cancelling the queued v0.97.0 apply-reboot. The test now redirects `config.REBOOT_REQUEST_FILE`/`REBOOT_REQUEST_DEFERRED_FILE` to a throwaway temp dir before running.

## v0.97.0 — Per-repo test-suite mutex for parallel sessions (2026-07-07)
### Added
- **Per-repo test-suite mutex across parallel sessions** (`bot/claude/hooks/test_mutex.py`, installed by `_install_worktree_hook`, `TEST_MUTEX_ENABLED` in `config.py`). Worktrees isolate files but not the machine: five parallel build sessions on one repo ran their full test suites simultaneously and starved each other over shared ports, databases, and CPU — orphaned/hung runs, restarted suites, context burned on retries (t-5976 incident). Full test-suite commands (`dotnet test`, `pytest`, `npm test`, `go test`, `cargo test`, gradle/maven/vitest/jest/playwright/ctest, …) now serialize per repo via a lock at `{repo}/.worktrees/.test-mutex/`: a PreToolUse hook acquires it (waiting up to 45s for short overlaps to resolve invisibly — under the 60s stall-warning threshold so a waiting session never shows "quiet — /kill if stuck" — then blocking with a do-other-work-and-retry reason), a PostToolUse hook releases it. Narrow/filtered runs that don't match the patterns stay fully parallel. Stale locks recover three ways: 30-min TTL, holder's worktree deleted (merge/discard), and a runner post-run backstop that frees the lock the moment a killed/crashed holder's CLI process exits. Per-repo control via `{repo}/.claude/parallel.json`: `{"test_mutex": false}` opts out, `test_patterns`/`extra_test_patterns` replace/extend the defaults (read from the main repo root so a build branch can't rewrite its own rules). The hook fails open on any internal error — a mutex bug can never wedge builds.
- `scripts/test_mutex_drill.py` — 26-check drill: acquire/re-enter/block-with-reason, mid-wait handoff, holder-only release, stale-TTL and dead-holder steals, per-repo opt-out, custom patterns, runner backstop.

### Changed
- **Sibling-session prompt now warns about shared-machine collisions** (`runner.py` sibling block): parallel sessions are told to prefer narrow test filters and that full-suite runs serialize per repo, so a blocked test command reads as "the lock is busy," not as a mystery failure.

## v0.96.0 — Fleet ship: batch merge, deploy, verify-back (2026-07-06)

### Added
- **Fleet ship** (`/fleet`, owner-only; `bot/discord/fleet.py`): one command ships every session at once — scan all forum threads for committed-but-unmerged work (completed instance with a live branch/worktree AND commits ahead of its merge base), show a per-repo roster with Confirm/Cancel, then merge → deploy → verify-back. Sessions with persisted autopilot-chain state are excluded from the roster (a paused chain owns its thread's merge; shipping under it would race), and a successful fleet merge clears any stale failed-merge record so the verify prompt isn't wrapped in the "[auto-merge still unresolved]" system note from an earlier attempt. Merges reuse `_finalize_merge` with a new `skip_close=True` so threads keep the merged tag + ✅ message but stay OPEN — the post-deploy verify prompt lands in a live thread instead of archive→reopen churn; merge failures get the standard banner/buttons + `set_pending_merge`, and a repo with ANY failed merge is never deployed (per-repo gate). Deploys branch by method: command repos run `execute_deploy` inline (verify-back gated on deploy success, plus the post-deploy healthcheck only when one is configured — `_post_deploy_healthcheck` now returns pass/fail); the bot's own repo requests a coalesced reboot and deploys LAST so it can't kill sibling deploys mid-flight; when that reboot defers behind active work the origin channel is told, including the manual fallback (deferred requests are dropped as stale after ~1 h — the verify set survives and drains at whatever boot comes next). Verify prompts replay into each shipped thread via `_replay_to_thread(source="fleet_verify")`; a clean verify turn closes the thread silently, a failed one leaves it open and flags the origin channel. Auto-close requires a NEW completed instance: the newest run is snapshotted immediately before the replay (shipped instance id, persisted per entry, as fallback baseline) and must have changed after it — so a replay that dispatched but never ran a turn (drain gate, usage limit, spawn refusal) can't false-positive close the thread, even if unrelated turns ran between ship and verify; a verify turn that ends on a question (`needs_input`) also stays open.
- **Reboot-safe verify-back**: shipped threads persist in `platform_state["discord"]["fleet_pending_verify"]` (written BEFORE the deploy), and `bot/app.py` drains leftovers at startup — so a self-managed deploy rebooting the process mid-pipeline, or a crash between deploy and verify, can't lose the verify fan-out.
- `scripts/test_fleet_ship.py` — contract tests: pending-verify add/pop round-trip (incl. shipped instance_id carried for the close guard) + dedup + multi-repo drain source, `_finalize_merge` `skip_close` signature (keyword-only, default False), empty healthcheck returns True, verify-prompt wording per deploy method, `_commits_ahead` returns 0 on garbage (no-op branches never ship).
- `scripts/verify_fleet_sim.py` — end-to-end pipeline simulation driving the real orchestration code over faked merge/deploy/replay seams: happy-path command deploy (persist-before-deploy ordering, prompts, closes), merge failure gating the repo, deploy failure skipping verify, self-deploy persisting the set for boot drain, and the no-op-replay close guard.

## v0.95.0 — Fable 5 model-limit failover (2026-07-06)

### Added
- **Fable 5 model-limit failover** (`parse_model_limit` in `parser.py`, model-limit branch in `runner.py`, `PRIMARY_MODEL`/`MODEL_FALLBACK` in `config.py`, `model_cooldowns` in `state.json`). The CLI's "You've reached your Fable 5 limit. …switch models with /model." message was previously unrecognized — the run just failed raw (t-5937). Now it's detected as a MODEL-specific limit, distinct from the account-wide caps: the account is never sidelined. Flow: record a per-account model cooldown → retry on the other account's Fable quota (session JSONL hydrates across) → when every account is Fable-limited, rerun on `MODEL_FALLBACK` (default `opus`) with a thread warning ("Continuing on opus until ~HH:MM — switching back automatically"). If the hop target dies before doing any work (auth-dead backup, or its own account-wide cap), the run also falls back to the downgrade instead of surfacing a raw error or stalling until reset. While a cooldown is active, new runs route to a Fable-free account when one exists, otherwise spawn directly with `--model opus` (no doomed attempt); elapsed cooldowns purge automatically, so the switch back to Fable needs no manual action. A model name in the limit wording beats generic cap words ("Fable 5 usage limit" is still a model limit). Explicit non-primary overrides (`EXPLORE_MODEL`) are exempt. Subscription-only — the path never touches pay-per-use. Cooldowns persist across reboots like account cooldowns.
- `scripts/test_model_limit_failover.py` — parser matrix (real Fable message, timed variant, model-name-beats-generic-words, account-wide caps rejected, ordering contract with `parse_usage_limit`) + runner integration (hop to backup on same model, both-limited downgrade to fallback, dead-backup downgrade, single-account downgrade, pre-emptive downgrade after reboot, routing to the model-free account, expired-cooldown switch-back, explicit-model exemption).

## v0.94.1 — Multi-spawn fan-out and chain-turn directive dispatch (2026-07-06)
...
### Fixed
- **Heuristic wake scheduling fully removed — claim check downgraded to notice-only** (`check_wake_request`, `claims_self_wake`, `_CLAIM_META_RE`). Minutes after the watch-promise heuristic was deleted (below), the surviving claim auto-arm misfired the same way: a verification report *quoting* the trigger phrase "self-wake queued/scheduled" armed a phantom 3-min re-check (log 17:00:36, `claim w/o request`). Meta-discussion of the feature is routine in this repo's threads, so prose scanning now schedules nothing, ever: an explicit parsed `[BOT_CMD: /wake]` directive is the sole arming channel. A turn that asserts it armed a self-wake while nothing parsed now gets a notice-only heads-up ("no valid directive found — nothing is scheduled"), and `claims_self_wake` strips code spans and quoted phrases first so quoting a trigger phrase can't even trip the notice.
...
### Changed
- **Self-wake is now scheduled by a `[BOT_CMD: /wake]` output directive instead of a wake file** (`config.WAKE_GUIDANCE`, `lifecycle._parse_wake_directive` + `check_wake_request`, `runner.py`). Root-cause fix for the class of bug the v0.93.10 regex guards only patched: scheduling a self-wake required the model to *remember* to write `data/wakes/<id>.json` via a separate tool call, which it sometimes narrated ("Self-wake queued…") without doing, leaving the thread silently stalled. The wake is now a directive in the turn's output — `[BOT_CMD: /wake delay=300 reason="…"]` + a `~~~wake … ~~~` prompt body — parsed post-turn by the same proven directive *mechanism* as `/spawn` (a separate parser in `lifecycle`, mirroring its kv + quoted-line-skip approach). The directive *is* the action (text in the response), so "claimed" and "done" can't diverge. `_parse_wake_directive` skips fenced/quoted/inline-backtick mentions so meta-discussion of the feature can't self-trigger a wake (the loop we just hit). The legacy wake-file path is still honored as a fallback for sessions on cached older guidance; the `WAKE_PROMISE_RE`/`claims_self_wake` + `ctx.source == "wake"` guards from v0.93.10 remain as defense-in-depth.
...
## v0.93.6 — Self-wake survives a promised-but-unscheduled watch (2026-06-18)
...
- **Bounded and conservative.** The fallback counts toward `MAX_CONSEC_WAKES` (25) so it can't loop forever — and when the cap is hit it emits the same "⏸ Stopped auto-checking" notice as a real wake (shared `_stop_capped` helper) rather than stalling silently. The detection regex (`WAKE_PROMISE_RE`, scanned after verify blocks are stripped so a `verify-board` item can't false-trigger it) requires a watch/poll *verb* near a machine/job *noun*, so human-directed waits ("I'll let you know", "wait for your reply", "continue once you confirm") don't trip it. A worktree build can't self-wake (its dir may be merged/discarded), so a promise there gets a "can't self-wake — reply or tap a button" notice instead of an armed wake. The happy path — a session that *does* write a wake file — is untouched. Wired on both `check_wake_request` call sites (interactive `on_text` and the `run_instance` path), so it also covers self-wake resumes. New `scripts/test_wake_promise.py` locks the detection regex and verify-block stripping against regression.
...
- **CHANGELOG union merge driver lands in the real gitdir.** The same wrong assumption wrote `CHANGELOG.md merge=union` to a junk `<registered>/.git/info/attributes` that git never reads (found literally on disk in the AIAgent subdir) — so every parallel build conflicted on CHANGELOG for no reason. Now written to the repo's common gitdir (shared across worktrees) via `_ensure_union_merge_driver`; skipped cleanly when unresolvable instead of mkdir-ing a fake `.git`, and read/written as explicit UTF-8 so a stray byte in an existing attributes file can't abort a merge.
- **Same fix applied to `MERGE_HEAD` poisoned-merge detection** (precheck + finally cleanup), **worktree-metadata recovery scans** (runner + workflows, stat-only `git_dir_stat` on the no-subprocess hot path), and **`/repo create`'s already-a-repo check** (rev-parse instead of `.git` stat, so a subdir of an existing repo registers instead of getting a nested `git init`).
- **Pre-merge origin sync + divergence guard.** `merge_branch` now runs `git fetch origin --tags --prune` (30s timeout, offline-tolerant) after checkout: behind-only fast-forwards the target to origin first; diverged aborts *before* merging with counts and concrete next steps (restoring the stash and rendering a dedicated `MERGE_FAIL_DIVERGED` banner). Resolve with Claude short-circuits for diverged failures — the resolver fixes worktree conflict markers, not local-vs-origin divergence, so it would have been a guaranteed no-op round-trip. Prevents the observed failure where another machine's releases (v0.93.0/v0.93.1) sat on origin while local master stayed on v0.92.60 — the next local release would have minted v0.92.61 below the remote and pushed nothing. Fetch failures surface a note in the merge result, not just the log. Startup stale-instance auto-merge inherits the guard (routes through `merge_branch`).
- **Stale-version release warning.** Build sessions mint versions from worktree-local tags, so a stale clone can produce a release commit at/below an existing tag — `_tag_release_at`'s no-clobber logic then silently skips tagging (exactly how v0.92.53 went untagged). The merge result now warns when the release commit wasn't tagged (name collision) or when the created tag is below the highest existing `v*` tag.
- **`scripts/merge_drill.py` regression drill.** Builds a throwaway subdirectory-registered repo, manufactures a two-sided conflict (code file + CHANGELOG), runs the production union-driver setup and `_auto_resolve_merge_conflicts`, and asserts the CHANGELOG keeps both sides while the code conflict resolves and commits. Run after any merge-path change.
- One-time repairs done alongside: created+pushed missing tags `v0.92.53`/`v0.93.0`/`v0.93.1`, pruned 12 merged-but-undeleted branches/worktrees (plus a stray dir), deleted the junk `.git` in the AIAgent subdir, and converted `AIAgent.csproj` from UTF-16 to UTF-8 (git saw it as binary — the source of the unresolvable csproj conflicts).
...
- **A session waiting on a long external job (backtest, deploy, CI) can now schedule a real wake-up instead of claiming it'll "poll in the background" and never returning.** Symptom (thread 1511317946423578807): the bot kicked off a backtest, said it would report back, then went idle — the user had to keep typing "update?" to drag each step forward. Cause: an inner Claude session is a one-shot CLI subprocess — it runs a single turn and exits, with no primitive to re-invoke itself, yet nothing in the system prompt told the model that, so it freely promised background polling it couldn't deliver. Fix has two parts:
  - **Honesty:** `BOT_CONTEXT` now states turns don't continue and bans "I'll poll in the background" phrasing; a per-session `WAKE_GUIDANCE` block (non-worktree origins only) tells the model how to self-wake.
  - **Mechanism:** the session writes `data/wakes/<instance_id>.json` (`{delay_secs|wake_at, prompt, reason}`, delay clamped 30s–24h). At turn-end `lifecycle.check_wake_request` converts it into a one-shot, thread-bound `Schedule` (`resume_thread=True` + `channel_id`); the 30s `Scheduler` tick fires it through the proven `_replay_to_thread()` path, re-invoking that exact session in that exact thread with the follow-up prompt. To poll, the model just writes a fresh wake file each round until done.
  - **Guards:** per-instance file path (absolute, so it works from worktree cwd; per-id, so 10+ parallel sessions never clobber a shared file); a `MAX_CONSEC_WAKES` (25) runaway cap on `ThreadInfo.wake_count` that resets whenever a turn ends without requesting a wake; worktree/build origins are refused (their dir may be merged/discarded before the wake fires); at most one pending wake per thread (a new wake deletes/supersedes the prior, so interleaved turns or a busy re-arm can't accumulate multiple pollers); the scheduler drops wakes whose thread is gone/sessionless, re-arms 60s out if the thread is mid-turn **or the bot isn't ready yet at startup** (so an overdue wake firing on the first tick before the forum map loads isn't dropped), and always settles the one-shot in a `finally` — deleting it once consumed (finished/superseded wakes don't pile up in `state.json`) or re-arming if busy, so a throwing resume can't storm-retry every 30s. Wakes are persisted `Schedule`s, so an overdue wake fires on the next tick after a reboot, and they show in `/schedule list` labelled `⏰ wake → <thread>`. Startup sweeps `data/wakes/` of orphaned hand-off files (a turn cancelled mid-finalize before consuming its wake file) so the directory stays bounded across the bot's frequent reboots — the real pending wake already lives in `state.json` as a `Schedule`.
...
- **Phantom-gate @ping headline matches the body for parse-failed verifier output.** When the release verifier's JSON output was unparseable, the fail-closed path correctly told the user in the in-thread body that "verifier output couldn't be parsed — failing closed", but the @mention notification still read "⚠ Release verifier flagged phantom claims. Tap Amend to fix, or Continue to ship anyway." — observed in t-4937 where the verifier wrote prose ("No phantom claims") instead of a fenced JSON code block, the parser fell through, and the user got contradictory copy. `on_verify_release` in `bot/engine/workflows.py` now sets a new ephemeral `_verifier_parse_failed: bool` field on the result Instance (declared in `bot/claude/types.py` alongside the existing `_accounts_tried` precedent — typed, defaults False, deliberately omitted from `to_dict`/`from_dict` because the chain doesn't re-enter verify_release on resume; the user clicks Amend or Continue instead). The chain dispatcher in `_run_autopilot_chain` reads it via direct attribute access (field is always present so no `getattr` fallback is needed) and, when set, overrides the default suffix with "⚠ Release verifier output couldn't be parsed. Tap Amend to retry, or Continue to ship anyway." — parallel grammar to the default phantom_detected copy so the "Continue anyway" button stays correctly named. Real-phantoms case (verifier returned a valid `mismatch` verdict with non-empty `phantom_bullets`) is unchanged. Updated the surrounding "phantom/needs_input keep their generic copy" comment to reflect that phantom_detected now has a parse-failed sub-case.

## v0.92.52 — Build button defaults to Build & Ship (2026-05-20)

- **Resumed chains preserve their originating label + silent_close (t-4788 follow-up).** Ship 1 added `silent_close` and `chain_label` kwargs to `_run_autopilot_chain`, but both resume sites (`resume_autopilot_chain` for the Continue button, and `app._resume_interrupted_chains` for reboot recovery) called `_run_autopilot_chain` directly without forwarding the kwargs — so a paused Build & Ship that the user tapped Continue on would revert to `chain_label="Autopilot"` (mis-labeled failure mentions like "Autopilot blocked at done" instead of "Build & Ship blocked at done") and `silent_close=False` (loud close ping on success even though the entry point opted into silent). Fixed by persisting the kwargs in a new `_chain_kwargs` dict on `StateStore` (loaded/saved alongside `_autopilot_chain_meta`, cleared in `clear_autopilot_chain`). `_launch_chain` writes once at chain start; both resume sites read via `get_chain_kwargs(session_id)` which defaults to `("Autopilot", False)` for legacy chains that pre-date the new field. Round-trip verified via in-process StateStore round-trip (init → set → save → reload → read → clear).

- **Build button now defaults to Build & Ship (t-4788).** The three button rows that exposed a bare `Build` / `Build It` action in `bot/platform/formatting.py` (post-plan, session-has-plan fallback, default session row) now emit `Build & Ship` (action `build_and_ship:`), so every Build tap runs the full ship chain (build → review → commit → merge) without requiring a manual Merge tap afterward. Root cause for the recurring "completed but not merged" worktrees (e.g. t-4736 on aiagent): a plain `/build` finishes correctly but leaves Merge as a separate user action, so unmerged worktrees accumulated whenever the user moved on before tapping. Threaded two new kwargs through chain plumbing in `bot/engine/workflows.py`: `_run_autopilot_chain(silent_close: bool = False, chain_label: str = "Autopilot")` and `_launch_chain(...)` forward both; `on_build_and_ship` passes `silent_close=True, chain_label="Build & Ship"` so a clean success closes the conversation without a redundant ping (the existing `✅ {merge_msg}` line at `workflows.py:1715` is enough). Failure paths intentionally still ping: standalone-Done merge fail (line ~1471) flipped `silent=True → silent=False` and prepends `⚠ Done hit a merge conflict — needs your attention.` (this path has no follow-up `_notify_user` mention so the banner itself must ping); chain merge fail and per-step chain failure use `suffix_override` with outcome-varied copy (`⚠ {chain_label} hit a merge conflict on {repo_ref}:{branch_ref} — tap Resolve / Retry / Discard.` and `⛔ {chain_label} blocked at {step} — needs your attention.`) so the Discord notification preview names the chain instead of the generic banner. `_CHAIN_EXIT_MESSAGES` gained `⚠`/`⛔` glyphs in front of each exit reason so the preview text differentiates info-vs-block at a glance.
...
- **Mid-session account failover no longer reattaches to a stale local session snapshot (t-4633).** When a session had been run on two accounts (e.g. after an earlier failover), both `~/.claude/projects/<enc>/<sid>.jsonl` and `~/.claude-klerk/projects/<enc>/<sid>.jsonl` existed but diverged. `_hydrate_session_for_account` previously short-circuited on `target_file.exists()` and the CLI's `--resume` reattached to whichever local snapshot happened to live on the failover target — often days old. Hydrate now picks the freshest JSONL across all configured accounts by parsing the **last-turn timestamp** from the append-only JSONL (`_jsonl_last_timestamp` tails 256KB→1MB, walks backward through newline-terminated lines, skips partials). On tie, prefer the target (no copy); else prefer largest size. If freshness is unknown for every candidate, fall through to the legacy cross-account scan unchanged. After a successful run, `_mirror_session_to_peer_accounts` copies the JSONL the CLI just wrote to every other configured account (skip-when-identical via `(size, mtime_ns)` match, peer sessions-index rebuilt on each write so the new copy is discoverable by `--resume`). The new hydrate-overwrite and peer-mirror writes go through `_atomic_copy_jsonl` (temp-then-rename with a brief Windows retry loop, 50/100/200ms backoff, for the case where a peer's CLI holds the file open mid-resume); pre-existing back-copy paths (`_copy_session_from_worktree`) are unchanged. Mirror failures log at WARNING with src/dst/account/sid so silent sync drift is visible in `bot.log` before the next failover regresses context. Freshness parses are cached on `(path, mtime_ns, size)` — the `size` key is load-bearing on coarse-mtime filesystems (SMB, FAT32) and must not be dropped by a future refactor.
...
### Fixed
- **Orphaned-unmerged-index recovery + kind-aware merge-failure banner (t-4114).** Real-world incident: thread `1503728770316501135` got stuck in an infinite "Replay succeeded → Merge failed: you need to resolve your current index first" loop. Root cause: AIAgent's main repo carried unmerged index entries (`AIAgent.csproj`, `CHANGELOG.md`, `Services/Automation/Backtesting/BacktestRunner.cs`) but no `MERGE_HEAD`, so `_check_main_repo_clean`'s MERGE_HEAD-only probe waved every retry through to the next `git checkout target` — which then refused with "you need to resolve your current index first". The user had no Discord-side signal that the problem was structural; the buttons looped indefinitely. Fix is two-fold:
  - **Auto-recovery (Path B) in `ClaudeRunner._check_main_repo_clean` (`bot/claude/runner.py`).** After the existing `MERGE_HEAD` cleanup (Path A) the precheck now runs `git ls-files --unmerged` and, when entries remain, tries `git reset --merge` first (preserves unrelated working-tree edits). If that's refused because dirty edits overlap the conflict, the precheck snapshots the working tree via `git stash create` + `git stash store -m "auto-stash poisoned-index-recovery <iso>" <sha>` (no pathspec — `stash push -- <unmerged paths>` was rejected as too version-fragile across git versions), then `git reset --hard HEAD` to clear the index. Stash-create + stash-store before any destructive reset is the safety contract: work always exists in a reachable ref before `--hard` runs. The return type changes from `Optional[str]` to a new `PrecheckResult(error, recovery_note)` dataclass so the success path can surface non-fatal recovery detail (e.g. the stash SHA) up the call chain — `merge_branch` appends `recovery_note` to its "Merged into master" return string so the user is told what we did to their repo (no silent stash creation). Three callers updated: `_merge_branch_sync` (line 2931), `_discard_branch_sync` (line 3529), `_fix_repo_head` (line 4275) — all swap `if precheck:` → `if precheck.error:`. Path A's "couldn't auto-abort" return is reclassified as `MERGE_FAIL_RECOVERY_FAILED` (distinct kind, distinct banner) so the user gets correct guidance instead of "Try Merge Again will fix it" when manual intervention is actually needed.
  - **Latent-bug fix bundled in the same edit:** the precheck-error return in `_merge_branch_sync` changed from `f"Merge skipped: {precheck}"` to `f"Merge failed: {precheck.error}"`. The v0.92.35 `merge_msg_is_failure` classifier is prefix-based on `"Merge failed:"`, so the prior `"Merge skipped:"` string was being silently routed through the success branch by every caller — `clear_pending_merge`, `clear_stale_branches`, `update_after_merge`, and `on_merged` all ran when no merge had occurred. Now precheck errors correctly trip the failure path. `_discard_branch_sync` keeps `"Discard skipped: …"` (discard messages don't flow through `merge_msg_is_failure`).
  - **Recovery-note plumbing on ALL exit paths.** Initial pass only surfaced `recovery_note` on the merge-success return; if precheck Path B stashed local changes and the merge then failed (or the call was a discard), the user got no signal that a stash was created. Fixed: `_merge_branch_sync` now appends `recovery_note` to both failure returns (`auto_resolved<0` and `CalledProcessError`), and `_discard_branch_sync` captures `precheck.recovery_note` and appends it to all three success outcomes (`Preserved …`, `Discarded (with warnings: …)`, `Discarded branch, back on …`). The "never silently mutate the main repo" contract now holds across every exit.
  - **Banner markdown preserved through `escape()`.** Three commands.py sites (`resolve_merge` CLEAN short-circuit, `resolve_merge` post-resolver, merge-button retry) had `ctx.messenger.escape(msg)` running on the banner-appended message — Discord escape replaces `*` `_` `` ` `` with `\*` `\_` `` \` ``, so `**Try Merge Again**` rendered as literal `\*\*Try Merge Again\*\*`. Each site now stores the banner in a separate `banner_suffix` variable, escapes only the runner output, and concatenates the banner unescaped. The other three surfaces (`/merge` slash, autopilot finalize, chain-step finalize) were already escape-free.
  - **Kind-aware banner copy (`bot/platform/formatting.py:merge_failed_banner`).** `merge_branch` now writes a `MERGE_FAIL_*` taxonomy entry to `ClaudeRunner._last_merge_failure_kind[instance.id]` after `git merge --abort` (via `_classify_post_abort_state` — must run AFTER abort, not before, so legitimate mid-flight conflicts don't get misclassified as orphaned-index). The classifier reads `git ls-files --unmerged` post-abort: residue → `ORPHANED_INDEX` ("Try Merge Again — next attempt will auto-recover, local changes preserved in stash"); clean → `GENERIC_FAILURE` (original copy). The precheck's "recovery already attempted and failed" path sets `RECOVERY_FAILED` ("manual intervention needed — run `git reset --merge`"). Kind is persisted on `pending_merge["failure_kind"]` via the new `state.set_pending_merge` `failure_kind=` kwarg and refresh-on-retry `set_pending_merge_failure_kind(instance_id, kind)` setter so the banner stays accurate across retries. Six merge-failure surfaces consume the helper: autopilot finalize (`workflows.py:_finalize` standalone-Done path), chain-step finalize (`workflows.py` chain merge step), `/merge` slash (`commands.py:on_merge`), merge button retry (`commands.py:_on_action` merge branch), and both `resolve_merge` follow-up merges (CLEAN short-circuit + post-resolver). Logical-site refs only; line numbers drift across edits. `_merge_branch_sync` clears the side-channel entry at the top of every call so a stale kind from a prior session can't leak into a fresh attempt. The classifier is `@staticmethod` and the side channel uses string-valued `MERGE_FAIL_*` constants so the JSON state round-trip is transparent and `formatting.py` can stay free of `runner.py` imports.
...
### Added
- **Orchestrator loop: child→parent callback + Resume button.** When a spawned child thread finalizes (COMPLETED/FAILED), the parent thread now receives a callback message with a "Resume parent" button — eliminating the manual back-and-forth where the user previously had to check each child and report completion back to the parent. New `bot/discord/orchestrator.py` builds the callback body (title ≤60 chars + status + child mention + summary truncated to 400 chars) and stores the synthesized resume prompt under a short `secrets.token_urlsafe(8)` token in `platform_state["discord"]["orch_resume_payloads"]` (works around Discord's 100-char `custom_id` cap — total payload `orch_resume:<token>` is ~23 chars). `_parent_is_alive()` dead-parent guard skips the callback if the parent thread is archived/locked/missing. `bot/engine/lifecycle.py` invokes `ctx.notify_parent_on_finalize(status, display_text)` once after `send_result()` for terminal states only, AND only when `not result.needs_input` — `finalize_run` flips `inst.status` to COMPLETED whenever the model paused asking the user a question (`lifecycle.py:480-484`), so a status-only check would falsely tell the parent "child done" while the child is actually waiting on a human reply. Errors in the post are logged but never raised so child finalize always completes. The Resume button (handled in `bot/discord/interactions.py` under `action == "orch_resume"`) pops the token-keyed payload, edits the message to strip the view (single-use), and replays the synth prompt into the parent thread via `_replay_to_thread(source="callback_resume")` so the orchestrator wave counter does NOT reset (callback resume is not a user-typed message).
- **Spawn wave cap (`_MAX_SPAWN_WAVES = 5`)** in `bot/engine/commands.py:_handle_spawn_directive`, bounding runaway orchestrator loops. Counter `spawn_wave_count` lives on `ThreadInfo` (turn-stable, persisted) and increments in `_spawn_session` against the parent's `ThreadInfo`. Reset is positive-rule: at the top of `on_text`, when `ctx.source == "user_message"`, the counter resets via `ctx.reset_spawn_wave_count()`. All other entry paths (replay, schedule, autopilot, callback resume, spawn dispatch) carry non-resetting source tags so the cap survives across the orchestrator's own iterations and only resets when a genuine human turn lands. Default `ctx.source = "system"` so a missing tag fails safe (does not reset).
- `RequestContext` (`bot/platform/base.py`) gains `source: str = "system"`, `notify_parent_on_finalize: Callable[[str, str], Awaitable[None]] | None`, `read_spawn_wave_count: Callable[[], int] | None`, `reset_spawn_wave_count: Callable[[], None] | None`. The Discord layer's `_ctx()` (`bot/discord/bot.py`) attaches three closures wiring these to `ForumManager` lookups + `post_parent_callback`; engine code stays platform-agnostic (mirrors existing `on_merged` / `add_verify_item` seam pattern).
- `ThreadInfo` (`bot/discord/forums.py`) gains `parent_thread_id: str | None = None` and `spawn_wave_count: int = 0`, persisted via truthy-gated `to_dict`/`from_dict` (zero values stay out of disk JSON, preserving forward/backward compat with existing `data/state.json`).

## v0.92.33 — Re-prime Discord context on compacted CLI resume (2026-05-12)

### Added
- Discord prime-briefing now also fires when `--resume` lands on a session JSONL that Claude Code has internally compacted, not just on cold-start threads. Background: thread `1501750744833855611` exposed the failure mode — a 3-day-old session resumed cleanly but the model couldn't identify "the Backtest Live-Parity design spec" because Claude Code had auto-summarised the early design Q-tasks into a single `isCompactSummary` user record, dropping the verbatim exchange. Old gating in `bot/engine/commands.py:744` only primed when there was no resumable session at all, so compacted resumes silently lost context. New `session_resume_state(session_id)` helper in `bot/engine/sessions.py` reads the resumed JSONL and returns `(compacted, already_rehydrated)` — `compacted` is True if any record has `subtype=="compact_boundary"` (the verified Claude Code marker; checked against real session JSONLs at `~/.claude/projects/.../*.jsonl` which write paired `system/compact_boundary` + `user/isCompactSummary` records mid-file at the point of compaction), `already_rehydrated` is True only if a `<<<PRIOR-` fence appears in a real user-prompt record (NOT an `isCompactSummary` record — its summary text could echo a previously-injected fence and false-trigger rehydration after re-compaction) whose record index is past the LAST `compact_boundary` (multiple compactions can occur in one session — the verification turned up 2 boundaries at lines 179 and 520 in one file — so a fence sitting above the latest boundary no longer counts because Claude Code's resume context starts from that boundary's summary). Content scanning handles both `message.content` shapes (string and list-of-blocks). On any read/parse error the helper returns `(False, False)` so the caller degrades to "don't inject" rather than injecting on bad data. Gating now fires `mode="resume"` priming when `compacted and not already_rehydrated`; healthy resumes pay zero extra tokens (prompt cache stays warm) and already-rehydrated resumes skip too (the briefing is already in the resumed context as a prior turn). `build_prime_briefing` (`bot/discord/forums.py`) takes new `mode` and `use_cache` keyword args: cold mode keeps the existing ~500-token brief (3 user × 350 chars + 1 bot × 200 chars, history_limit=50, HARD_BYTE_BUDGET=64 KiB) and the in-memory `_prime_cache` (single fire per spawn); resume mode raises caps to ~12K-token brief (15 user × 2000 chars + 8 bot × 2000 chars, history_limit=100 — Discord's per-call max — HARD_BYTE_BUDGET=128 KiB) and bypasses the cache because the briefing must reflect any new Discord messages that landed since the prior compaction-triggered prime. The `Messenger.maybe_prime_briefing` callback signature in `bot/platform/base.py:209` widens from `Callable[[], Awaitable[str | None]]` to `Callable[[str], Awaitable[str | None]]` carrying the mode arg; the Discord lambda at `bot/discord/forums.py:1719` now reads `lambda mode="cold", _tid=thread_id: self.build_prime_briefing(_tid, mode=mode, use_cache=(mode == "cold"))`. Status-message copy varies by path: cold-start keeps "Reconstructing context…" (existing UX), resume path posts "Refreshing context after long gap…" so the mid-thread flicker reads as intentional rather than buggy. The injected prompt preamble also varies — resume-mode preamble accurately tells the model the session resumed but its history was compacted (the cold-start preamble's "previous CLI session is no longer accessible" would have been a lie on the resume path). Fence prefix `<<<PRIOR-` is hardcoded in two places (`forums.py:1821` writer, `sessions.py:_PRIOR_FENCE_PREFIX` reader) — both sites carry an explicit `must match` comment so a future rename in either is caught at review (extracting to a shared constant was deferred as the Medium DRY revision in this plan's review chain). No store-schema change. No new persistent state. Self-healing: rehydration is decided by re-reading the JSONL each turn, so if Claude Code re-compacts after the fence was injected, the next prompt re-fires with a fresh briefing. Tradeoff: on already-compacted long-lived sessions the per-prompt JSONL scan reads the full file every turn — acceptable for now (still sub-second on the heaviest threads observed), can be optimised with reverse-scan-and-stop later if it shows up in profiling.
...
### Fixed
- Manual `Build` button (and any non-autopilot Build entry point) now re-injects the verbatim plan into the resumed Claude session, closing the failure mode observed on Discord thread `1502626771843944569` where two consecutive builds reported "the previous context window was compacted, I need the verbatim step 2 plan back" and halted. `BUILD_PLAN_INJECTION_PREFIX` (`bot/config.py:546`) was already wired into the autopilot chain runner's two build sites at `bot/engine/workflows.py:2417` (phase build) and `:2583` (single-shot autopilot build) but was missing from the manual `on_build()` path at `bot/engine/workflows.py:713-778`; that path now calls `_extract_latest_plan_text(ctx, [], source_id)` and prepends the prefix when a plan is recovered. `_extract_latest_plan_text` itself gains a third fallback that fires when both the in-flight `chain_instances` walk and the PLAN-source check come up empty: it scans `ctx.store.list_instances(all_=False)` (24h window, most-recent-first) for any same-`session_id` `APPLY_REVISIONS` (preferred) or `PLAN` instance whose `created_at <= source.created_at`, parsing both timestamps via `datetime.fromisoformat` rather than lexicographic string compare so a stray `Z`-suffixed writer can't corrupt ordering, and coercing tz-naive parses to UTC before the `>` comparison so a naive writer can't trigger a `TypeError: can't compare offset-naive and offset-aware datetimes` that would propagate out and halt Build mid-flight. Defensive guards return `""` on missing/malformed `source.created_at` instead of TypeErroring mid-Build. The 24h list-instances cutoff is the soft recency cap that keeps unrelated old plans from same-thread sessions from being resurrected; the `<= source.created_at` bound prevents resurrection of plans created after the user's click. `/retry` is covered transitively — `on_retry` (`bot/engine/commands.py:1198-1216`) clones `inst.prompt` verbatim, so the original Build's already-injected prompt carries through automatically. Regression coverage: `scripts/test_extract_latest_plan_text.py` (14 cases — chain-walk happy path, PLAN-source fallback, store-fallback session/recency filters, malformed-timestamp resilience, tz-naive-vs-aware comparison resilience in both directions, `APPLY_REVISIONS`-over-`PLAN` precedence, metadata stripping).
...
### Fixed
- Defer release tag creation until after merge to eliminate the orphan-tag failure class. Tags live in the global `.git/refs/tags/` namespace shared across all worktrees, so a release session that ran `git tag vX.Y.Z` inside its build worktree and was then Discarded (or whose chain was abandoned) left the tag pointing at an unreachable commit on the deleted branch — re-pollution that broke subsequent version-pick logic and required manual `git tag -d` cleanup. New flow: the LLM commits `vX.Y.Z: Summary` but does NOT run `git tag` (`_RELEASE_STEPS` and `RELEASE_PROMPT` in `bot/config.py` updated). After the merge to master succeeds, `_merge_branch_sync` (`bot/claude/runner.py`) walks the branch-unique commit window via the new `_release_commit_window` helper (`git log --first-parent HEAD^1..HEAD^2`, capped at `_TAG_WALKBACK_CAP=50`) and the new `_tag_release_at` helper matches subjects against `_RELEASE_COMMIT_RE` (supports 4-part `v1.3.0.255` style), creates the tag at the matching commit, and is idempotent (returns the same tag if it already points at the release commit) and conflict-safe (refuses to clobber a tag that already exists on a different commit). Existing-tag check uses `refs/tags/<name>^{commit}` to dereference annotated tags down to the commit they point at, so an annotated tag already at the release commit takes the idempotent path instead of falsely tripping no-clobber on the tag-object SHA. Wrapped in `try/except Exception` so a future helper bug can't poison the merge push path. The standalone `/release` flow (`on_release` in `bot/engine/commands.py`) calls the same helper with `branch_tip="HEAD"` after the LLM session completes, so both autopilot and manual release paths converge on the same idempotent tag-create code. Tag-push block now walks `candidate_shas` per-sha and dedupes — picks up both the bot-created tag and any legacy LLM-self-tagged commits during the transition.
- Restore the `_tag_release_at` and `_release_commit_window` helpers in `bot/claude/runner.py` that the v0.92.24 call site in `bot/engine/commands.py:on_release` referenced but were never defined on master. Until this release any `/release` invocation would have `AttributeError`'d when the LLM session completed and the tag-create branch ran.

### Tests
- New `scripts/test_tag_release.py` regression covers seven scenarios using temp git repos and the real `ClaudeRunner._tag_release_at`: (a) tip is the release commit with 4-part version `v1.3.0.255`, (b) cleanup commit after release commit — walk-back finds the right one, (c) idempotent return when the tag is already at the target, (d) no-clobber when the tag exists at a different commit, (e) no matching subject in the window — returns `(None, [shas])` so the push block can still walk for legacy tags, (f) empty branch-unique range returns `(None, [])`, (g) annotated tag already at the release commit takes the idempotent path (locks in the `^{commit}` deref).
...
- New startup probe `scan_orphan_release_commits` walks non-terminal RELEASE/DONE instances at boot, looking for a `vX.Y.Z:` commit on the instance's worktree HEAD with no matching git tag — the signature of a release session that crashed between bumping and tagging. Marks the instance FAILED, clears its autopilot chain state (`clear_autopilot_chain` + `clear_chain_entry_sha`) so `_resume_interrupted_chains` doesn't silently re-spawn another release attempt against the same session, and surfaces a one-line warning into the Discord thread (`_surface_release_recovery_events` in `bot/discord/bot.py`). Conservative on the no-worktree / non-release-shaped HEAD / tag-already-exists cases — leaves them for existing recovery paths so we don't misattribute another worktree's release commit to this instance (the multi-worktree scenario this fix exists for).
...
## v0.92.20 — Idempotent merge against stale branch (2026-05-06)
...
### Tests
- New `scripts/test_worktree_recovery_dedupe.py` regression covers seven scenarios: chained-sibling dedupe (4 → 1), FAILED/KILLED skip with COMPLETED/RUNNING/QUEUED kept, positive lock that COMPLETED is scanned, flagged-sibling silences whole branch, post-filter handles flag-found-late iteration order, FAILED+flagged still silences branch (locks in the flag-before-status check order), and missing-required-fields skip.
## v0.92.19 — Inject master drift on resume + auto-abort MERGE_HEAD residue (2026-05-06)

### Fixed
- Resumed/compacted sessions now see "what shipped to master while I was away" so they don't blindly rebuild already-merged work. New `_build_master_context_block` (`bot/claude/runner.py`) emits last 10 commits on the default branch, a "since this session started" delta against `Instance.master_baseline_head` (lazily captured on first prompt build, inherited through `spawn_from`, capped at 30 lines), and a branch-vs-master diff stat that explicitly warns when the diff is empty (likely already merged). Injected at TWO sites: always into `_build_system_prompt` (covers fresh spawns), and prepended to the user-message prompt whenever `instance.session_id` is set (covers every resume — including the Build button path where `instance.prompt` is non-empty, which is the exact code path that produced the duplicate-rebuild incident). The user-message copy is the load-bearing one against the `--resume` verbatim-replay case where prompt-builder edits to the system prompt never reach the LLM. Field is added to `Instance.to_dict`/`from_dict` so the baseline survives reboots via the existing `finalize_run` save path.
...
- Refactored `_find_mergeable_instance` to share the new `_find_session_branch_instance` helper, so the merge-resume path and the chain detector can't drift out of sync when one gets a new criterion added.
...
- `_review_plan_loop` now appends each successful intermediate apply to `chain_instances`. Previously these spawns were invisible to the chain caller, so the chain-cost sum at the budget gate undercounted by however many apply rounds ran, and `_extract_latest_plan_text` would fall back to the original plan in the converged-with-no-deferred case (apply ran in round N, round N+1 review was clean, last_review was the only thing visible).
- `_extract_latest_plan_text` now only falls back to the chain's source instance when its origin is `PLAN`. Without this guard, a build-and-ship chain launched from a query/BG task would have prepended the agent's reply text under "Plan to implement (verbatim) — implement it exactly", actively misleading the build agent.
...
## v0.92.5 — Verify Board emoji-prefix rename (2026-04-28)

### Changed
- Verify Board thread name now uses the `✅ Verify Board` emoji-prefixed Title Case naming pattern matching the other pinned threads (Control Room, Archive, Monitor) for visual scanning in the Discord sidebar. Existing `verify-board` threads are auto-renamed in-place by `ensure_verify_board()` on next access (covers both `verify-board` and unprefixed `Verify Board`).

## v0.92.4 — Image attachments readable in non-bot repos and worktrees (2026-04-28)
...
### Changed
- Mode-button taps, the welcome-embed mode picker, and `/mode <build|plan|explore>` slash command now refresh the forum tag immediately (previously the tag only refreshed when the next instance finished, leaving a stale-tag window after a manual mode change). All three mode-changing paths in `bot/discord/interactions.py` (mode_* instance buttons, `_handle_mode_set` welcome picker) and `cmd_mode` in `bot/discord/slash_commands.py` now schedule `_try_apply_tags_after_run` post-toggle.

## v0.92.1 — Verify-board cleanup migration (2026-04-28)
...
- Threads no longer accumulate orphan sessions across crash recovery (mechanism: layer-1 cross-account swap fired before layer-2 session-index rebuild, so claude.exe's "No conversation found" — caused when the JSONL is on disk but missing from the CLI's index after a mid-stream crash — got "treated" by rebuilding the *wrong* account's index, forcing layer 3 to drop the session and create a fresh one. Each subsequent message hit the same trap and rebound again, fragmenting history across N orphaned JSONLs in 90s for the t-3260 case). `bot/claude/runner.py` now runs index rebuild on the *current* account FIRST, then cross-account, then last-resort drop.
- Recovery-exhaustion now surfaces a Discord notice instead of silently rebinding to a fresh session. When layer 3 fires (resume failed permanently), `bot/engine/commands.py` posts "⚠️ Lost prior conversation context… re-paste anything important" so the dementia is visible. Plumbed via new `RunResult.session_recovery_exhausted` flag set by the runner's layer-3 path.
...
- Release verifier no longer fires a false "mismatch" when the local CHANGELOG regex can't parse `[Unreleased]` but the LLM verdict is `ok`. The LLM sees the full diff and is now authoritative; the regex hit was a tooling artifact that was synthesizing a fake mismatch and forcing an Amend/Continue prompt on clean releases. Logs a `warning` when this happens so a real regex regression remains visible.
- Verifier-output-unparseable gate copy no longer mislabels itself as "Phantom claims" — when phantoms is empty (the only path that now reaches the gate without real phantoms), the body says "Release verifier output couldn't be parsed — failing closed" instead.
## v0.91.2 — Fix Windows cp1252 git-output decoding crash on autopilot resume (2026-04-27)

### Fixed
- Smoke test reports UNHEALTHY on every reboot due to autopilot-resume crashes (mechanism: `subprocess.run(text=True)` on Windows decodes git output via cp1252; UTF-8-only commit-message characters crash the reader thread silently and `subprocess.run` returns `CompletedProcess(stdout=None, returncode=0)`, sneaking past the `returncode != 0` early-return guard and crashing on `len(None)` / `None.split()` in `on_verify_release`). Fixed in `bot/engine/workflows.py` by setting `encoding="utf-8", errors="replace"` on the five `git` subprocess calls in `_git_head_sha`, `_git_log_messages`, `_git_diff_stat`, and `_git_diff_payload`, plus `(r.stdout or "")` belt-and-suspenders so any future None can't recreate the crash. Same risk exists in ~50 other `subprocess.run(text=True)` callsites across the bot — left for a separate sweep.
...
## v0.81.0 — Verify Board (2026-04-25)
