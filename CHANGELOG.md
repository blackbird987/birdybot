...
### Added
- `bot/engine/lifecycle.should_bind_session()` / `bind_thread_session()` — the single place that decides whether a turn may claim the thread's session, and the single place that writes it. Both the chat path and the cooldown retry now go through them instead of open-coding the rule.
- `scripts/test_cooldown_session_bind.py` — locks the eligibility rule (success and usage-limit bind; crash and recovery-exhausted don't), drives a real `ForumManager` to prove the bind reaches `ThreadInfo` and that the cross-repo rebind guard is untouched, and asserts the source order that was the actual bug: the bind must sit above the cooldown early-return. Registered in `.claude/test.json`.
