...
### Changed
- Mode-button taps and `/mode <build|plan|explore>` slash command now refresh the forum tag immediately (previously the tag only refreshed when the next instance finished, leaving a stale-tag window after a manual mode change). Hooks added to the mode_ branch in `bot/discord/interactions.py:455-461` and to `cmd_mode` in `bot/discord/slash_commands.py:179-187`, both calling `_try_apply_tags_after_run` post-toggle.
