...
- `scripts/test_fleet_ship.py` — contract tests: pending-verify add/pop round-trip + dedup + multi-repo drain source, `_finalize_merge` `skip_close` signature (keyword-only, default False), empty healthcheck returns True, verify-prompt wording per deploy method, `_commits_ahead` returns 0 on garbage (no-op branches never ship).
