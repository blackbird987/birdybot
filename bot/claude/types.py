...
class InstanceOrigin(str, Enum):
    ...
    SENSOR_FIX = "sensor_fix"   # Sensors step — fix deterministic check failures
...
class RunResult:
    ...
    # ``proc.terminate()`` via ``kill_and_wait`` (Steer / resolve-cancel)
    # AND the exit shape matched a kill. The lifecycle layer reads this to
